...
def make_it_stand(pose):
    ...
    pose[:,np.array([2,5,8,11,])+1] = 0    
    pose[:,np.array([14,])+1] = 0    
    return pose
...
def run(cfg,
    ...
    if not (osp.exists(osp.join(output_pth, 'tracking_results.pth')) and 
        ...
        # TODO: mod video framerate here
        if run_global: slam = SLAMModel(video, output_pth, width, height, calib, stride = stride)
        else: slam = None
        ...
        while (cap.isOpened()):
            ...
            if args.anno_pkl_path is not None:
                ...
                for person_id in persons_ids:
                    ...
                    if frame_idx<person_data['st_frame'] or frame_idx>person_data['et_frame']: continue
                    if cur_idx>=len(person_data['track_bbox']): continue
                    bboxes = person_data['track_bbox'][cur_idx]
                    bboxes = [bboxes[0], bboxes[1], bboxes[0] + bboxes[2], bboxes[1] + bboxes[3]]
                    contact_detect_result = person_data['contact_detect_result'][cur_idx]
                    pre_def_bboxes.append({'bbox': bboxes, 'contact': contact_detect_result})
    ...
    for subj in range(n_subjs):
        ...
        if args.vis_kpts:
            ...
            if not os.path.exists(save_path):
                ...
                tmp = np.concatenate([tmp,np.zeros((tmp.shape[0],6,3)),dst_data['face_keyp'],dst_data['left_hand_keyp'],dst_data['right_hand_keyp']],axis=1)
                save_kpts2d(
                    toth(tmp), save_path, fps, int(width), int(height), dataset='TopDownCocoWholeBodyDataset',
                    bg_gray=0
                    )
        ...
        if args.inp_body_pose is not None:
            ...
            body_pose = poses[:,:24,:]
            hand_pose = poses[:,25:,:]
            smplerx_hand_pose = torch.stack([hand_pose[:,:15,:], hand_pose[:,15:,:]], dim=1).unsqueeze(0) #(1, T, 2, 15, 6)
            
            pred['pose'] = body_pose.reshape(-1,24*6).unsqueeze(0)
            pred['cam'] = toth(inp_data['trans']).squeeze().unsqueeze(0)

        if args.use_hmr2:
            pred['pose'] = a2s(toth(np.concatenate([dst_data['hmr_orient'],dst_data['hmr_body_pose']],axis=1))).reshape(-1,24*6).unsqueeze(0)
            pred['cam'] = toth(dst_data['hmr_pred_cam']).squeeze().unsqueeze(0)
            pred['betas'] = toth(dst_data['hmr_betas']).squeeze().unsqueeze(0)
         
        flame2_smplx_convt = toth(np.load('model_files/FLAME2020/flame2020to2019_exp_trafo.npy'))
        ...
        pred_root_world_no_fit = matrix_to_axis_angle(pred['poses_root_world']).reshape(-1, 3)
        ...
        if args.run_smplify:
            print('run_smplify...')
            if args.use_smplx:
                smplify = TemporalSMPLifyX(smplx, img_w=width, img_h=height, device=cfg.DEVICE)
        ...
        if (
            'face_keyp' in dst_data and 
            'left_hand_keyp' in dst_data and 
            'right_hand_keyp' in dst_data
        ):
            tmp = dst_data['keypoints']
            whole_body_kpts = np.concatenate([tmp,np.zeros((tmp.shape[0],6,3)),dst_data['face_keyp'],dst_data['left_hand_keyp'],dst_data['right_hand_keyp']],axis=1)            
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video', type=str, default='examples/demo_video.mp4', help='input video path or youtube link')
    parser.add_argument('--output_pth', type=str, default='/apdcephfs/private_wallyliang/wham_private_output', help='output folder to write results')
    parser.add_argument('--calib', type=str, default=None, help='Camera calibration file path')
    parser.add_argument('--estimate_local_only', action='store_true',help='Only estimate motion in camera coordinate if True')
    ...
    parser.add_argument("--not_full_body", action='store_true', help='do not force filter out partial body frame.')
    parser.add_argument('--anno_pkl_path', type=str, default=None, help='deprecated.')
    parser.add_argument('--use_smplx', action='store_true', help='Use SMPLX model instead of original SMPL.')
    parser.add_argument('--use_hamer', action='store_true', help='HAMER for right hands.')
    parser.add_argument('--use_deca', action='store_true', help='DECA for exp prediction.')
    parser.add_argument('--use_smirk', action='store_true', help='SMRIK for exp prediction.')
    parser.add_argument('--use_acr', action='store_true', help='ACR for interhands prediction.')
    ...
    parser.add_argument('--vitpose_model', type=str, default='ViTPose+-G', help='ViTPose+-G or ViTPose+-B')
    ...
    #<==== AUTO
    parser.add_argument('--save_wham_npz', action='store_true',help='save wham pose if run optimization.')
    parser.add_argument('--run_smplify', action='store_true',help='Run Temporal SMPLify for post processing, not used, we switch it automatically')
    parser.add_argument("--replace_lower", action='store_true', help='for speech scnerio, replace lower body with predifined motion. we switch it automatically')
    parser.add_argument('--use_hmr2', action='store_true', help='HMR2 for half body pose prdiction. we switch it automatically')
    #<==== AUTO
    
    parser.add_argument('--inp_body_pose', type=str, default=None, help='if set, we will use it as body pose initialization, only support single person video.')
    ...
    logger.add(str(Path(output_pth)/'main.log'), rotation="10 MB", level="DEBUG", format="{time} {message}")
