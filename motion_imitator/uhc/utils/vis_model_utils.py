...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--in_model",
        type=str,
        default="assets/mujoco_models/models/character1/model.xml",
    )
    parser.add_argument(
        "--out_model",
        type=str,
        default="assets/mujoco_models/models/character1/model_vis.xml",
    )
    args = parser.parse_args()

    in_model = (
        f"assets/mujoco_models/models/{args.cfg}/model.xml"
        if args.cfg is not None
        else args.in_model
    )
    out_model = (
        f"assets/mujoco_models/models/{args.cfg}/model_vis.xml"
        if args.cfg is not None
        else args.out_model
    )
