...
def lbs(
    ...
    if pose2rot:
        if pose.numel() == batch_size * 24 * 4:
            rot_mats = quat_to_rotmat(pose.reshape(batch_size * 24, 4)).reshape(
                batch_size, 24, 3, 3
            )
        else:
...
def batch_get_pelvis_orient(rel_pose_skeleton, rel_rest_pose, parents, children, dtype):
    ...
    center_final_loc = (
        center_final_loc
        - torch.sum(center_final_loc * spine_norm, dim=1, keepdim=True) * spine_norm
    )
    center_rest_loc = (
        center_rest_loc
        - torch.sum(center_rest_loc * spine_norm, dim=1, keepdim=True) * spine_norm
    )
...
@torch.jit.script
def batch_get_pelvis_orient_svd(
...
def batch_inverse_kinematics_transform_old(
    pose_skeleton, global_orient, phis, rest_pose, children, parents, leaf_thetas
):
    ...
    final_pose_skeleton = (
        final_pose_skeleton - final_pose_skeleton[:, 0:1] + rel_rest_pose[:, 0:1]
    )
    ...
    for i in range(1, parents.shape[0]):
        if children[i] == -1:
            ...
            # (B, 3, 1)
            child_final_loc = (
                final_pose_skeleton[:, children[i]] - rotate_rest_pose[:, i]
            )
...
def batch_inverse_kinematics_transform(
    ...
    for idx_lev in range(1, len(idx_levs)):
        ...
        if idx_lev == len(idx_levs) - 1:
            ...
            rotate_rest_pose[:, indices] = rotate_rest_pose[
                :, parents[indices]
            ] + torch.matmul(
            ...
            children_final_loc = (
                final_pose_skeleton[:, spine_child] - rotate_rest_pose[:, [i]]
            )
            children_rest_loc = rel_rest_pose[:, spine_child]

            rot_mat = batch_get_3children_orient_svd(
                children_final_loc.clone(),
                children_rest_loc.clone(),
            ...
            rot_mat_chain[:, i] = torch.matmul(
                rot_mat_chain[:, parents[i]].clone(), rot_mat
            )
            ...
            # (B, K, 3, 1)
            rel_pose_optimized = (
                final_pose_skeleton[:, children[indices]] - rotate_rest_pose[:, indices]
            )
            ...
            child_final_loc = torch.matmul(
                rot_mat_chain[:, parents[indices]].transpose(2, 3), rel_pose_optimized
            )

            child_rest_loc = rel_rest_pose[:, children[indices]]  # need rotation back ?
            # (B, K, 1, 1)
            child_final_norm = torch.norm(
                child_final_loc, dim=2, keepdim=True
            )  # \arrow{p_k}
            child_rest_norm = torch.norm(
                child_rest_loc, dim=2, keepdim=True
            )  # \arrow{t_k}
            ...
            # (B, K, 1, 1)
            cos = torch.sum(child_rest_loc * child_final_loc, dim=2, keepdim=True) / (
                child_rest_norm * child_final_norm + 1e-8
            )
            sin = axis_norm / (child_rest_norm * child_final_norm + 1e-8)
            ...
            K = torch.cat([zeros, -rz, ry, rz, zeros, -rx, -ry, rx, zeros], dim=2).view(
                (batch_size, len_indices, 3, 3)
            )
