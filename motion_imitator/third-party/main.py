...
class ACModel2(torch.nn.Module):
    ...
    def __init__(self, state_dim: int, act_dim: int, goal_dim: int=0, value_dim: int=1, 
        ...
        self.actor = ACModel.Actor(state_dim, act_dim, self.goal_dim_actor, init_mu=init_mu, init_sigma=init_sigma)
...
def main_adaptnet():
    ...
    parser.add_argument("config", type=str,help="Configure file used for training. Please refer to files in `config` folder.")
    parser.add_argument("--meta", type=str, default=None,help="Pretrained meta checkpoint file.")
    parser.add_argument("--ckpt", type=str, default=None,help="Checkpoint directory or file for training or evaluation.")
    parser.add_argument("--test", action="store_true", default=False,help="Run visual evaluation.")
    parser.add_argument("--seed", type=int, default=42,help="Random seed.")
    parser.add_argument("--device", type=int, default=0,help="ID of the target GPU device for model running.")
    ...
    if hasattr(config, "training_params"): TRAINING_PARAMS.update(config.training_params)
    if not TRAINING_PARAMS["save_interval"]: TRAINING_PARAMS["save_interval"] = TRAINING_PARAMS["max_epochs"]
    training_params = namedtuple('x', TRAINING_PARAMS.keys())(*TRAINING_PARAMS.values())

    if hasattr(config, "discriminators"):
        discriminators = {name: envs.DiscriminatorConfig(**prop) for name, prop in config.discriminators.items()}
    else:
    ...
    env = getattr(envs, config.env_cls)(num_envs,discriminators=discriminators,compute_device=settings.device, **config.env_params)
    if settings.test:
    ...
    model = ACModel2(env.state_dim, env.act_dim, env.goal_dim, value_dim, meta_goal_dim=map_dim)
    discriminators = torch.nn.ModuleDict({name: Discriminator(dim) for name, dim in env.disc_dim.items()})
    ...
    if settings.test:
        ...
        buffer = dict(s=[], a=[], v=[], lp=[], v_=[], not_done=[], terminate=[],ob_seq_len=[])
        multi_critics = env.reward_weights is not None
        if multi_critics:buffer["reward_weights"] = []
        has_goal_reward = env.rew_dim > 0
        if has_goal_reward:buffer["r"] = []

        buffer_disc = {name: dict(fake=[], real=[], seq_len=[]) for name in env.discriminators.keys()}
            ...
            if len(buffer["s"]) == HORIZON:
                with torch.no_grad():
                    ...
                    for name, data in buffer_disc.items():
                        ...
                        length = torch.arange(fake.size(1), dtype=end_frame.dtype, device=end_frame.device)
                ...
                for name, disc, ref, ob, seq_end_frame_ in disc_data_training:
                    ...
                    for batch in range(n_samples//BATCH_SIZE):
                        ...
                        grad = torch.autograd.grad(score_interp, interp, torch.ones_like(score_interp),retain_graph=True, create_graph=True, only_inputs=True)[0]
                ...
                with torch.no_grad():
                    ...
                    length = torch.arange(env.ob_horizon, dtype=ob_seq_lens.dtype, device=ob_seq_lens.device)
                ...
                # import ipdb;ipdb.set_trace()
                print("Epoch: {}, Loss: {:.4f}/{:.4f}, Reward: {}, Lifetime: {:.4f} -- train comsume time: {:.4f}s".format(
...
def main_composite():
    ...
    if hasattr(config, "discriminators"):
        discriminators = {name: env.DiscriminatorConfig(**prop) for name, prop in config.discriminators.items()}
    else:
    ...
    env = env_cls(num_envs,discriminators=discriminators,compute_device=settings.device, **config.env_params)
    if settings.test:
        env.episode_length = 500000
