...
class DiscriminatorConfig(object):
    def __init__(
        ...
        replay_speed: Optional[Callable] = None,
...
DiscriminatorProperty = namedtuple(
    "DiscriminatorProperty", "name key_links parent_link replay_speed ob_horizon id"
)
Skeleton = namedtuple("Skeleton", "nodes parents trans rot")
Motion = namedtuple(
    "Motion", "fps pos orient ang_vel lin_vel local_q local_p local_vel"
)
...
class Env(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.character_model = (
            self.CHARACTER_MODEL if character_model is None else character_model
        )

        sim_params = self.setup_sim_params()
        self.gym = gymapi.acquire_gym()
        self.sim = self.gym.create_sim(
            compute_device, graphics_device, gymapi.SIM_PHYSX, sim_params
        )
        ...
        self.actor_ids = torch.arange(
            n_actors_per_env * len(self.envs), dtype=torch.int32, device=self.device
        ).view(len(self.envs), -1)
        controllable_actors = []
        for i in range(self.gym.get_actor_count(self.envs[0])):
            dof = self.gym.get_actor_dof_count(self.envs[0], i)
            if dof > 0:
                controllable_actors.append(i)
        self.actor_ids_having_dofs = n_actors_per_env * torch.arange(
            len(self.envs), dtype=torch.int32, device=self.device
        ).unsqueeze(-1) + torch.tensor(
            controllable_actors, dtype=torch.int32, device=self.device
        ).unsqueeze(
            -2
        )
        ...
        self.cam_target = gymapi.Vec3(
            *self.vector_up(1.0, [base_pos[0], base_pos[1], base_pos[2]])
        )

        self.simulation_step = 0
        self.lifetime = torch.zeros(
            len(self.envs), dtype=torch.int64, device=self.device
        )
        ...
        for i in range(self.gym.get_actor_count(self.envs[0])):
            ...
            print(
                "Links", sorted(rigid_body.items(), key=lambda x: x[1]), len(rigid_body)
            )
    ...
    def setup_sim_params(self, physx_params=dict()):
        ...
        p.physx.contact_collection = gymapi.ContactCollection(
    ...
    def create_envs(self, n: int, actuate_all_dofs=True, asset_options=dict()):
        ...
        if actuate_all_dofs:
            self.actuated_dof = [
                i for i in range(self.gym.get_asset_dof_count(actor_asset))
            ]
        else:
            actuators = []
        ...
        for i in range(n):
            env = self.gym.create_env(
                self.sim, spacing_lower, spacing_upper, n_envs_per_row
            )
            actor = self.gym.create_actor(
                env, actor_asset, start_pose, "actor", i, -1, 0
            )
        ...
        cam_pos = gymapi.Vec3(
            *self.vector_up(
                ...
                [
                    ...
                    base_pos[2] + self.camera_pos[1],
                ],
            )
        )
        self.gym.viewer_camera_look_at(
            self.viewer, self.envs[tar_env], cam_pos, self.cam_target
        )
        self.gym.subscribe_viewer_keyboard_event(self.viewer, gymapi.KEY_ESCAPE, "QUIT")
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_F, "TOGGLE_CAMERA_FOLLOWING"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_P, "TOGGLE_PAUSE"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_D, "SINGLE_STEP_ADVANCE"
        )
    ...
    def update_camera(self):
        tar_env = len(self.envs) // 4 + int(len(self.envs) ** 0.5) // 2
        cam_trans = self.gym.get_viewer_camera_transform(
            self.viewer, self.envs[tar_env]
        )
    ...
    def create_tensors(self):
        ...
        self.joint_tensor = joint_tensor.view(
            len(self.envs), num_dof, -1
        )  # n_envs x n_dof x 2
    ...
    def setup_action_normalizer(self):
        ...
        for i in range(self.gym.get_actor_count(self.envs[0])):
            ...
            if self.control_mode == "torque":
                ...
                action_lower.extend(
                    [
                        min(dof_prop["lower"][j], dof_prop["upper"][j])
                        for j in self.actuated_dof
                    ]
                )
                action_upper.extend(
                    [
                        max(dof_prop["lower"][j], dof_prop["upper"][j])
                        for j in self.actuated_dof
                    ]
                )
        ...
        self.action_offset = torch.tensor(
            action_offset, dtype=torch.float32, device=self.device
        )
    ...
    def reset_envs(self, env_ids):
        ...
        self.gym.set_actor_root_state_tensor_indexed(
            self.sim, gymtorch.unwrap_tensor(self.root_tensor), actor_ids, n_actor_ids
        )
        actor_ids = self.actor_ids_having_dofs[env_ids].flatten()
        n_actor_ids = len(actor_ids)
        actor_ids = gymtorch.unwrap_tensor(actor_ids)
        self.gym.set_dof_state_tensor_indexed(
            self.sim, gymtorch.unwrap_tensor(self.joint_tensor), actor_ids, n_actor_ids
        )
    ...
    def step(self, actions):
        ...
        self.request_quit = (
            ...
            else self.gym.query_viewer_has_closed(self.viewer)
        )
...
class ReferenceMotion:
    def __init__(
        self,
        motion_file: str or Sequence[str],
        character_model: str,
    ...
    def load_motions(self, motion_file):
        ...
        def compute_motion(fps: int, skeleton: Skeleton, local_q, local_p):
            ...
            for nid in range(len(skeleton.nodes)):
                ...
                if pid == -1:
                    ...
                    pos.append(
                        rotatepoint(skeleton.rot[nid].unsqueeze(0), local_p[:, nid])
                    )
                    root = nid
                else:
                    ...
                    pos.append(
                        ...
                        + rotatepoint(
                            orient[pid],
                            local_p[:, nid] + skeleton.trans[nid].unsqueeze(0),
                        )
                    )
        ...
        def load_mjcf(filename: str):
            ...
            if world is None:
                raise ValueError(
                    "Failed to find worldbody definition from MJCF file", filename
                )
            root = world.find("body")
            if root is None:
                raise ValueError(
                    "Failed to find any body definition from MJCF file", filename
                )
        ...
        for f, motion_file in enumerate(motion_files):
            print(
                "Loading {:d}/{:d} motion files: {:s}".format(
                    f + 1, n_motion_files, motion_file
                )
            )
            ...
            if os.path.splitext(motion_file)[1] == ".json":
                ...
                assert (
                    "fps" in motion_data.keys() or "sampling_rate" in motion_data.keys()
                )
            ...
            if tot_length != tot_length_with_weights:
                ...
                if curr_weight is None or curr_weight < 0:
                    if tot_length_with_weights == 0:
                        ...
                        curr_weight = (
                            curr_length * tot_weights / tot_length_with_weights
                        )
                elif curr_weight > 0:
        ...
        print(
            "Loaded {:d} motions with a total length of {:.3f}s.".format(
                len(self.motion), sum(self.motion_length)
            )
        )
    ...
    def state(self, motion_ids, motion_times):
        ...
        joint_q0 = torch.empty(
            (n, n_controllable_links, 4), dtype=torch.float, requires_grad=False
        )
        joint_q1 = torch.empty(
            (n, n_controllable_links, 4), dtype=torch.float, requires_grad=False
        )
        joint_p0 = torch.empty(
            (n, n_controllable_links, 3), dtype=torch.float, requires_grad=False
        )
        joint_p1 = torch.empty(
            (n, n_controllable_links, 3), dtype=torch.float, requires_grad=False
        )
        joint_vel = torch.empty(
            (n, n_controllable_links, 3), dtype=torch.float, requires_grad=False
        )
        link_pos0 = torch.empty(
            (n, n_key_links, 3), dtype=torch.float, requires_grad=False
        )
        link_pos1 = torch.empty(
            (n, n_key_links, 3), dtype=torch.float, requires_grad=False
        )
        link_orient0 = torch.empty(
            (n, n_key_links, 4), dtype=torch.float, requires_grad=False
        )
        link_orient1 = torch.empty(
            (n, n_key_links, 4), dtype=torch.float, requires_grad=False
        )
        link_lin_vel = torch.empty(
            (n, n_key_links, 3), dtype=torch.float, requires_grad=False
        )
        link_ang_vel = torch.empty(
            (n, n_key_links, 3), dtype=torch.float, requires_grad=False
        )
        ...
        frac = np.minimum((motion_times - fid0 * dt) / dt, 1.0)
        ...
        joint_pos = (quat2expmap(joint_q) + joint_p).view(joint_q.size(0), -1)[
            :, self.dofs
        ]
...
class ICCGANHumanoid(Env):
    ...
    CHARACTER_MODEL = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "assets", "humanoid.xml"
    )
    CONTROLLABLE_LINKS = [
        "torso",
        "head",
    ...
    def __init__(
        ...
        **kwargs
        ...
        contactable_links = parse_kwarg(
            kwargs, "contactable_links", self.CONTACTABLE_LINKS
        )
        controllable_links = parse_kwarg(
            kwargs, "controllable_links", self.CONTROLLABLE_LINKS
        )
        dofs = parse_kwarg(kwargs, "dofs", self.DOFS)
        goal_reward_weight = parse_kwarg(
            kwargs, "goal_reward_weight", self.GOAL_REWARD_WEIGHT
        )
        self.enable_goal_timer = parse_kwarg(
            kwargs, "enable_goal_timer", self.ENABLE_GOAL_TIMER
        )
        self.goal_tensor_dim = parse_kwarg(
            kwargs, "goal_tensor_dim", self.GOAL_TENSOR_DIM
        )
        ...
        controllable_links = [
            self.gym.find_actor_rigid_body_handle(self.envs[0], self.actors[0], link)
        ...
        if contactable_links is None:
            ...
            for link in contactable_links:
                lid = self.gym.find_actor_rigid_body_handle(
                    self.envs[0], self.actors[0], link
                )
                assert lid >= 0, "Unrecognized contactable link {}".format(link)
                contact[:, lid] = True
            self.contactable_links = torch.tensor(contact).to(
                self.contact_force_tensor.device
            )
        else:
        ...
        self.ref_motion = ReferenceMotion(
            motion_file=init_pose,
            character_model=self.character_model,
            key_links=np.arange(n_links),
        ...
        for id, config in discriminators.items():
            ...
            if key not in disc_ref_motion:
                disc_ref_motion[key] = [0, []]
        ...
        if goal_reward_weight is not None:
            reward_weights = torch.empty(
                (len(self.envs), self.rew_dim), dtype=torch.float32, device=self.device
            )
        ...
        if n_comp > 1:
            self.reward_weights = torch.zeros(
                (n_envs, n_comp), dtype=torch.float32, device=self.device
            )
            weights = [
                disc.weight
                for _, disc in discriminators.items()
                if disc.weight is not None
            ]
            total_weights = sum(weights) if weights else 0
            assert (
                total_weights <= 1
            ), "Discriminator weights must not be greater than 1."
        ...
        for i, (id, config) in enumerate(discriminators.items()):
            ...
            parent_link = (
                ...
                else self.gym.find_actor_rigid_body_handle(
                    self.envs[0], self.actors[0], config.parent_link
                )
            )
        ...
        if max_ob_horizon != self.state_hist.size(0):
            self.state_hist = torch.empty(
                (max_ob_horizon, *self.state_hist.shape[1:]),
                dtype=self.root_tensor.dtype,
                device=self.device,
            )
        self.disc_ref_motion = [
            (
                ...
                replay_speed,
                max_ob_horizon,
                [self.discriminators[id] for id in disc_ids],
            )
            for (ref_motion, replay_speed), (
                max_ob_horizon,
                disc_ids,
            ) in disc_ref_motion.items()
        ]
        ...
        self.disc_dim = {
            name: ob.size(-1) for name, ob in self.info["disc_obs"].items()
        }
    ...
    def termination_check(self):
        ...
        contacted = torch.any(masked_contact > 1.0, dim=-1)  # N x n_links
    ...
    def init_state(self, env_ids):
        ...
        ref_root_tensor, ref_link_tensor, ref_joint_tensor = self.ref_motion.state(
    ...
    def create_tensors(self):
        ...
        self.root_pos, self.root_orient = (
            self.root_tensor[:, 0, :3],
            self.root_tensor[:, 0, 3:7],
        )
        self.root_lin_vel, self.root_ang_vel = (
            self.root_tensor[:, 0, 7:10],
            self.root_tensor[:, 0, 10:13],
        )
        self.char_root_tensor = self.root_tensor[:, 0]
        if self.link_tensor.size(1) > n_links:
            self.link_pos, self.link_orient = (
                self.link_tensor[:, :n_links, :3],
                self.link_tensor[:, :n_links, 3:7],
            )
            self.link_lin_vel, self.link_ang_vel = (
                self.link_tensor[:, :n_links, 7:10],
                self.link_tensor[:, :n_links, 10:13],
            )
            self.char_link_tensor = self.link_tensor[:, :n_links]
        else:
            self.link_pos, self.link_orient = (
                self.link_tensor[..., :3],
                self.link_tensor[..., 3:7],
            )
            self.link_lin_vel, self.link_ang_vel = (
                self.link_tensor[..., 7:10],
                self.link_tensor[..., 10:13],
            )
            self.char_link_tensor = self.link_tensor
        if self.joint_tensor.size(1) > n_dofs:
            self.joint_pos, self.joint_vel = (
                self.joint_tensor[:, :n_dofs, 0],
                self.joint_tensor[:, :n_dofs, 1],
            )
        ...
        self.state_hist = torch.empty(
            (self.ob_horizon + 1, len(self.envs), 13 + n_links * 13),
            dtype=self.root_tensor.dtype,
            device=self.device,
        )
        ...
        if self.goal_tensor_dim:
            try:
                ...
                self.goal_tensor = torch.zeros(
                    (len(self.envs), self.goal_tensor_dim),
                    dtype=self.root_tensor.dtype,
                    device=self.device,
                )
        else:
        ...
        self.goal_timer = (
            torch.zeros((len(self.envs),), dtype=torch.int32, device=self.device)
    ...
    def observe(self, env_ids=None):
        self.ob_seq_lens = (
            self.lifetime + 1
        )  # (self.lifetime+1).clip(max=self.state_hist.size(0)-1)
        n_envs = len(self.envs)
        if env_ids is None or len(env_ids) == n_envs:
            self.state_hist[-1] = torch.cat(
                (self.char_root_tensor, self.char_link_tensor.view(n_envs, -1)), -1
            )
            env_ids = None
        else:
            n_envs = len(env_ids)
            self.state_hist[-1, env_ids] = torch.cat(
                (
                    self.char_root_tensor[env_ids],
                    self.char_link_tensor[env_ids].view(n_envs, -1),
                ),
                -1,
            )
    ...
    def _observe(self, env_ids):
        if env_ids is None:
            ...
            return observe_iccgan(
                self.state_hist[-self.ob_horizon :],
                self.ob_seq_lens,
                self.key_links,
                self.parent_link,
            ...
            ground_height = self.ground_height(
                self.state_hist[-1, env_ids, :3], env_ids
            )
            return observe_iccgan(
                self.state_hist[-self.ob_horizon :][:, env_ids],
                self.ob_seq_lens[env_ids],
                self.key_links,
                self.parent_link,
                ground_height=ground_height,
            ).flatten(start_dim=1)
    ...
    def observe_disc(self, state):
        ...
        if torch.is_tensor(state):
            ...
            for id, disc in self.discriminators.items():
                res[id] = observe_iccgan(
                    state[-disc.ob_horizon :],
                    seq_len,
                    disc.key_links,
                    disc.parent_link,
    ...
    def fetch_real_samples(self):
        ...
        for ref_motion, replay_speed, ob_horizon, discs in self.disc_ref_motion:
            ...
            motion_ids, motion_times0 = ref_motion.sample(
                n_inst, truncate_time=dt * (ob_horizon - 1)
            )
            motion_ids = np.tile(motion_ids, ob_horizon)
            motion_times = np.concatenate(
                (motion_times0, *[motion_times0 + dt * i for i in range(1, ob_horizon)])
            )
            root_tensor, link_tensor, joint_tensor = ref_motion.state(
                motion_ids, motion_times
            )
            real = torch.cat(
                (root_tensor, link_tensor.view(root_tensor.size(0), -1)), -1
            ).view(ob_horizon, n_inst, -1)
...
class ICCGANHumanoidTarget(ICCGANHumanoid):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.sharp_turn_rate = parse_kwarg(
            kwargs, "sharp_turn_rate", self.SHARP_TURN_RATE
        )
        ...
        self.goal_timer_range = parse_kwarg(
            kwargs, "goal_timer_range", self.GOAL_TIMER_RANGE
        )
    ...
    def update_viewer(self):
        ...
        for e, l in zip(self.envs, lines):
            self.gym.add_lines(
                self.viewer, e, n_lines, l, [[1.0, 0.0, 0.0] for _ in range(n_lines)]
            )
        n_lines = 10
            ...
            [
                np.stack(
                    (
                        ...
                        zero,
                        target_pos[:, 0]
                        + self.goal_radius * np.cos(2 * np.pi / n_lines * i),
                        target_pos[:, 1]
                        + self.goal_radius * np.sin(2 * np.pi / n_lines * i),
                        zero,
                    ),
                    -1,
                )
        ...
        for e, l in zip(self.envs, lines):
            self.gym.add_lines(
                self.viewer, e, n_lines, l, [[0.0, 0.0, 1.0] for _ in range(n_lines)]
            )
    ...
    def _observe(self, env_ids):
        if env_ids is None:
            ...
            return observe_iccgan_target(
                ...
                self.goal_tensor,
                self.goal_timer,
                sp_upper_bound=self.sp_upper_bound,
                fps=self.fps,
                ground_height=ground_height,
            )
        else:
    ...
    def reset_goal(self, env_ids, goal_tensor=None, goal_timer=None):
        if goal_tensor is None:
            goal_tensor = self.goal_tensor
        if goal_timer is None:
        ...
        large_angle = torch.rand(n_envs, dtype=torch.float32, device=self.device).mul_(
            2 * np.pi
        )
        small_angle = (
            torch.rand(n_envs, dtype=torch.float32, device=self.device)
            .sub_(0.5)
            .mul_(2 * (np.pi / 3))
        )
        ...
        timer = torch.randint(
            self.goal_timer_range[0],
            self.goal_timer_range[1],
            (n_envs,),
            dtype=self.goal_timer.dtype,
            device=self.device,
        )
        if self.goal_sp_min == self.goal_sp_max:
            ...
            vel = torch.nn.init.trunc_normal_(
                torch.empty(n_envs, dtype=torch.float32, device=self.device),
                mean=self.goal_sp_mean,
                std=self.goal_sp_std,
                a=self.goal_sp_min,
                b=self.goal_sp_max,
            )
    ...
    def reward(self, goal_tensor=None, goal_timer=None):
        ...
        sp_ = (dist_ / self.step_time).clip_(
            max=v_mag.clip(min=self.sp_lower_bound, max=self.sp_upper_bound)
        )
...
def observe_iccgan(
    ...
    key_links: Optional[List[int]] = None,
    ...
    if key_links is None:
        ...
        link_pos, link_orient = (
            link_tensor[:, :, key_links, :3],
            link_tensor[:, :, key_links, 3:7],
        )
    ...
    if include_velocity:
        if key_links is None:
            ...
            link_lin_vel, link_ang_vel = (
                link_tensor[:, :, key_links, 7:10],
                link_tensor[:, :, key_links, 10:13],
            )
        ...
        ob = torch.cat(
            (ob_link_pos, ob_link_orient, ob_link_lin_vel, ob_link_ang_vel), -1
        )  # L x N x n_links x 13
    else:
    ...
    arange = torch.arange(
        n_hist, dtype=seq_len.dtype, device=seq_len.device
    ).unsqueeze_(0)
...
class ICCGANHumanoidTargetAiming(ICCGANHumanoidTarget):
    ...
    def create_tensors(self):
        super().create_tensors()
        self.hand_link = self.gym.find_actor_rigid_body_handle(
            self.envs[0], self.actors[0], "right_hand"
        )
        self.lower_arm_link = self.gym.find_actor_rigid_body_handle(
            self.envs[0], self.actors[0], "right_lower_arm"
        )
    ...
    def update_viewer(self):
        ...
        q = torch.where(target_dir[:, :1] < -0.99999, self.reverse_rotation, q)
        ...
        for i, l in zip(not_near, lines):
            e = self.envs[i]
            self.gym.add_lines(
                self.viewer, e, n_lines, l, [[0.0, 1.0, 0.0] for _ in range(n_lines)]
            )
    ...
    def reset_aiming_goal(self, env_ids):
        n_envs = len(env_ids)
        elev = torch.rand(n_envs, dtype=torch.float32, device=self.device).mul_(
            -np.pi / 6
        )
        azim = torch.rand(n_envs, dtype=torch.float32, device=self.device).mul_(
            np.pi / 4
        )
        if self.viewer is not None:
            azim.add_(0.3)
    ...
    def reward(self):
        ...
        q = torch.where(target_dir[:, :1] < -0.99999, self.reverse_rotation, q0)
        ...
        e = torch.linalg.norm(target_hand_pos.sub_(hand_pos), ord=2, dim=-1).div_(
            arm_len.squeeze_(-1)
        )
...
class ICCGANHumanoidJugglingTarget(ICCGANHumanoidTarget):
    ...
    def create_envs(self, n: int):
        ...
        ball_asset_options.density = self.ball_mass / (
            4 * np.pi * (self.ball_radius**3) / 3
        )
        ball_asset = self.gym.create_sphere(
            self.sim, self.ball_radius, ball_asset_options
        )
    ...
    def add_actor(self, env, i):
        ...
        self.right_hand_link = self.gym.find_actor_rigid_body_handle(
            env, 0, "right_hand"
        )
        left_lower_arm_link = self.gym.find_actor_rigid_body_handle(
            env, 0, "left_lower_arm"
        )
        right_lower_arm_link = self.gym.find_actor_rigid_body_handle(
            env, 0, "right_lower_arm"
        )
        ...
        for k in range(1, self.n_balls + 1):
            self.gym.create_actor(
                env, self.ball_asset, self.ball_init_pose, "ball", i, -1, 0
            )
            r, g, b = (k // 4) % 2, (k // 2) % 2, k % 2
            self.gym.set_rigid_body_color(
                env, k, 0, gymapi.MESH_VISUAL_AND_COLLISION, gymapi.Vec3(r, g, b)
            )
    ...
    def create_tensors(self):
        ...
        self.env_range = torch.arange(
            len(self.envs), dtype=torch.int64, device=self.ball_root_tensor.device
        )
        self.env_range_ = self.env_range.unsqueeze(-1)
        self.ball_index = (
            torch.arange(
                self.ball_root_tensor.size(1),
                dtype=torch.int64,
                device=self.ball_root_tensor.device,
            )
            .add_(self.link_tensor.size(1) - self.ball_root_tensor.size(1))
            .unsqueeze_(0)
            .repeat(len(self.envs), 1)
        )
        self.ball_attach = torch.zeros(
            (len(self.envs), self.ball_root_tensor.size(1)),
            dtype=torch.int64,
            device=self.ball_root_tensor.device,
        )

        self.target_ball_release = torch.zeros(
            (len(self.envs),), dtype=torch.int64, device=self.ball_root_tensor.device
        )
        ...
        self.catachable_right = torch.zeros_like(
            self.dist2target_right, dtype=torch.bool
        )
        ...
        self.v_out_target = (
            0.5
            * 9.81
            * ((self.throw_interval // 2) * n_balls - self.dwell_time)
            * self.step_time
        )
    ...
    def init_state(self, env_ids):
        ...
        for i in range(n_balls):
            self.ball_attach[env_ids, i] = (
                self.right_hand_link if i % 2 else self.left_hand_link
            )
        ball_root_tensor = ref_link_tensor[
            (self.env_range_[: len(env_ids)], self.ball_attach[env_ids])
        ]
        ...
        ref_root_tensor = torch.cat(
            (ref_root_tensor.unsqueeze_(1), ball_root_tensor), 1
        )
    ...
    def refresh_tensors(self):
        ...
        self.ball_attach[target_ball] = torch.where(
            release, self.ball_index[target_ball], self.ball_attach[target_ball]
        )

        self.timer_left[throw_left] = self.throw_interval
        self.miss_left[throw_left] = False
        self.target_ball_left[throw_left] = target_ball_release[throw_left] + (
            2 - n_balls
        )
        self.timer_right[throw_right] = self.throw_interval
        self.miss_right[throw_right] = False
        self.target_ball_right[throw_right] = target_ball_release[throw_right] + (
            2 - n_balls
        )
        ...
        self.ball_h_out[target_ball] = torch.where(
            release,
            self.ball_root_tensor[:, :, self.UP_AXIS][target_ball],
            self.ball_h_out[target_ball],
        )
        ...
        dist = torch.where(t < 0, dist, torch.linalg.norm(dp, ord=2, dim=-1))
        self.dist2target_left = torch.where(
            catched_already, self.dist2target_left, dist
        )
        ...
        delta = v.square().sub_(19.62 * h)
        t = torch.where(delta > 0, delta.sqrt().add_(v).div_(9.81), v / 9.81)
        self.catachable_right = torch.any(
            torch.stack(
                (
                    catched_already,
                    dist < self.ball_radius * catch_range,
        ...
        dp[:, self.UP_AXIS] += 4.905 * t.square() - ball_vel[:, self.UP_AXIS] * t
        dist = torch.where(t < 0, dist, torch.linalg.norm(dp, ord=2, dim=-1))
        self.dist2target_right = torch.where(
            catched_already, self.dist2target_right, dist
        )

        self.ball_root_tensor.copy_(
            self.link_tensor[(self.env_range_, self.ball_attach)]
        )
    ...
    def reward(self):
        ...
        catched = (
            self.ball_attach[(self.env_range, self.target_ball_left)]
            == self.left_hand_link
        )
        dist2 = self.dist2target_left.square()
        rew_left_catch = 0.9 * dist2.mul(-20).exp_() + 0.2 * dist2.mul(-1).exp_()
        dh = (
            self.ball_h_out[(self.env_range, self.target_ball_left)]
            - self.link_tensor[:, self.left_hand_link, self.UP_AXIS]
        )
        dh[catched] = 0
        rew_left_catch = (
            dh.square_().mul_(-20).exp_() * 0.1
            + 0.9 * (~self.miss_left) * rew_left_catch
        )

        rew_left_throw = (
            (self.v_out_left / self.v_out_target - 1).square_().mul_(-5).exp_()
        )
        rew_left = torch.where(
            self.timer_left == self.throw_interval,
            rew_left_throw,
            started_left * rew_left_catch,
        )

        catched = (
            self.ball_attach[(self.env_range, self.target_ball_right)]
            == self.right_hand_link
        )
        dist2 = self.dist2target_right.square()
        rew_right_catch = 0.9 * dist2.mul(-20).exp_() + 0.2 * dist2.mul(-1).exp_()
        dh = (
            self.ball_h_out[(self.env_range, self.target_ball_right)]
            - self.link_tensor[:, self.right_hand_link, self.UP_AXIS]
        )
        dh[catched] = 0
        rew_right_catch = (
            dh.square_().mul_(-20).exp_() * 0.1
            + 0.9 * (~self.miss_right) * rew_right_catch
        )

        rew_right_throw = (
            (self.v_out_right / self.v_out_target - 1).square_().mul_(-5).exp_()
        )
        rew_right = torch.where(
            self.timer_right == self.throw_interval,
            rew_right_throw,
            started_right * rew_right_catch,
        )
    ...
    def _observe(self, env_ids):
        ...
        if env_ids is None:
            ...
            res = observe_iccgan_juggling_target(
                ...
                self.sp_upper_bound,
                self.fps,
                target_ball_left[env_ids],
                catched_left[env_ids],
                target_ball_right[env_ids],
                catched_right[env_ids],
...
@torch.jit.script
def observe_iccgan_juggling_target(
    state_hist: torch.Tensor,
    seq_len: torch.Tensor,
    goal_tensor: torch.Tensor,
    goal_timer: torch.Tensor,
    sp_upper_bound: float,
    fps: int,
    target_ball_left: torch.Tensor,
    catched_left: torch.Tensor,
    target_ball_right: torch.Tensor,
    catched_right: torch.Tensor,
    target_ball_next: torch.Tensor,
    timer: torch.Tensor,
):
    UP_AXIS = 2

    ob = observe_iccgan_target(
        state_hist,
        seq_len,
        goal_tensor,
        goal_timer,
    ...
    return torch.cat(
        (ob, ball_state_l, ball_state_r, ball_state_n, timer.unsqueeze_(-1)), -1
    )
...
class ICCGANHumanoidLowFriction(ICCGANHumanoidTarget):
    ...
    def __init__(self, *args, **kwargs):
        self.ground_friction = parse_kwarg(kwargs, "ground_friction", 1.0)
        super().__init__(*args, **kwargs)
    ...
    def add_actor(self, env, i):
        if self.ground_friction != 1:
            ...
            rb_shape_props[rb_shape[left_foot].start].rolling_friction = (
                self.ground_friction
            )
            rb_shape_props[rb_shape[left_foot].start].torsion_friction = (
                self.ground_friction
            )
            rb_shape_props[rb_shape[right_foot].start].friction = self.ground_friction
            rb_shape_props[rb_shape[right_foot].start].rolling_friction = (
                self.ground_friction
            )
            rb_shape_props[rb_shape[right_foot].start].torsion_friction = (
                self.ground_friction
            )
            self.gym.set_actor_rigid_shape_properties(env, 0, rb_shape_props)
...
class ICCGANHumanoidTerrain(ICCGANHumanoidTarget):
    ...
    def add_ground(self):
        ...
        field_x = (
            sub_terrain_size_x * int(np.ceil(n_envs / n_envs_per_row))
            + border * 2 * sub_terrain_size_x
        )

        grids_y = int(field_y / sub_terrain_size_y)
        grids_x = int(field_x / sub_terrain_size_x)

        height_map_raw = np.zeros(
            (grids_x * sub_grids_x, grids_y * sub_grids_y), dtype=np.int16
        )
        ...
        vertices, triangles = convert_heightfield_to_trimesh(
            heightfield_raw, self.horizontal_scale, vertical_scale, slope_threshold
        )
        ...
        self.height_map = torch.tensor(
            heightfield_raw * vertical_scale - tm_params.transform.p.z,
            dtype=torch.float32,
        ).to(self.device)
        ...
        map_offset = torch.tensor(
            [tm_params.transform.p.x, tm_params.transform.p.y]
        ).to(self.device)
        self.env_origins = env_origins.sub_(map_offset)
        self.terrain_border = 1, torch.tensor(
            [
    ...
    def observe_ground_height(self, env_ids=None):
        if env_ids is None:
            height_map = observe_height_map(
                self.char_root_tensor,
