...
parser.add_argument(
    "--amass_data", type=str, default="/home/lithiumice/PHC/sample_data/wudang2.pkl"
)
parser.add_argument("--out_dir", type=str, default="data/motion_lib/tennis_djokovic")
...
args = attrdict.AttrDict(
    amass_data="/home/lithiumice/PHC/sample_data/wudang2.pkl",
    out_dir="data/motion_lib/tennis_djokovic",
    num_seq=None,
    num_motion_libs=1,
)
...
_, index = np.unique(
    [",".join([f"{x:.6f}" for x in beta]) for beta in all_beta], return_index=True
)
index.sort()
...
joblib.dump(
    {"beta_arr": beta_arr, "beta_mapping": beta_mapping},
    f"{args.out_dir}/shape_data.pkl",
)
...
for i, seq_arr in enumerate(motion_lib_seq_arr):
    seq_name_splits[i] = [seq_name.item()[:] for seq_name in seq_arr]
