...
class TennisBallInEstimator:
    ...
    def get_ball_traj_index(self, height, vel_x, vel_y, vspin):
        ...
        index = (
            torch.round((height - HEIGHT_RANGE[0]) / HEIGHT_RANGE[2])
            * dim[1]
            * dim[2]
            * dim[3]
        ...
        height = (
            torch.round((height - HEIGHT_RANGE[0]) / HEIGHT_RANGE[2]) * HEIGHT_RANGE[2]
            + HEIGHT_RANGE[0]
        )
        vel_x = (
            torch.round((vel_x - VEL_X_RANGE[0]) / VEL_X_RANGE[2]) * VEL_X_RANGE[2]
            + VEL_X_RANGE[0]
        )
        vel_y = (
            torch.round((vel_y - VEL_Y_RANGE[0]) / VEL_Y_RANGE[2]) * VEL_Y_RANGE[2]
            + VEL_Y_RANGE[0]
        )
        vspin = (
            torch.round((vspin - VSPIN_RANGE[0]) / VSPIN_RANGE[2]) * VSPIN_RANGE[2]
            + VSPIN_RANGE[0]
        )
    ...
    def estimate(self, ball_states, adjust=False):
        ...
        traj_index, (height, vel_x, vel_y, vspin) = self.get_ball_traj_index(
            height, vel_x, vel_y, vspin
        )

        traj = self._ball_traj[traj_index].clone().to(device)
        # transform traj
        traj_trans = torch.cat(
            [
                traj[:, :, :1] * dir.view(-1, 1, 2) + ball_states[:, :2].view(-1, 1, 2),
                traj[:, :, 1:],
            ],
            dim=-1,
        )
        ...
        ball_states_out[:, 10:13] = (
            vspin.view(-1, 1)
            * pi
            * 2
            * F.normalize(
                torch.cross(
                    ball_states_out[:, 7:10],
                    torch.FloatTensor([0, 0, -1])
                    .repeat(ball_states.shape[0], 1)
                    .to(device),
                ),
...
def generate_incoming_trajectory(traj_path, params):
    ...
    device = (
        torch.device("cuda:0")
        if ball_gen.args.use_gpu_pipeline
    ...
    batch_size = (
        len(height_range) * len(vel_z_range) * len(vel_y_range) * len(vspin_range)
    )
...
if __name__ == "__main__":
    ...
    generate_incoming_trajectory(
        "data/ball_traj/ball_traj_in_dual.npy", params=traj_in_params
    )
