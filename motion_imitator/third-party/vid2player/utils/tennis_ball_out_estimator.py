...
def simulate_without_bounce(
    ...
    traj_x = torch.zeros(
        (num_ball, int((TRAJ_X_RANGE[1] - TRAJ_X_RANGE[0]) / TRAJ_X_RANGE[2])),
        dtype=torch.float32,
        device=device,
    )
    traj_y = torch.zeros(
        (num_ball, int((TRAJ_Y_RANGE[1] - TRAJ_Y_RANGE[0]) / TRAJ_Y_RANGE[2]), 2),
        dtype=torch.float32,
        device=device,
    )
    ...
    for y in torch.arange(*TRAJ_Y_RANGE):
        while True:
            ...
            if ids.sum() > 0:
                ...
                break
        traj_y[:, int(y * 10), 0], traj_y[:, int(y * 10), 1] = interpolate_y_batch(
            traj_all, t, -y
        )
...
class TennisBallOutEstimator:
    ...
    def get_ball_traj_out_index(self, vel_x, vel_y, vspin):
        VEL_X_RANGE, VEL_Y_RANGE, VSPIN_RANGE = (
            self.params.VEL_X_RANGE,
            self.params.VEL_Y_RANGE,
            self.params.VSPIN_RANGE,
        )
        ...
        index = (
            torch.round((vel_x - VEL_X_RANGE[0]) / VEL_X_RANGE[2]) * dim[1] * dim[2]
    ...
    def estimate(self, ball_states_all):
        VEL_X_RANGE, VEL_Y_RANGE, TRAJ_Y_RANGE = (
            self.params.VEL_X_RANGE,
            self.params.VEL_Y_RANGE,
            self.params.TRAJ_Y_RANGE,
        )

        device = ball_states_all.get_device()
        has_valid_contact = (
            (ball_states_all[:, 8] > VEL_X_RANGE[0])
            & (ball_states_all[:, 9] > VEL_Y_RANGE[0])
            & (ball_states_all[:, 9] < VEL_Y_RANGE[1])
            & (ball_states_all[:, 2] < TRAJ_Y_RANGE[1])
        )
        # inside when passing the net
        x_net = ball_states_all[:, 0] + ball_states_all[:, 7] * abs(
            ball_states_all[:, 1] / ball_states_all[:, 8]
        )
        ...
        if num_need_update > 0:
            ...
            if (vel_x >= VEL_X_RANGE[1]).sum() > 0:
                print("velocity X overflow")
            vel_y = ball_states[:, 9]
            ...
            bounce_pos = ball_states[:, :2] + ball_traj_y[
                torch.arange(num_need_update), height_index, :1
            ].to(device) * ball_states[:, 7:9] / vel_x.unsqueeze(-1)
            bounce_time = ball_traj_y[
                torch.arange(num_need_update), height_index, 1
            ].to(device)
            ...
            not_pass_net = (
                ball_traj_x[torch.arange(num_need_update), net_index]
                + ball_states[:, 2].cpu()
                < NET_HEIGHT
            )
...
def generate_outgoing_trajectory(traj_path, params):
    ...
    device = (
        torch.device("cuda:0")
        if isaac_ball_gen.args.use_gpu_pipeline
...
if __name__ == "__main__":
    ...
    generate_outgoing_trajectory(
        "data/ball_traj/ball_traj_out_x.npy", params=traj_out_params
    )
