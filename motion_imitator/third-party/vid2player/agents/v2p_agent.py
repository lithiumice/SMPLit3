...
class V2PAgent(common_agent.CommonAgent):
    ...
    def set_network_weights(self, weights):
        ...
        if network_obs_dim != weight_obs_dim:
            ...
            self.model.a2c_network.actor_mlp[0].weight.data[:, :weight_obs_dim] = (
                weights["a2c_network.actor_mlp.0.weight"]
            )
            del weights["a2c_network.actor_mlp.0.weight"]

            self.model.a2c_network.critic_mlp[0].weight.data.zero_()
            self.model.a2c_network.critic_mlp[0].weight.data[:, :weight_obs_dim] = (
                weights["a2c_network.critic_mlp.0.weight"]
            )
            del weights["a2c_network.critic_mlp.0.weight"]

        if network_action_dim != weight_action_dim:
            print(
                f"Warning: network action dim = {network_action_dim} but weight action dim = {weight_action_dim}"
            )
            assert network_action_dim > weight_action_dim
            self.model.a2c_network.mu.weight.data.zero_()
            self.model.a2c_network.mu.weight.data[:weight_action_dim, :] = weights[
                "a2c_network.mu.weight"
            ]
            self.model.a2c_network.mu.bias.data.zero_()
            self.model.a2c_network.mu.bias.data[:weight_action_dim] = weights[
                "a2c_network.mu.bias"
            ]
    ...
    def set_stats_weights(self, weights):
        if self.normalize_input:
            ...
            if network_obs_dim != weight_obs_dim:
                ...
                self.running_mean_std.running_mean[:weight_obs_dim] = weights[
                    "running_mean_std"
                ]["running_mean"]
                self.running_mean_std.running_var[:weight_obs_dim] = weights[
                    "running_mean_std"
                ]["running_var"]
            else:
    ...
    def train(self):
        ...
        if pretrained_model_cp is not None and not self.config.get(
        ...
        while True:
            ...
            if self.rank == 0:
                ...
                if self.print_stats:
                    ...
                    print(
                        "{}\tT_play {:.2f}\tT_update {:.2f}\tETA {}\tstep_rewards {:.4f} {}\teps_len {:.2f}\tfps step {}\tfps total {}\t{}".format(
                            epoch_num,
                            play_time,
                            update_time,
                ...
                best_model_file = os.path.join(
                    self.network_path, f"{self.config['name']}_best"
                )
                ...
                if (
                    self.game_rewards.current_size > 0
                    and mean_rewards[0] > self.best_mean_rewards + 1
                ):
