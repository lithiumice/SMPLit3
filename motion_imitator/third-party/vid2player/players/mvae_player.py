...
class MVAEPlayer:
    def __init__(self, cfg, num_envs, is_train, enable_physics, device):
        ...
        self._root_pos = torch.zeros(
            (self.num_envs, 3), device=self.device, dtype=torch.float32
        )
        self._root_vel = torch.zeros(
            (self.num_envs, 3), device=self.device, dtype=torch.float32
        )
        self._joint_rot6d = torch.zeros(
            (self.num_envs, 24, 6), device=self.device, dtype=torch.float32
        )
        self._joint_rotmat = torch.zeros(
            (self.num_envs, 24, 3, 3), device=self.device, dtype=torch.float32
        )
        ...
        self._joint_rot = torch.zeros(
            (self.num_envs, 24, 3), device=self.device, dtype=torch.float32
        )
        if not self._is_train:
            self._joint_rot_ori = torch.zeros(
                (self.num_envs, 24, 3), device=self.device, dtype=torch.float32
            )
        ...
        if not self._is_dual:
            ...
            self._racket = Racket(
                "tennis",
                self.device,
                grip=self.cfg.get("grip")[server],
                righthand=self.cfg.get("righthand")[server],
            )
        if self._predict_phase:
            self._phase_pred = torch.zeros(
                self.num_envs, device=self.device, dtype=torch.float32
            )
            self._swing_type = (
                torch.zeros(self.num_envs, device=self.device, dtype=torch.int64) - 1
            )
        ...
        self._conditions = torch.zeros(
            (self.num_envs, opt.num_condition_frames, opt.frame_size),
            device=self.device,
            dtype=torch.float32,
        )
        ...
        if opt.condition_root_x_only:
            dim = 1
        if opt.no_condition_root_y:
            dim = 2
        if "root_pos" not in opt.pose_feature:
            dim = 0
        if dim > 0:
            self._root_pos_inds = torch.LongTensor([i for i in range(dim)]).to(
                self.device
            )
        self._root_vel_inds = torch.LongTensor([i for i in range(dim, dim + 3)]).to(
            self.device
        )
        self._joint_pos_inds = torch.LongTensor(
            [i for i in range(dim + 3, dim + 3 + 23 * 3)]
        ).to(self.device)
        dim += 3
        ...
        if "joint_velo" in opt.pose_feature:
            dim += 23 * 3
        self._joint_rot6d_inds = torch.LongTensor(
            [i for i in range(dim, dim + 24 * 6)]
        ).to(self.device)
                ...
                for i, rh in enumerate(self.cfg["righthand"]):
                    if rh:
                        self._residual_joint_inds[i] = torch.LongTensor(
                            [SMPLPose.RElbow, SMPLPose.RWrist]
                        ).to(self.device)
                    else:
                        self._residual_joint_inds[i] = torch.LongTensor(
                            [SMPLPose.LElbow, SMPLPose.LWrist]
                        ).to(self.device)
            else:
    ...
    def init_states(self):
        if self.cfg.get("dual_mode") == "different":
            init_data_fg = (
                torch.load(self.cfg["vae_init_conditions_serve"][0])[:1]
                .to(self.device)
                .float()
            )
            init_data_bg = (
                torch.load(self.cfg["vae_init_conditions_serve"][1])[:1]
                .to(self.device)
                .float()
            )
            self._init_data_serve = torch.cat(
                [init_data_fg, init_data_bg], dim=0
            ).repeat(self.num_envs // 2, 1, 1)

            init_data_fg = (
                torch.load(self.cfg["vae_init_conditions_ready"][0])[:1]
                .to(self.device)
                .float()
            )
            init_data_bg = (
                torch.load(self.cfg["vae_init_conditions_ready"][1])[:1]
                .to(self.device)
                .float()
            )
            self._init_data_ready = torch.cat(
                [init_data_fg, init_data_bg], dim=0
            ).repeat(self.num_envs // 2, 1, 1)
            return
        elif self.cfg.get("dual_mode") == True:
            init_data_serve = (
                torch.load(self.cfg["vae_init_conditions_serve"])[:1]
                .to(self.device)
                .float()
            )
            ...
            init_data_ready = (
                torch.load(self.cfg["vae_init_conditions_ready"])[:1]
                .to(self.device)
                .float()
            )
        ...
        if self.cfg.get("vae_init_conditions") is not None:
            ...
            self._init_data = (
                torch.load(self.cfg["vae_init_conditions"]).to(self.device).float()
            )
    ...
    def reset(self, env_ids):
        ...
        self._update_mvae_state(
            self._init_data[env_ids, -1].clone(), phase=None, env_ids=env_ids, init=True
        )

    def reset_dual(self, reset_reaction_env_ids, reset_recovery_env_ids):
        env_ids = (
            torch.cat([reset_reaction_env_ids, reset_recovery_env_ids]).sort().values
        )
        self._swing_type[env_ids] = -1
        self._swing_type_cycle[env_ids] = -1

        self._conditions[reset_reaction_env_ids] = self._init_data_ready[
            reset_reaction_env_ids
        ].clone()
        self._conditions[reset_recovery_env_ids] = self._init_data_serve[
    ...
    def step(self, latents, residual=None):
        ...
        with torch.no_grad():
            if not self._is_dual:
                ...
                feature2, phase2 = self._mvae2.forward(
                    latents[1::2], condition_flat[1::2]
                )
                self._update_mvae_state(
                    feature1,
                    phase1,
                    residual,
                    env_ids=torch.arange(0, self.num_envs, 2),
                    mvae=self._mvae1,
                    player=self.cfg["player"][0],
                    residual_joint_inds=self._residual_joint_inds[0],
                )
                self._update_mvae_state(
                    feature2,
                    phase2,
                    residual,
                    env_ids=torch.arange(1, self.num_envs, 2),
                    mvae=self._mvae2,
                    player=self.cfg["player"][1],
                    residual_joint_inds=self._residual_joint_inds[1],
                )

    def _update_mvae_state(
        self,
        feature,
        phase=None,
        residual=None,
        ...
        if init:
            ...
            if "root_pos" not in mvae.opt.pose_feature:
                feature = feature[:, 3:]
            self._conditions[env_ids] = mvae.normalize(feature).view(
                len(env_ids), 1, -1
            )
        else:
        ...
        if init:
            ...
            if (
                self._is_train
                or self.cfg.get("test_mode", "random") == "random"
                or self.cfg.get("dual_mode") is not None
            ):
                # start player near court baseline center (default -1m to 1m)
                new_root_xy = (
                    torch.rand((len(env_ids), 2), device=self.device) - 0.5
                ) * torch.FloatTensor([2, 1.5]).to(self.device) + torch.FloatTensor(
                    [0, -13]
                ).to(
                    self.device
                )
                ...
                # reset to center
                new_root_xy = (
                    torch.FloatTensor([0, -13]).repeat(len(env_ids), 1).to(self.device)
                )
                ...
                if "root_pos" in mvae.opt.pose_feature:
                    ...
                    if mvae.opt.no_condition_root_y:
                        new_conditions[:, :, self._root_pos_inds] = mvae.normalize(
                            self._root_pos[env_ids][:, [0, 2]], self._root_pos_inds
                        ).view(len(env_ids), 1, 2)
                    elif mvae.opt.condition_root_x_only:
                        new_conditions[:, :, self._root_pos_inds] = mvae.normalize(
                            self._root_pos[env_ids][:, [0]], self._root_pos_inds
                        ).view(len(env_ids), 1, 1)
                    else:
                        new_conditions[:, :, self._root_pos_inds] = mvae.normalize(
                            self._root_pos[env_ids], self._root_pos_inds
                        ).view(len(env_ids), 1, 3)
                    self._conditions[env_ids] = new_conditions
        else:
            self._root_pos[env_ids] = (
                self._root_pos[env_ids] + feature[:, self._root_vel_inds]
            )
            # replace root position(normalized) in the condition
            if "root_pos" in mvae.opt.pose_feature:
                if mvae.opt.no_condition_root_y:
                    self._conditions[:, -1, self._root_pos_inds] = mvae.normalize(
                        self._root_pos[:, [0, 2]], self._root_pos_inds
                    )
                elif mvae.opt.condition_root_x_only:
                    self._conditions[:, -1, self._root_pos_inds] = mvae.normalize(
                        self._root_pos[:, [0]], self._root_pos_inds
                    )
                else:
                    self._conditions[:, -1, self._root_pos_inds] = mvae.normalize(
                        self._root_pos, self._root_pos_inds
                    )
        ...
        if self.cfg.get("vis_mvae") == "origin" and not self._is_train:
            self._joint_rot_ori[env_ids] = rot6d_to_angle_axis(
                self._joint_rot6d[env_ids]
            )
        ...
        if self._predict_phase and phase is not None:
            ...
            self._phase_pred[env_ids] = torch.where(
                self._phase_pred[env_ids] < 0,
                self._phase_pred[env_ids] + math.pi * 2,
                self._phase_pred[env_ids],
            )
            if self._is_dual:
            ...
            self._swing_type[env_ids] = torch.where(
                (self._swing_type[env_ids] == -1)
                & (self._phase_pred[env_ids] > 2.0)
                & (self._phase_pred[env_ids] < 3.5),
                torch.where(
                    joint_pos[:, SMPLPose.RWrist - 1, 0] > 0,
        ...
        if residual is not None and len(residual) != 0:
            ...
            if player == "djokovic":
                ...
                joint_rot = rotation_matrix_to_angle_axis(
                    self._joint_rotmat[:, residual_joint_inds]
                )
                during_fh_swing = (
                    (self._phase_pred > 2.0)
                    & (self._phase_pred < 3.2)
                    & (self._swing_type == 1)
                )
                during_fh_pre_contact = (
                    (self._phase_pred > 2.0)
                    & (self._phase_pred < 3.1)
                    & (self._swing_type == 1)
                )
                during_fh_post_contact = (
                    (self._phase_pred > 2.0)
                    & (self._phase_pred >= 3.1)
                    & (self._phase_pred < 3.2)
                    & (self._swing_type == 1)
                ...
                during_bh_pre_contact = (
                    (self._phase_pred > 2.0)
                    & (self._phase_pred < 3.0)
                    & (self._swing_type == 2)
                )

                eblow_twist_base = torch.zeros(
                    self.num_envs, dtype=torch.float32, device=self.device
                )
                wrist_swing_base = torch.zeros(
                    self.num_envs, dtype=torch.float32, device=self.device
                )
                ...
                wrist_twist_base = torch.zeros(
                    self.num_envs, dtype=torch.float32, device=self.device
                )
                wrist_shake_base = torch.zeros(
                    self.num_envs, dtype=torch.float32, device=self.device
                )
                wrist_swing_base = torch.zeros(
                    self.num_envs, dtype=torch.float32, device=self.device
            ...
            if self._is_dual:
                ...
                joint_rotmat[:, residual_joint_inds] = angle_axis_to_rotation_matrix(
                    joint_rot[env_ids]
                )
                self._joint_rotmat[env_ids] = joint_rotmat
                joint_rot6d = self._joint_rot6d[env_ids].clone()
                joint_rot6d[:, residual_joint_inds] = rotmat_to_rot6d(
                    joint_rotmat[:, residual_joint_inds]
                )
                self._joint_rot6d[env_ids] = joint_rot6d
            else:
                self._joint_rotmat[:, residual_joint_inds] = (
                    angle_axis_to_rotation_matrix(joint_rot)
                )
                self._joint_rot6d[:, residual_joint_inds] = rotmat_to_rot6d(
                    self._joint_rotmat[:, residual_joint_inds]
                )
