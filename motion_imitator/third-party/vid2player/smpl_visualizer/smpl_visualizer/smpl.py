...
class SMPL(_SMPL):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "betas" in kwargs.keys():
            v_shaped = self.v_template + blend_shapes(
                torch.FloatTensor(kwargs["betas"]), self.shapedirs
            )
        else:
    ...
    def forward(
        self, *args, root_trans=None, root_scale=None, orig_joints=False, **kwargs
    ):
        """
        ...
        if root_trans is not None:
            ...
            output.vertices[:] = (output.vertices - cur_root_trans) * root_scale[
                :, None, None
            ] + root_trans[:, None, :]
            output.joints[:] = (output.joints - cur_root_trans) * root_scale[
                :, None, None
            ] + root_trans[:, None, :]
        return output
    ...
    def get_joints(
        ...
        joints, A = batch_rigid_transform(
            rot_mats, J, self.parents, dtype=torch.float32
        )
    ...
    def get_joints_fast(self, pose=None, root_trans=None):
        ...
        ).reshape(-1, rel_joints.shape[1], 4, 4)
