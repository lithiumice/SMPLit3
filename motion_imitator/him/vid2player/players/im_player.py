...
class ImitatorPlayer(common_player.CommonPlayer):
        ...
        for network, cp in zip(networks, dual_model_cp):
            ...
            if load_rlgame_norm:
                if "running_mean_std" in checkpoint:
                    ...
                    network.running_obs.mean[:] = checkpoint["running_mean_std"][
                        "running_mean"
                    ].float()
                    network.running_obs.var[:] = checkpoint["running_mean_std"][
                        "running_var"
                    ].float()
    ...
    def set_weights(self, weights):
        ...
        if self.normalize_input:
            if "running_mean_std" in weights:
                ...
                self.running_mean_std.count = weights["model"][
                    "a2c_network.running_obs.n"
                ].long()
                self.running_mean_std.running_mean[:] = weights["model"][
                    "a2c_network.running_obs.mean"
                ].float()
                self.running_mean_std.running_var[:] = weights["model"][
                    "a2c_network.running_obs.var"
                ].float()
    ...
    def run_one_step(self):
        # get observation
        obs = torch.clamp(self.task.obs_buf, -self.env.clip_obs, self.env.clip_obs).to(
            self.device
        )
        ...
        action = torch.clamp(action, -self.env.clip_actions, self.env.clip_actions).to(
            self.device
        )
