...
class MotionVAERunner(BaseRunner):
    ...
    def step(self):
        next_frame = self.motion_vae.infer_single(
            self.latent, self.condition, self.action
        )
    ...
    def update_state(self, frame):
        ...
        if opt.update_joint_pos:
            ...
            self.joint_pos_history = torch.cat(
                (self.joint_pos_history, self.joint_pos_cur.view(23, 3).unsqueeze(0))
            )
        else:
            self.joint_rot_cur = frame["joint_rot"]
            self.joint_rot_history = torch.cat(
                (self.joint_rot_history, self.joint_rot_cur.view(24, 3).unsqueeze(0))
            )
...
def test_motion_vae_randomwalk(
    opt,
    num_test=5,
    num_runner=5,
    result_dir_suffix="",
    ...
    for tid in range(num_test):
        ...
        for r in range(num_runner):
            if opt.update_joint_pos:
                joint_pos_all[r, :, 1:, :] = runner_dict[
                    r
                ].joint_pos_history  # N x 23 x 3
            else:
                joint_rot_all[r, :, :, :] = runner_dict[
                    r
                ...
                for i in range(nframes):
                    racket_all[r] += [
                        infer_racket_from_smpl(
                            joint_pos_all[r][i].numpy(),
                            joint_rot_all[r][i].numpy(),
                            sport=opt.sport,
                            righthand=opt.player_name != ["Nadal"],
                        )
                    ]
