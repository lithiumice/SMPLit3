...
def get_sim_params(substeps=2):
    ...
    args = gymutil.parse_arguments(
        ...
        custom_parameters=[
            {
                "name": "--enable_gym_viewer",
                "action": "store_true",
                "help": "enable gym viewer",
            },
        ],
    )
...
def simulate(
    ...
    if enable_gym_viewer:
        ...
        gym.viewer_camera_look_at(
            viewer, None, gymapi.Vec3(15, -6, 3), gymapi.Vec3(0, -6, 0)
        )
    ...
    launch_ang_vel = (
        launch_vspin.view(-1, 1)
        * pi
        * 2
        * F.normalize(torch.cross(launch_vel, g_tensor[:num_ball]), dim=1)
    )
    ...
    for t in range(num_frames):
        ...
        for i in range(control_freq_inv):
            ...
            vspin[:num_ball] = torch.where(
                launch_vspin > 0, vspin[:num_ball], vspin[:num_ball] * -1
            )
            vspin = vspin.view(-1, 1)

            pass_net_now = (~has_pass_net) & (pos[:, 1] < 0)
            pass_net_success[pass_net_now] = (~has_bounce[pass_net_now]) & (
                pos[pass_net_now, 2] > NET_HEIGHT
            )
            ...
            cl = cl * torch.where(
                vspin > 0, -1 * torch.ones_like(cl), 1 * torch.ones_like(cl)
            )
...
class TennisBallGeneratorIsaac:
    def __init__(
        ...
        self.sim = sim
        self.device = (
            torch.device("cuda:0") if args.use_gpu_pipeline else torch.device("cpu")
        )
        ...
        for i in range(self.num_env):
            ...
            gym.set_rigid_body_color(
                env, ahandle, 0, gymapi.MESH_VISUAL_AND_COLLISION, gymapi.Vec3(1, 0, 0)
            )
    ...
    def reset(self):
        ...
        launch_vel_scalar = torch_sample_range(
            (num_samples), self.vel_range[0], self.vel_range[1]
        )
        launch_theta = torch_sample_range(
            (num_samples), self.theta_range[0], self.theta_range[1]
        )
        launch_vspin = torch_sample_range(
            (num_samples), self.vspin_range[0], self.vspin_range[1]
        )
        ...
        print(
            "Generated {}/{} valid ball trajectories".format(valid.sum(), num_samples)
        )
        assert valid.sum() > 0
...
def generate_incoming_trajectory(traj_path, substeps, vis=False, append=True):
    ...
    if os.path.exists(traj_path) and append:
        ...
        traj_data_all = None
    isaac_ball_gen = TennisBallGeneratorIsaac(
        {}, is_train=True, need_reset=False, substeps=substeps
    )
    for i in tqdm(range(100)):
        ...
        try:
            ...
            continue
        traj_data = torch.cat(
            [launch_pos, launch_vel, launch_vspin.view(-1, 1), traj.view(-1, 300)],
            dim=1,
        )
        ...
        print(traj_data_all.shape)
        if traj_data_all.shape[0] > 1000000:
            break
        if vis:
            break
    ...
    def generate(self, n_traj, need_init_state=False, start_pos=None, env_ids=None):
        if self.sample_random:
            ...
            if start_pos is not None:
                ...
                if other_side.sum() > 0:
                    indices[other_side] = (
                        (start_pos[other_side, 0] + 4) / 8 * len(self.traj_pool)
                    ).long()
                    indices[other_side] += torch.randint(
                        -1000, 1000, (other_side.sum(),)
                    )
