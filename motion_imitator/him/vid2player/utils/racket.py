...
TENNIS_RACKET_SEMIWESTERN = {
    ...
    "shaft_angle": np.pi / 10,
    "racket_dir": [-1, 0, 0],  # [-5, 0, 1], # 45degree [-1, 0, 1] # 90 degree [0, 0, 1]
    "racket_normal": [0, 1.0 / math.sqrt(2), 1.0 / math.sqrt(2)],
    "racket_dir_vert": [0, 1.0 / math.sqrt(2), -1.0 / math.sqrt(2)],
}
...
def infer_racket_from_smpl(
    joint_pos, joint_rot, sport="badminton", grip="eastern", righthand=True
):
    ...
    if sport == "tennis":
        ...
        shaft_left_dir_canonical = normalize(
            racket_dir_canonical / np.tan(shaft_angle) + racket_dir_vert
        )
        shaft_right_dir_canonical = normalize(
            racket_dir_canonical / np.tan(shaft_angle) - racket_dir_vert
        )
    ...
    for j in [
        ...
        rotmat = angle_axis_to_rotation_matrix(
            torch.from_numpy(joint_rot[j]).unsqueeze(0)
        )[0].numpy()
    ...
    if sport == "badminton":
        ...
        grip_pos = (
            joint_pos[Hand] + hand_normal * grip_offset + hand_to_wrist * grip_offset
        )
        ...
        shaft_left_center = (
            handle_head + shaft_left_dir * shaft_length / np.cos(shaft_angle) / 2
        )
        shaft_right_center = (
            handle_head + shaft_right_dir * shaft_length / np.cos(shaft_angle) / 2
        )
...
class Racket:
    ...
    def __init__(self, sport, device, grip="eastern", righthand=True):
        ...
        racket_dir_canonical = F.normalize(
            torch.FloatTensor(racket_params["racket_dir"]), dim=0
        ).to(device)
        racket_normal_canonical = F.normalize(
            torch.FloatTensor(racket_params["racket_normal"]), dim=0
        ).to(device)
        hand_normal_canonical = F.normalize(
            torch.FloatTensor(racket_params.get("hand_normal", [0, -1, 0])), dim=0
        ).to(device)
        ...
        self.vectors = (
            hand_normal_canonical,
            racket_dir_canonical,
            racket_normal_canonical,
        )
    ...
    def infer_with_fk(self, joint_rotmat, joint_pos_bind_rel, root_pos):
        ...
        handle_length, shaft_length, head_radius, grip_offset, grip_handle_ratio = (
            self.params
        )
...
def infer_racket_with_fk(
    ...
    transforms_mat = torch.cat(
        [
            ...
            F.pad(joint_pos_bind_rel.reshape(-1, 3, 1), [0, 0, 0, 1], value=1.0),
        ],
        dim=2,
    ...
    for i in range(1, num_joints):
        ...
        curr_res = torch.matmul(transform_chain[i - 1], transforms_mat[:, i])
