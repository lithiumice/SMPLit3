...
from embodied_pose.utils.torch_transform import (
    ypr_euler_from_quat,
...
class HumanoidSMPL:
    def __init__(
        ...
        self.viewer_camera_handle = self.gym.create_camera_sensor(
        ...
        if self.cfg["show_mujoco_viewer"]:
            ...
            def create_vis_model_xml(
                ...
                geom_capsule = Element(
                    "geom",
                    attrib={
                        "fromto": "0 0 -10000 0 0 -9999",
                        "size": "0.02",
                        "type": "capsule",
                    },
                )
                geom_sphere = Element(
                    "geom",
                    attrib={"pos": "0 0 -10000", "size": "0.02", "type": "sphere"},
                )
                geom_plane = Element(
                    "geom",
                    attrib={
        ...
        if self.cfg["env"].get("record_npz", False):
            ...
            self.record_npz_tqdm_bar = tqdm(
                range(self.cfg["env"]["num_rec_frames"]), desc="recording npz..."
            )
    ...
    def _setup_character_props(self, key_bodies):
        ...
        if asset_file == "mjcf/amp_humanoid.xml":
            ...
            self._num_obs = (
                13 + self._dof_obs_size + 37 + 3 * num_key_bodies
            )  # [root_h, root_rot, root_vel, root_ang_vel, dof_pos, dof_vel, key_body_pos]
            ...
            self._num_obs = (
                13 + self._dof_obs_size + 40 + 3 * num_key_bodies
            )  # [root_h, root_rot, root_vel, root_ang_vel, dof_pos, dof_vel, key_body_pos]
            ...
            self.dof_names = dof_names = self.gym.get_asset_dof_names(
                self.humanoid_asset
            )
            self._dof_body_ids = []
            self._dof_offsets = []
            ...
            if ENABLE_MAX_COORD_OBS:
                ...
                self._num_obs = (
                    13 + self._dof_obs_size + self._num_dof + 3 * num_key_bodies
                )  # [root_h, root_rot, root_vel, root_ang_vel, dof_pos, dof_vel, key_body_pos]
...
class HumanoidSMPLIM(HumanoidSMPL):
    ...
    def _load_motion(self, motion_file):
        ...
        if os.path.isdir(motion_file):
            ...
            if self.args.train_low_imitator:
                ...
                smpl_local_robot = Robot(
                    robot_cfg, data_dir="data/smpl", model_xml_path=model_xml_path
                )
                mujoco_joint_names = [
                    "Pelvis",
                    "L_Hip",
                ...
                if self.args.cvt_npz_to_amass:
                    sys.path.insert(
                        0,
                        "/apdcephfs/private_wallyliang/PLANT/Thirdparty/Pose_to_SMPL/fit/tools",
                    )
    ...
    def _init_context(self, motion_ids, motion_times):
        ...
        self.context_mask = all_motion_times <= (
    ...
    def end_recording_npz(self):
        ...
        to_save = {
            ...
            "poses": np.concatenate(
                [poses[:, :22, :], np.zeros((trans.shape[0], 33, 3))], axis=1
            ).reshape(-1, 55, 3)[::1],
            "gender": "male",
            "mocap_framerate": 30,
        }
    ...
    def render(self, sync_frame_time=False):
        ...
        if self.viewer:
            ...
            self.gym.set_camera_location(
                self.viewer_camera_handle, self.envs[0], new_cam_pos, new_cam_target
            )
            ...
            for evt in self.gym.query_viewer_action_events(self.viewer):
                if evt.action == "QUIT" and evt.value > 0:
                    ...
                    if not self.recording:
                        ...
                        self.writer = imageio.get_writer(
                            filename, fps=30, quality=7, macro_block_size=None
                        )
                        print(f"============ Writing video to {filename} ============")
    ...
    def render_vis(self, init=False):
        ...
        if self.recording_npz:
            ...
            pose_quat = (
                (sRot.from_quat(body_quat.cpu().reshape(-1, 4).numpy()))
                .as_quat()
                .reshape(N, -1, 4)
            )
            new_sk_state = SkeletonState.from_rotation_and_root_translation(
