...
class ResultParser(nn.Module):
    def __init__(self):
        ...
        self.part_idx = [
            args().cam_dim,
            args().rot_dim,
            (args().mano_theta_num - 1) * args().rot_dim,
            10,
        ]  # [cam_dim:3, rot_dim:3/6, pose:15*3/6, beta:10]
    ...
    def parse(self, outputs, meta_data, cfg):
        ...
        if "params_pred" in outputs:
            ...
            for i, (idx, name) in enumerate(zip(self.part_idx, self.part_name)):
                ...
                params_dict[name] = outputs["params_pred"][
                    :, idx_list[i] : idx_list[i + 1]
                ].contiguous()

            if args().Rot_type == "6D":
                params_dict["hand_pose"] = rot6D_to_angular(params_dict["hand_pose"])
                params_dict["global_orient"] = rot6D_to_angular(
                    params_dict["global_orient"]
                )

            params_dict["poses"] = torch.cat(
                [params_dict["global_orient"], params_dict["hand_pose"]], 1
            )  # 3 + 45
            L, R = outputs["left_hand_num"], outputs["right_hand_num"]
            outputs["output_hand_type"] = (
                torch.cat((torch.zeros(L), torch.ones(R))).cuda().to(torch.int32)
            )
    ...
    def determine_coeff(self, l_centers, r_centers, l_priors, r_priors):
        ...
        diff = torch.sqrt(
            (l_centers[0, 0] - r_centers[0, 0]) ** 2
            + (l_centers[0, 1] - r_centers[0, 1]) ** 2
        )
        if diff > 32:
    ...
    def reorganize_data(
        self, outputs, meta_data, exclude_keys, gt_keys, batch_ids, person_ids
    ):
    ...
    def parse_maps(self, outputs, meta_data, cfg):
        ...
        #######################
        l_center_preds_info = (
            self.centermap_parser.parse_centermap_heatmap_adaptive_scale_batch(
        ...
        #######################
        if (
            "params_pred" not in outputs
            and "l_params_maps" in outputs
            and "r_params_maps" in outputs
        ):
            if len(l_batch_ids) != 0:
                for i in range(len(l_batch_ids)):
                    detection_flag.append(True)
                outputs["l_params_pred"] = self.parameter_sampling(
            ...
            if len(r_batch_ids) != 0:
                for i in range(len(r_batch_ids)):
                    detection_flag.append(True)
                outputs["r_params_pred"] = self.parameter_sampling(
        ...
        if args().inter_prior and args().dataset != "FreiHand":
            l_cat_r, counts = torch.cat((l_batch_ids, r_batch_ids)).unique(
                return_counts=True
            )
            ...
            if len(all_hand_valid_batch_ids) > 0 and sum(detection_flag) == len(
                detection_flag
            ):
                l_corr_idx = torch.nonzero(
                    l_batch_ids[..., None] == all_hand_valid_batch_ids
                )
                r_corr_idx = torch.nonzero(
                    r_batch_ids[..., None] == all_hand_valid_batch_ids
                )
                ...
                if args().prior_mode == "cross":
                    l_prior = self.parameter_sampling(
                        outputs["l_prior_maps"],
                        all_hand_valid_batch_ids,
                        r_flat_inds[r_corr_idx],
                        use_transform=True,
                    )  # [N, 106]
                    r_prior = self.parameter_sampling(
                        outputs["r_prior_maps"],
                        all_hand_valid_batch_ids,
                        l_flat_inds[l_corr_idx],
                        use_transform=True,
                    )  # [N, 106]
                    l_prior, r_prior = self.determine_coeff(
                        l_cyxs, r_cyxs, l_prior, r_prior
                    )
                    ...
                    x = torch.cat(
                        (
                            outputs["l_params_pred"][l_corr_idx, 3:],
                            l_prior,
                            outputs["r_params_pred"][r_corr_idx, 3:],
                            r_prior,
                        ),
                        dim=1,
                    )
                    ...
                    outputs["r_params_pred"][r_corr_idx, 3:] = merged_outputs[
                        :, 106:212
                    ]
        ...
        outputs["params_pred"] = torch.cat(
            (outputs["l_params_pred"], outputs["r_params_pred"])
        )
        ...
        if "centers_pred" not in outputs and len(batch_ids) != 0:
            outputs["l_centers_pred"] = torch.stack(
                [
                    l_flat_inds % args().centermap_size,
                    torch.div(
                        l_flat_inds, args().centermap_size, rounding_mode="floor"
                    ),
                ],
                1,
            )
            outputs["r_centers_pred"] = torch.stack(
                [
                    r_flat_inds % args().centermap_size,
                    torch.div(
                        r_flat_inds, args().centermap_size, rounding_mode="floor"
                    ),
                ],
                1,
            )
            outputs["l_centers_conf"] = self.parameter_sampling(
                outputs["l_center_map"], l_batch_ids, l_flat_inds, use_transform=True
            )
            outputs["r_centers_conf"] = self.parameter_sampling(
                outputs["r_center_map"], r_batch_ids, r_flat_inds, use_transform=True
            )
...
class CenterMap(object):
    ...
    def generate_kernels(self, kernel_size_list):
        ...
        for kernel_size in set(kernel_size_list):
            ...
            gaussian_distribution = -((x - x0) ** 2 + (y - y0) ** 2) / (
                2 * self.sigma**2
            )
            gk_group[kernel_size] = np.exp(gaussian_distribution)
            pool_group[kernel_size] = torch.nn.MaxPool2d(
                kernel_size, 1, (kernel_size - 1) // 2
            )
        return gk_group, pool_group

    def parse_centermap_heatmap_adaptive_scale_batch(
        self, center_maps, train_flag=False
    ):
        center_map_nms = nms(
            center_maps, pool_func=self.pool_group[args().kernel_sizes[-1]]
        )
