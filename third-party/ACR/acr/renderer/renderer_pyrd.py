...
class Renderer(object):
    ...
    def __init__(self, focal_length=600, height=512, width=512, **kwargs):
        ...
        self.colors = [
            (0.7, 0.7, 0.6, 1.0),
            (0.7, 0.5, 0.5, 1.0),  # Pink
            (0.5, 0.5, 0.7, 1.0),  # Blue
            (0.5, 0.55, 0.3, 1.0),  # capsule
            (0.3, 0.5, 0.55, 1.0),  # Yellow
        ]

    def __call__(
        self, verts, faces, colors=None, focal_length=None, camera_pose=None, **kwargs
    ):
        # Need to flip x-axis
        ...
        camera = pyrender.camera.IntrinsicsCamera(
            fx=fx, fy=fy, cx=self.camera_center[0], cy=self.camera_center[1]
        )
...
def get_renderer(test=False, **kwargs):
    ...
    if test:
        ...
        model = pickle.load(
            open(os.path.join(args().smpl_model_path, "SMPL_NEUTRAL.pkl"), "rb"),
            encoding="latin1",
        )
        np_v_template = (
