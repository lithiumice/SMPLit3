...
def get_kp2d_on_org_img(kp2d, offset):
    assert kp2d.shape[1] >= 2, print(
        "Espected shape of kp2d is Kx2, while get {}".formt(kp2d.shape)
    )
...
def collect_image_list(image_folder=None, collect_subdirs=False):
    ...
    def collect_image_from_subfolders(
        image_folder, file_list, collect_subdirs, img_exts
    ):
        for path in glob.glob(os.path.join(image_folder, "*")):
            if os.path.isdir(path) and collect_subdirs:
                collect_image_from_subfolders(
                    path, file_list, collect_subdirs, img_exts
                )
            elif os.path.splitext(path)[1] in img_exts:
                file_list.append(path)
        return file_list

    file_list = collect_image_from_subfolders(
        image_folder, [], collect_subdirs, img_exts
    )
...
def save_results(image_folder, output_dir, results_dict):
    ...
    with open(
        output_dir
        + f"/{path_name}_hand{model_name}_{args().centermap_conf_thresh}.pkl",
        "wb",
    ) as f:
...
def save_result_dict_tonpz(results, test_save_dir, side):
    for img_path, result_dict in results.items():
        ...
        file_name = (
            "_".join(os.path.splitext(file_name)).replace(".", "") + f"_{side}.npz"
        )
...
def shrink(leftTop, rightBottom, width, height):
    ...
    if (xl <= 0 and xr <= 0) or (yt <= 0 and yb <= 0):
        ...
        rightBottom[0] <= width and rightBottom[1] <= height
    ):  # right bottom corner is in box
        l = min(yt, xl)
        r = r - l / 2
        cx = cx + l / 2
        cy = cy + l / 2
...
def calc_aabb_batch(ptSets_batch):
    ...
    ptLeftTop = np.array(
        [np.min(ptSets_batch[:, :, 0], axis=1), np.min(ptSets_batch[:, :, 1], axis=1)]
    ).T
    ptRightBottom = np.array(
        [np.max(ptSets_batch[:, :, 0], axis=1), np.max(ptSets_batch[:, :, 1], axis=1)]
    ).T
    bbox = np.concatenate(
        (ptLeftTop.reshape(batch_size, 1, 2), ptRightBottom.reshape(batch_size, 1, 2)),
        axis=1,
    )
    return bbox
...
def vertices_kp3d_projection(
    outputs, params_dict, meta_data=None, presp=args().model_version > 3
):
    params_dict, vertices, j3ds = params_dict, outputs["verts"], outputs["j3d"]
    verts_camed = batch_orth_proj(
        vertices, params_dict["cam"], mode="3d", keep_dim=True
    )
...
def estimate_translation(
    ...
    for i in range(joints_3d.shape[0]):
        ...
        try:
            trans[i] = estimate_translation_cv2(
                S_i[valid_mask],
                joints_i[valid_mask],
...
def batch_global_rigid_transformation(
    Rs, Js, parent, rotate_base=False, root_rot_mat=None
):
    """
    进行成堆的全局刚性变换。
    """
...
def batch_global_rigid_transformation_cpu(
    Rs, Js, parent, rotate_base=False, root_rot_mat=None
):
    """
    进行成堆的全局刚性变换。
...
def get_rectangle_intersect_ratio(lt0, rb0, lt1, rb1):
    (lt0, rb0), (lt1, rb1) = get_intersected_rectangle(
        lt0, rb0, lt1, rb1
    ), get_union_rectangle(lt0, rb0, lt1, rb1)
...
def copy_state_dict(
    cur_state_dict, pre_state_dict, prefix="module.", drop_prefix="", fix_loaded=False
):
    ...
    for k in cur_state_dict.keys():
        ...
        try:
            ...
            continue
    logging.info(
        "missing parameters of layers:{}, {}".format(len(failed_layers), failed_layers)
    )
    logging.info(
        "success layers:{}/{}, pre_state_dict have {}".format(
            len(success_layers), len(cur_state_dict.keys()), len(pre_state_dict.keys())
        )
    )
    logging.info(
        "**************************************************************************"
    ...
    if fix_loaded and len(failed_layers) > 0:
        print(
            "fixing the layers that were loaded successfully, while train the layers that failed,"
        )
        for k in cur_state_dict.keys():
            try:
...
def determine_rendering_order(rendered_img, thresh=0.0):
...
def reorganize_results(outputs, img_paths, reorganize_idx):
    ...
    cam_results = (
        outputs["params_dict"]["cam"]
        .detach()
        .cpu()
        .numpy()
        .astype(np.float16)[detected]
    )
    trans_results = (
        outputs["cam_trans"].detach().cpu().numpy().astype(np.float16)[detected]
    )
    mano_pose_results = (
        outputs["params_dict"]["poses"]
        .detach()
        .cpu()
        .numpy()
        .astype(np.float16)[detected]
    ...
    pj2d_org_results = (
        outputs["pj2d_org"].detach().cpu().numpy().astype(np.float16)[detected]
    )
    hand_type = (
        outputs["output_hand_type"].detach().cpu().numpy().astype(np.int32)[detected]
    )
...
def image_crop_pad(
    ...
    if bbox is not None:
        assert len(bbox) == 4, print(
            "bbox input of image_crop_pad is supposed to be in length 4!, while {} is given".format(
                bbox
            )
        )
...
def image_pad_white_bg(image, pad_trbl=None, pad_ratio=1.0, pad_cval=255):
    ...
    pad_func = iaa.Sequential(
        [iaa.Pad(px=pad_trbl, keep_size=False, pad_mode="constant", pad_cval=pad_cval)]
    )
...
def process_image_ori(originImage, bbox=None):
    ...
    image_aug, kp2ds_aug, offsets = image_crop_pad(
        originImage, bbox=bbox, pad_ratio=1.0
    )
    return orgImage_white_bg, offsets


def img_preprocess(
    image, imgpath=None, input_size=512, single_img_input=False, bbox=None
):
    # args:
    # image: bgr frame
    image = image[:, :, ::-1]
    ...
    image = torch.from_numpy(
        cv2.resize(image_org, (input_size, input_size), interpolation=cv2.INTER_CUBIC)
    )
    ...
    input_data = {"image": image, "offsets": offsets, "data_set": "internet"}  #
...
def split_frame(videopath):
    ...
    if vc.isOpened():
        ...
        if "MOV" not in videopath:
            ...
            cv2.imencode(".jpg", frame[::-1, ::-1])[1].tofile(
                path + "/" + str(0).zfill(6) + ".jpg"
            )
    else:
        rval = False
...
def extract_motion_sequence(results_track_video, video_track_ids):
    ...
    for subject_id in subject_ids:
        frame_ids_appear = {
            fid: np.where(np.array(track_ids) == subject_id)[0][0]
            for fid, track_ids in video_track_ids.items()
            if subject_id in track_ids
        }
        motion_sequence[subject_id] = [
            results_track_video[frame_ids[fid]][sid]
            for fid, sid in frame_ids_appear.items()
        ]
...
def smooth_global_rot_matrix(pred_rots, OE_filter):
    ...
    smoothed_rot = rotation_matrix_to_angle_axis(
        smoothed_rot_mat.reshape(1, 3, 3)
    ).reshape(-1)
    return smoothed_rot
...
def create_OneEuroFilter(smooth_coeff):
    return {
        "poses": OneEuroFilter(smooth_coeff, 0.7),
