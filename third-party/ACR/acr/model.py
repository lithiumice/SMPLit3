...
class ACR(nn.Module):
    ...
    def head_forward(self, x, gt_segm=None):
        ...
        (
            ...
            l_prior_maps,
            r_prior_maps,
        ) = self.global_forward(x)
        l_params_maps, r_params_maps, segms = self.part_forward(
            x, gt_segm, pred_segm, l_params_maps, r_params_maps
        )
        output = {
            "l_params_maps": l_params_maps.float(),
    ...
    def part_forward(self, x, gt_segm, pred_segm, l_params_maps, r_params_maps):
        ...
        part_attention = F.interpolate(
            pred_segm.clone().float(), scale_factor=(1 / 2, 1 / 2), mode="nearest"
        )  # because pred_segm is [256, 256], part-attention need to be 128
        logits = pred_segm
        part_attention = part_attention[:, 1:, :, :]

        ############## 3D SMPL BRANCH FEATURES ##############
        deconv_contact_features = self.contact_layers[1](
            x
        )  # from Bx (128+2)x128x128 -> Bx 256x128x128
        deconv_shape_features = self.cam_shape_layers[1](
            deconv_contact_features
        )  # from Bx 256x128x128 -> Bx 64x128x128, (smpl_final_layer)

        ############## SAMPLE LOCAL FEATURES ##############
        weighted_contact_features = self.Hadamard_product(
            deconv_contact_features, part_attention
        )  # Hadamard product: (Bx256x128x128) * σ(BxJx128x128) -> Bx 256xJ
        weighted_shape_features = self.Hadamard_product(
            deconv_shape_features, part_attention
        )  # Hadamard product: (Bx256xHxW) * σ(BxJxHxW) -> Bx 64xJ
        weighted_contact_features = weighted_contact_features.unsqueeze(
            -1
        )  # Bx 256xJ -> Bx 256xJx1

        ############ SORT LEFT AND RIGHT ORDER #############
        # pose feature
        l_weighted_contact_features, r_weighted_contact_features = (
            weighted_contact_features[:, :, 16:, :],
            weighted_contact_features[:, :, :16, :],
        )  # Bx 256x16x1

        # shape feature
        l_weighted_shape_features, r_weighted_shape_features = (
            weighted_shape_features[:, :, 16:],
            weighted_shape_features[:, :, :16],
        )
        l_weighted_shape_features = torch.flatten(
            l_weighted_shape_features, start_dim=1
        )  # Bx (64*16)
        r_weighted_shape_features = torch.flatten(
            r_weighted_shape_features, start_dim=1
        )  # Bx (64*16)

        ############## GET FINAL PREDICTIONS ##############
        # left
        l_contact_offsets = (
            self.contact_layers[2](l_weighted_contact_features)
            .squeeze(-1)
            .transpose(2, 1)
            .reshape(BS, 96)
        )  # Bx 256xJx1 conv-> [Bx(96)xJx1 or  B x 6 x J x1] -> [Bx1x96 or BxJx6] -reshape>  [Bx96, Bx96]
        l_shape_offsets = self.cam_shape_layers[2](
            l_weighted_shape_features
        )  # Bx (64*J) -> Bx(10)

        # right
        r_contact_offsets = (
            self.contact_layers[3](r_weighted_contact_features)
            .squeeze(-1)
            .transpose(2, 1)
            .reshape(BS, 96)
        )  # Bx 256xJx1 -> [Bx(96)xJx1 or  B x 6 x J x1] -> [Bx1x96 or BxJx6] -reshape> [Bx96, Bx96]
        r_shape_offsets = self.cam_shape_layers[3](
            r_weighted_shape_features
        )  # Bx (64*J) -> Bx(10)

        # replicate original camera for concat
        l_pare_features = (
            torch.cat((l_contact_offsets, l_shape_offsets), dim=1)
            .unsqueeze(-1)
            .unsqueeze(-1)
            .repeat(1, 1, 64, 64)
        )  # Bx109x1x1 -> Bx109x64x64
        r_pare_features = (
            torch.cat((r_contact_offsets, r_shape_offsets), dim=1)
            .unsqueeze(-1)
            .unsqueeze(-1)
            .repeat(1, 1, 64, 64)
        )  # Bx109x1x1 -> Bx109x64x64
        l_params_pare = torch.cat(
            (l_params_maps[:, :3].clone(), l_pare_features), dim=1
        )
        r_params_pare = torch.cat(
            (r_params_maps[:, :3].clone(), r_pare_features), dim=1
        )

        l_params_maps = self.contact_layers[4](
            torch.cat((l_params_maps, l_params_pare), dim=1)
        )
        r_params_maps = self.contact_layers[5](
            torch.cat((r_params_maps, r_params_pare), dim=1)
        )
    ...
    def _build_head(self):
        ...
        self.head_cfg = {
            "NUM_HEADS": 1,
            "NUM_CHANNELS": 64,
            "NUM_BASIC_BLOCKS": args().head_block_num,
        }
        self.output_cfg = {
            "NUM_PARAMS_MAP": params_num - cam_dim,
            "NUM_CENTER_MAP": 1,
            "NUM_CAM_MAP": cam_dim,
        }
        ...
        self.segmentation_layers = self._make_segmentation_layers(
            self.backbone.backbone_channels
        )
    ...
    def _make_final_layers(self, input_channels):
        ...
        if args().merge_mano_camera_head:
            ...
            raise NotImplementedError
            final_layers.append(
                self._make_head_layers(
                    input_channels,
                    self.output_cfg["NUM_PARAMS_MAP"] + self.output_cfg["NUM_CAM_MAP"],
                )
            )
            final_layers.append(
                self._make_head_layers(
                    input_channels, self.output_cfg["NUM_CENTER_MAP"]
                )
            )
        else:
            final_layers.append(
                self._make_head_layers(
                    input_channels, self.output_cfg["NUM_PARAMS_MAP"]
                )
            )
            final_layers.append(
                self._make_head_layers(
            ...
            final_layers.append(
                self._make_head_layers(input_channels, self.output_cfg["NUM_CAM_MAP"])
            )
            if args().inter_prior:
                final_layers.append(
                    self._make_head_layers(
    ...
    def _make_segmentation_layers(self, input_channels):
        ...
        final_layers.append(
            nn.Sequential(
                nn.Conv2d(
                    in_channels=256, out_channels=33, kernel_size=1, stride=1, padding=0
                )
            )
        )
    ...
    ):  # from [B, C, H, W] to [B, aim_c, H/2, W/2]
        head_layers = []
...
class Up(nn.Module):
    ...
    def forward(self, x1):
        if self.bilinear:
            x1 = F.interpolate(
                x1, scale_factor=(2, 2), mode="bilinear", align_corners=True
            )
        else:
            x1 = self.up(x1)
        return self.conv(x1)
