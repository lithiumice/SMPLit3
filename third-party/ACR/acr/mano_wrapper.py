...
class MANOWrapper(nn.Module):
    def __init__(self):
        ...
        mano_root = "/apdcephfs/private_wallyliang/PLANTmodels/uhc_data/smpl"
    ...
    def forward(self, outputs, meta_data):
        ...
        l_vertices, l_joints, _ = self.mano_layer["l"](
            params_dict["poses"][:L], th_betas=params_dict["betas"][:L]
        )  # if empty, return empty
        r_vertices, r_joints, _ = self.mano_layer["r"](
            params_dict["poses"][L : L + R], th_betas=params_dict["betas"][L : L + R]
        )

        mano_outs = {
            "verts": torch.cat((l_vertices, r_vertices)),
            "j3d": torch.cat((l_joints, r_joints)),
        }
        outputs.update({**mano_outs})
        outputs.update(
            vertices_kp3d_projection(
                outputs,
