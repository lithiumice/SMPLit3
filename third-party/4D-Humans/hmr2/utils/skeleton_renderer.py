...
class SkeletonRenderer:
    ...
    def __call__(
        ...
        pred_keypoints_3d_proj = perspective_projection(
            pred_keypoints_3d.reshape(batch_size, -1, 3),
            rotation=rotation.repeat(batch_size, 1, 1),
        ...
        pred_keypoints_3d_proj = (
            torch.cat(
                [
                    pred_keypoints_3d_proj,
                    keypoints_to_render.reshape(batch_size, -1, 1),
                ],
                dim=-1,
            )
            .cpu()
        ...
        pred_keypoints_3d_proj_side = perspective_projection(
            pred_keypoints_3d_side.reshape(batch_size, -1, 3),
            rotation=rotation.repeat(batch_size, 1, 1),
            translation=camera_translation.reshape(batch_size, -1),
            focal_length=focal_length.repeat(batch_size, 1),
            camera_center=camera_center.repeat(batch_size, 1),
        ...
        for i in range(batch_size):
            ...
            for op, gt in zip(openpose_indices, gt_indices):
                if (
                    pred_keypoints_3d_proj_side[i, gt, -1]
                    >= body_keypoints_3d_proj[op, -1]
                ):
