    ...
    def forward(
        ...
        **kwargs,
        ...
        left_hand_pose = (
            ...
            else self.left_hand_pose.repeat(batch_size, 1)
        )
        right_hand_pose = (
            right_hand_pose
            if right_hand_pose is not None
            else self.right_hand_pose.repeat(batch_size, 1)
        )
        jaw_pose = (
            jaw_pose if jaw_pose is not None else self.jaw_pose.repeat(batch_size, 1)
        )
        leye_pose = (
            leye_pose if leye_pose is not None else self.leye_pose.repeat(batch_size, 1)
        )
        reye_pose = (
            reye_pose if reye_pose is not None else self.reye_pose.repeat(batch_size, 1)
        )
        expression = (
            expression
            if expression is not None
            else self.expression.repeat(batch_size, 1)
        )
