    ...
    def load_tars_as_webdataset(
        ...
        **kwargs
        ...
        def supress_bad_poses(item):
            ...
            if has_hand_pose:
                ...
                pose_is_probable = poses_check_probable(
                    torch.from_numpy(hand_pose)[None, 3:], amass_poses_hist100_smooth
                ).item()
        ...
        def poses_betas_simultaneous(item):
            ...
            has_betas = item["data.pyd"]["has_betas"]
            has_hand_pose = item["data.pyd"]["has_hand_pose"]
            item["data.pyd"]["has_betas"] = item["data.pyd"]["has_hand_pose"] = (
                np.array(float((has_hand_pose > 0) and (has_betas > 0)))
            )
            return item
        ...
        dataset = dataset.map(
            lambda x: ImageDataset.process_webdataset_tar_item(
                x,
                train,
