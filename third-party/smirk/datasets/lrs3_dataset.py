...
def get_datasets_LRS3(config):
    if not os.path.exists("assets/LRS3_lists.pkl"):
        print(
            "Creating train, validation, and test lists for LRS3... (This only happens once)"
        )
    ...
    test_list = lists[2]
    return (
        LRS3Dataset(train_list, config=config),
        LRS3Dataset(val_list, config=config, test=True),
        LRS3Dataset(test_list, config=config, test=True),
    )
...
def get_LRS3_test(config):
    ...
    def gather_LRS3_split(folder_list, split="trainval"):
        ...
        for folder in folder_list:
            for file in os.listdir(
                os.path.join(f"{config.dataset.LRS3_path}/{split}", folder)
            ):
                if file.endswith(".txt"):
                    ...
                    landmarks_filename = os.path.join(
                        config.dataset.LRS3_landmarks_path, file_inner_path + ".pkl"
                    )
                    subject = folder
                    mediapipe_landmarks_filepath = os.path.join(
                        config.dataset.LRS3_path, file_inner_path + ".npy"
                    )

                    list_.append(
                        [
                            os.path.join(
                                config.dataset.LRS3_path, file_inner_path + ".mp4"
                            ),
                            os.path.join(
                                config.dataset.LRS3_landmarks_path,
                                file_inner_path + ".pkl",
                            ),
    ...
    return LRS3Dataset(
        test_list, config=config, test=True, sample_full_video_for_testing=True
    )
