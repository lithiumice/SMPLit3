...
class MEADSidesDataset(BaseDataset):
    ...
    def __getitem_aux__(self, index):
        ...
        data_dict = self.prepare_data(
            image=image, landmarks_fan=None, landmarks_mediapipe=landmarks_mediapipe
        )
...
def get_datasets_MEAD_sides(config=None):
    ...
    if not os.path.exists("assets/MEAD_lists.pkl"):
        print(
            "Creating train, validation, and test lists for MEAD Sides... (This only happens once)"
        )
        ...
        for view in [
            ...
            files = [
                f
                for f in os.listdir(os.path.join(config.dataset.MEAD_sides_path, view))
            ]
            ...
            for file in files:
                if file.split("_")[0] in train_subjects:
                    folder_path = os.path.join(
                        config.dataset.MEAD_sides_path,
                        view,
                        file.split(".")[0] + ".mp4",
                    )
            ...
            pickle.dump(
                [train_list, val_list, test_list], open(f"assets/MEAD_lists.pkl", "wb")
            )
    else:
        train_list, val_list, test_list = pickle.load(
            open("assets/MEAD_lists.pkl", "rb")
        )
    ...
    return (
        MEADSidesDataset(train_list, config),
        MEADSidesDataset(val_list, config, test=True),
        MEADSidesDataset(test_list, config, test=True),
    )
