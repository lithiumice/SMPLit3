...
def load_dataloaders(config):
    ...
    train_dataset_MEAD_sides, val_dataset_MEAD_sides, test_dataset_MEAD_sides = (
        get_datasets_MEAD_sides(config)
    )
...
def create_LRS3_lists(lrs3_path, lrs3_landmarks_path):
    ...
    train_folder_list, val_folder_list = train_test_split(
        trainval_folder_list, test_size=0.2, random_state=42
    )
            ...
            for file in os.listdir(os.path.join(f"{lrs3_path}/{split}", folder)):
                if file.endswith(".txt"):
                    ...
                    landmarks_filename = os.path.join(
                        lrs3_landmarks_path, file_inner_path + ".pkl"
                    )
                    ...
                    if os.path.exists(landmarks_filename) and valid:
                        ...
                        list_.append(
                            [
                                os.path.join(lrs3_path, file_inner_path + ".mp4"),
                                os.path.join(
                                    lrs3_landmarks_path, file_inner_path + ".pkl"
                                ),
