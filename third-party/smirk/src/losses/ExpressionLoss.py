...
class ExpressionLoss(nn.Module):
    ...
    def __init__(self):
        ...
        self.backbone = (
            resnet50(num_classes=100, include_top=False, emoca_specific=True)
            .eval()
            .cuda()
        )

        emotion_checkpoint = torch.load(
            f"{_DATA}/ResNet50/checkpoints/deca-epoch=01-val_loss_total/dataloader_idx_0=1.27607644.ckpt"
        )["state_dict"]
