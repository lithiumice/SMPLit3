...
class FLAME(nn.Module):
    ...
    def __init__(
        self,
        flame_model_path=f"{_DATA}/generic_model.pkl",
        flame_lmk_embedding_path=f"{_DATA}/landmark_embedding.npy",
        n_shape=300,
        n_exp=50,
    ):
        ...
        self.register_buffer(
            "l_eyelid",
            torch.from_numpy(np.load(f"{_DATA}/l_eyelid.npy")).to(self.dtype)[None],
        )
        self.register_buffer(
            "r_eyelid",
            torch.from_numpy(np.load(f"{_DATA}/r_eyelid.npy")).to(self.dtype)[None],
        )
