...
_DATA = os.path.join(os.path.dirname(__file__), "../../../../model_files/smirk")


def load_templates():
    templates_path = f"{_DATA}/expression_templates_famos"
    classes_to_load = [
        "lips_back",
        "rolling_lips",
        "mouth_side",
        ...
        if os.path.isdir(os.path.join(templates_path, subject)):
            for template in os.listdir(os.path.join(templates_path, subject)):
                ...
                if template not in classes_to_load:
                    continue
                exps = []
                for npy_file in os.listdir(
                    os.path.join(templates_path, subject, template)
                ):
                    params = np.load(
                        os.path.join(templates_path, subject, template, npy_file),
                        allow_pickle=True,
                    )
