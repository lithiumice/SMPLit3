...
def L_trans(source, target, T, up=True):
    ...
    origin = target[:, 0]
    trans = source
    trans[..., model.v_axis] = trans[..., model.v_axis] + origin[
        ..., model.v_axis
    ].unsqueeze(1)
    trans_gt = target
...
def interpolation(idx1, sample1, source1, idx2, sample2, source2, output_dir):
    ...
    for i in range(z_l.shape[0]):
        ...
        np.savez(
            os.path.join(output_dir, f"{idx1}-{idx2}_lerp_{i}.npz"),
            poses=poses,
            trans=c2c(trans[i]),
            betas=np.zeros(10),
...
def run_interpolation():
    ...
    for pair in zip(subset1, subset2):
        ...
        source1["rotmat"] = rotation_6d_to_matrix(
            sample1["global_xform"].to(model.device)
        )
        ...
        source2["rotmat"] = rotation_6d_to_matrix(
            sample2["global_xform"].to(model.device)
        )
...
def motion_reconstruction(target, output_dir, steps, T=None, offset=0):
    ...
    for step in steps:
        ...
        for i in range(rotmat.shape[0]):
            ...
            np.savez(
                os.path.join(output_dir, f"recon_{offset + i:03d}_{fps}fps.npz"),
                poses=poses,
                trans=c2c(trans[i]),
                betas=np.zeros(10),
                gender=args.data.gender,
            ...
            if step == 1:
                np.savez(
                    os.path.join(output_dir, f"recon_{offset + i:03d}_gt.npz"),
                    poses=poses_gt,
                    trans=c2c(trans_gt[i]),
                    betas=np.zeros(10),
                    gender=args.data.gender,
                    mocap_framerate=args.data.fps,
                )
...
def run_motion_reconstruction():
    ...
    for data in data_loader:
        ...
        error = motion_reconstruction(
            target, output_dir, steps=[0.5, 1.0], offset=offset
        )
...
def slerp_initialization(data, T, mask):
    ...
    local_rotmat = fk.global_to_local(
        global_rotmat.view(-1, n_joints, 3, 3)
    )  # (B x T, J, 3, 3)
    ...
    for i in range(local_quat.shape[0]):
        quat, trans = slerp(
            local_quat[i], data["trans"][i], T, range(args.data.clip_length), mask
        )
        orient, _ = slerp(
            root_orient[i], data["trans"][i], T, range(args.data.clip_length), mask
        )
    ...
    slerp_orient = (
        torch.from_numpy(slerp_orient).squeeze(2).to(model.device)
    )  # (B, T, 4)
    ...
    slerp_xform = slerp_xform[:, :, :3, :3].view(
        b_size, -1, n_joints, 3, 3
    )  # (B, T, J, 3, 3)
...
def run_motion_inbetweening():
    ...
    for interval in clip_inbetweens:
        ...
        T = list(range(end_frame)) + list(
            range(end_frame + interval, args.data.clip_length)
        )
        print(T)
        ...
        for data in data_loader:
            ...
            target = dict()
            target["pos"] = data["pos"].to(model.device)
            target["rotmat"] = rotation_6d_to_matrix(
                data["global_xform"].to(model.device)
            )
            target["trans"] = data["trans"].to(model.device)
            target["root_orient"] = data["root_orient"].to(model.device)

            error = motion_reconstruction(
                target, sub_folder_dir, steps=[1], T=T, offset=offset
            )
...
def load_aist_data(path):
    ...
    change_of_basis = torch.FloatTensor([[[1, 0, 0], [0, 0, -1], [0, 1, 0]]]).cuda()
    ...
    global_xform = global_xform[
        :, :, :3, :3
    ]  # global transformation matrix for each joint (T, J, 3, 3)
    ...
    )  # angular velocity of all the joints (1, T, 24, 3)
    velocity = estimate_linear_velocity(
        pos, dt=1.0 / args.data.fps
    )  # linear velocity of all the joints (1, T, J, 3)
    root_vel = estimate_linear_velocity(
        trans, dt=1.0 / args.data.fps
    )  # linear velocity of the root joint (1, T, 3)
...
def process_video(path, output_dir, start_frame, end_frame):
    ...
    command = [
        ...
        f"{output_dir}/{filename}.mp4",
        "-ss",
        str(start_frame / args.data.fps),
        "-to",
        str(end_frame / args.data.fps),
        "-c:v",
        "libx264",
...
def run_aist_dance_inbetweening():
    ...
    output_dir = os.path.join(
        opt.save_path, "aist_inbetweening", datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
    )
    os.makedirs(output_dir, exist_ok=True)
    ...
    for video_file1, video_file2 in list(zip(subset1, subset2)):
        ...
        aist_file1 = glob.glob(
            os.path.join(
                aist_dataset, "motions", f"{filename1.replace('c01', 'cAll')}.pkl"
            )
        )
        aist_file2 = glob.glob(
            os.path.join(
                aist_dataset, "motions", f"{filename2.replace('c01', 'cAll')}.pkl"
            )
        )
        ...
        for t in interval_length:
            ...
            process_video(
                video_file2, sub_output_dir, start_frame=0, end_frame=106 - t + 128
            )
            ...
            for key in sample1.keys():
                ...
                data[1][key] = torch.cat(
                    (sample1[key][:, 128:150], sample2[key][:, : 106 - t]), dim=1
                )
            ...
            for i in range(3):
                ...
                if i == 1:
                    ...
                    target["rotmat"] = rotation_6d_to_matrix(
                        initial["global_xform"].float().to(model.device)
                    )
                    target["trans"] = initial["trans"].float().to(model.device)
                    target["root_orient"] = (
                        initial["root_orient"].float().to(model.device)
                    )
                else:
                    T = None
                    ...
                    target["rotmat"] = rotation_6d_to_matrix(
                        data[i]["global_xform"].to(model.device)
                    )
            ...
            with torch.no_grad():
                output = model.decode(
                    z_l_total, z_g_total, length=args.data.clip_length, step=1
                )
            ...
            np.savez(
                os.path.join(sub_output_dir, f"{filename1}_{filename2}_{t}.npz"),
                poses=poses,
                trans=c2c(trans),
                betas=np.zeros(10),
                gender=args.data.gender,
...
def motion_renavigating(reference, traj, z_l=None):
    ...
    if z_l is None:
        z_l = Variable(
            torch.randn(reference["rotmat"].shape[0], args.local_prior.z_dim).to(
                model.device
            ),
            requires_grad=True,
        )
    ...
    if args.data.root_transform:  # optimize both z_l and z_g
        z_g = Variable(
            torch.randn(reference["rotmat"].shape[0], args.global_prior.z_dim).to(
                model.device
            ),
            requires_grad=True,
        )
    ...
    canonical_frame_ref = build_canonical_frame(
        forward_ref, up_axis=args.data.up
    )  # (B, T, 3, 3)
    root_rotation_ref = torch.matmul(
        torch.linalg.inv(canonical_frame_ref), root_orient_ref
    )  # (B, T, 3, 3), local -> world -> canonical
    root_rotation_ref = root_rotation_ref.unsqueeze(2)  # (B, T, 1, 3, 3)
    pos_ref = torch.matmul(root_rotation_ref, reference["pos"].unsqueeze(-1)).squeeze(
        -1
    )  # (B, T, J, 3)
    pos_ref[..., "xyz".index(args.data.up)] += reference["trans"][
        ..., "xyz".index(args.data.up)
    ].unsqueeze(
        -1
    )  # add root height to joint positions
    angle_ref = compute_orient_angle(
        root_orient_ref, reference["trans"].clone()
    )  # (B, T)
    ...
    for i in range(args.iterations):
        ...
        root_rotation = torch.matmul(
            torch.linalg.inv(canonical_frame), root_orient
        )  # (B, T, 3, 3)
        root_rotation = root_rotation.unsqueeze(2)  # (B, T, 1, 3, 3)
        pos = torch.matmul(root_rotation, output["pos"].unsqueeze(-1)).squeeze(
            -1
        )  # (B, T, J, 3)
        pos[..., "xyz".index(args.data.up)] += output["trans"][
            ..., "xyz".index(args.data.up)
        ].unsqueeze(-1)
        ...
        if args.dtw_loss:
            ...
            angle_loss = L_DTW(angle, angle_ref) - 0.5 * (
                L_DTW(angle, angle) + L_DTW(angle_ref, angle_ref)
            )
            loss = (
                args.lambda_trans * trans_loss
                + args.lambda_dtw * sim_loss.mean()
                + args.lambda_angle * angle_loss.mean()
            )
        else:
            ...
            loss = (
                args.lambda_trans * trans_loss
                + args.lambda_pos * sim_loss
                + args.lambda_angle * angle_loss
            )
        ...
        print(
            "[{:03d}/{}] trans_loss: {:.4f}\t sim_loss: {:.4f}\t angle_loss: {:.4f}\t loss: {:.4f}".format(
                i,
                args.iterations,
                trans_loss.item(),
                sim_loss.item(),
...
def analytical_trajectory(traj_type, num_points):
    ...
    if traj_type == "line":
        ...
        traj[:, : theta.shape[0], 1] = (
            13 * torch.cos(theta)
            - 5 * torch.cos(2 * theta)
            - 2 * torch.cos(3 * theta)
            - torch.cos(4 * theta)
        ) * scale_y
        traj[:, theta.shape[0] :, 0] = torch.linspace(
            start=0, end=16 * scale_x, steps=28
        )
...
def run_motion_renavigating():
    ...
    output_dir = os.path.join(
        opt.save_path, "motion_renavi", datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
    )
    os.makedirs(output_dir, exist_ok=True)
    ...
    if args.data.root_transform:
        ...
        root_orient_ref = rotation_6d_to_matrix(
            reference["root_orient"][0]
        )  # (T, 3, 3)
        local_rotmat[:, 0] = root_orient
    ...
    np.savez(
        os.path.join(output_dir, f"renavi.npz"),
        poses=poses,
        trans=c2c(trans),
        betas=np.zeros(10),
        gender=args.data.gender,
    ...
    np.savez(
        os.path.join(output_dir, f"reference.npz"),
        poses=poses_ref,
        trans=c2c(trans_ref),
        betas=np.zeros(10),
        gender=args.data.gender,
    ...
    export_ply_trajectory(
        points=trans,
        color=(0, 255, 255),
        ply_fname=os.path.join(output_dir, "renavi_traj.ply"),
    )
    export_ply_trajectory(
        points=trans_gt,
        color=(0, 255, 0),
        ply_fname=os.path.join(output_dir, "gt_traj.ply"),
    )
...
def time_translation():
    ...
    output_dir = os.path.join(
        opt.save_path, "time_translation", datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
    )
    os.makedirs(output_dir, exist_ok=True)
    ...
    for subject in ["subject_02", "subject_03", "subject_04"]:
        ...
        offset = 10
        seq_num = (
            int((all_motion["rotmat"].shape[1] - args.data.clip_length) // offset) + 1
        )
        seqs.append(seq_num)
        for i in range(seq_num):
            ...
            for key in all_motion.keys():
                if key != "scaling":
                    data[key] = all_motion[key][
                        :, i * offset : i * offset + args.data.clip_length
                    ]
    ...
    ax.plot(
        z_l_data[seq[0] : seq[1], 0],
        z_l_data[seq[0] : seq[1], 1],
        z_l_data[seq[0] : seq[1], 2],
        linestyle="-",
        marker="o",
        c="red",
    )
    ax.plot(
        z_l_data[seq[1] : seq[2], 0],
        z_l_data[seq[1] : seq[2], 1],
        z_l_data[seq[1] : seq[2], 2],
        linestyle="-",
        marker="o",
        c="green",
    )
    ...
    ax = fig.add_subplot(111, projection="3d")
    ax.plot(
        z_g_data[seq[0] : seq[1], 0],
        z_g_data[seq[0] : seq[1], 1],
        z_g_data[seq[0] : seq[1], 2],
        linestyle="-",
        marker="o",
        c="red",
    )
    ax.plot(
        z_g_data[seq[1] : seq[2], 0],
        z_g_data[seq[1] : seq[2], 1],
        z_g_data[seq[1] : seq[2], 2],
        linestyle="-",
        marker="o",
        c="green",
    )
...
if __name__ == "__main__":
    ...
    if opt.task != "motion_renavigating":
        ...
        data_loader = DataLoader(
            ...
            shuffle=False,
            pin_memory=True,
            drop_last=True if opt.task == "latent_interpolation" else False,
        )
    else:
