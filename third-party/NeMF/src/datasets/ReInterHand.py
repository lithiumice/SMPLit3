...
def dump_amass2pytroch(out_posepath, logger=None):
    ...
    pose = (
        torch.from_numpy(np.asarray(data_poses, np.float32))
        .to(device)
        .view(-1, clip_frames, 16, 3)
    )  # axis-angle (N, T, J, 3)
...
def collect_amass_stats(logger=None):
    ...
    for capture_id_dir in ReInterHand_data_root.iterdir():
        ...
        if args.hand_type == "L":
            json_path = sorted(
                [pth for pth in capture_id_dir.rglob("params/*_left.json")],
                key=lambda x: int(x.stem.split("_")[0]),
            )
        if args.hand_type == "R":
            json_path = sorted(
                [pth for pth in capture_id_dir.rglob("params/*_right.json")],
                key=lambda x: int(x.stem.split("_")[0]),
            )

        if len(json_path) == 0:
            continue
        pose = np.array([json.load(open(str(p), "r"))["pose"] for p in json_path])[
            :, :45
        ]
        fps = 30
...
if __name__ == "__main__":
    ...
    out_posepath = makepath(
        os.path.join(
            work_dir,
            f"pose-{args.hand_type}-{args.data.clip_length}-{args.data.fps}fps.pt",
        ),
        isfile=True,
    )
