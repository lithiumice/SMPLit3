...
def dump_animal2single(animal_data_dir, logger):
    ...
    for bvh_fname in tqdm(bvh_fnames):
        ...
        data_rotation = np.insert(
            data_rotation, [5, 9, 13, 16, 19, 21], [1, 0, 0, 0], axis=1
        )
        poses = torch.from_numpy(np.asarray(data_rotation, np.float32)).to(
            device
        )  # quaternion (T, J, 4)
        trans = torch.from_numpy(np.asarray(data_translation, np.float32)).to(
            device
        ...
        out_posepath = makepath(
            os.path.join(work_dir, "train", f"trans_{index}.pt"), isfile=True
        )
        ...
        if args.canonical:
            ...
            torch.save(
                forward_ang.detach().cpu(), out_posepath.replace("trans", "forward_ang")
            )
            torch.save(
            ...
            torch.save(
                root_orient.detach().cpu(), out_posepath.replace("trans", "root_orient")
            )  # (N, T, 3, 3)
