...
from nemf_rotations import matrix_to_axis_angle, rotation_6d_to_matrix
from nemf_utils import compute_trajectory, export_ply_trajectory, normalize
...
class Predictor(nn.Module):
    def __init__(self, args, topology):
        ...
        out_channels = (
            args.out_channels
        )  # root orient (6) + root vel (3) + root height (1) + contacts (8)
...
class GlobalMotionPredictor(BaseModel):
    def __init__(self, args, ngpu):
        ...
        smpl_fname = os.path.join(
            args.smpl.smpl_body_model, args.data.gender, "model.npz"
        )
    ...
    def predict(self, data, dt):
        ...
        if "velocity" in self.args.data.normalize:
            velocity = normalize(
                data["velocity"], mean=self.mean["velocity"], std=self.std["velocity"]
            )
        else:
            velocity = data["velocity"]
        if "rot6d" in self.args.data.normalize:
            rot6d = normalize(
                data["rot6d"], mean=self.mean["rot6d"], std=self.std["rot6d"]
            )
        else:
            rot6d = data["rot6d"]
        if "angular" in self.args.data.normalize:
            angular = normalize(
                data["angular"], mean=self.mean["angular"], std=self.std["angular"]
            )
        else:
        ...
        output["trans"] = compute_trajectory(
            output["root_vel"],
            output["root_height"],
            origin,
            dt,
            up_axis=self.args.data.up,
        )
    ...
    def backward(self, validation=False):
        ...
        v_loss = self.criterion_pred(
            self.pred_data["root_vel"], self.input_data["root_vel"]
        )
        up_loss = self.criterion_pred(
            self.pred_data["root_height"], self.input_data["trans"][..., self.up_axis]
        )
        ...
        trans_loss = self.criterion_pred(
            self.pred_data["trans"], self.input_data["trans"]
        )
        ...
        contacts_loss = self.criterion_bce(
            self.pred_data["contacts"], self.input_data["contacts"]
        )
    ...
    def report_errors(self):
        ...
        return {"translation": c2c(translation_error)}
    ...
    def save(self, optimal=False):
        ...
        predictor = (
            ...
            if isinstance(self.predictor, nn.DataParallel)
            else self.predictor
        )
    ...
    def load(self, epoch=None, optimal=False):
        ...
        predictor.load_state_dict(
            torch.load(os.path.join(path, "predictor.pth"), map_location=self.device)
        )
        if self.is_train:
    ...
    def compute_test_result(self):
        ...
        rotmat_gt = rotation_6d_to_matrix(
        ...
        for i in range(b_size):
            export_ply_trajectory(
                ...
                ply_fname=os.path.join(
                    self.smpl_dir, f"test_{self.test_index + i:03d}.ply"
                ),
            )
            export_ply_trajectory(
                points=trans_gt[i],
                color=(0, 255, 0),
                ply_fname=os.path.join(
                    self.smpl_dir, f"test_{self.test_index + i:03d}_gt.ply"
                ),
            )
            ...
            np.savez(
                os.path.join(self.smpl_dir, f"test_{self.test_index + i:03d}.npz"),
                poses=poses,
                trans=c2c(trans[i]),
                betas=np.zeros(10),
            ...
            np.savez(
                os.path.join(self.smpl_dir, f"test_{self.test_index + i:03d}_gt.npz"),
                poses=poses_gt,
