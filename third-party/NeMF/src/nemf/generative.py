...
MEAN_PATH = os.path.join(
    ...
    "../../../../model_files/nemf_body/mean-male-128-30fps.pt",
)
STD_PATH = os.path.join(
    os.path.dirname(__file__), "../../../../model_files/nemf_body/std-male-128-30fps.pt"
)
...
class Architecture(BaseModel):
    def __init__(self, args, ngpu, batch_num=0):
        ...
        self.args = args
        args.local_prior.use_group_norm = True
        ...
        edges = build_edge_topology(self.fk.parents.cpu().numpy())
        ...
        if args.multi_gpu is True:
            self.local_encoder = nn.DataParallel(
                self.local_encoder, device_ids=range(ngpu)
            )
            self.global_encoder = nn.DataParallel(
                self.global_encoder, device_ids=range(ngpu)
            )
        ...
        self.mean = torch.load(MEAN_PATH, map_location=self.device)
        self.std = torch.load(STD_PATH, map_location=self.device)

        if args.pretrained_gmp:
            assert (
                args.output_trans is False
            ), "already output global translation in this model"
        ...
        if self.is_train:
            parameters = list(self.local_encoder.parameters()) + list(
    ...
    def encode_global(self):
        if "root_orient" in self.args.data.normalize:
            root_orient = normalize(
                self.input_data["root_orient"],
                mean=self.mean["root_orient"],
                std=self.std["root_orient"],
            )
        else:
        ...
        if self.args.global_prior.in_channels == 9:
            if "root_vel" in self.args.data.normalize:
                root_vel = normalize(
                    self.input_data["root_vel"],
                    mean=self.mean["root_vel"],
                    std=self.std["root_vel"],
                )
            else:
    ...
    def forward(self, step=1):
        ...
        self.recon_data = self.decode(
            z_l, z_g, length=self.args.data.clip_length, step=step
        )
    ...
    def decode(self, z_l, z_g, length, step=1):
        ...
        if self.args.output_trans:
            ...
            if self.args.data.root_transform:
                root_orient_rotmat = rotation_6d_to_matrix(
                    output["root_orient"]
                )  # (B, T, 3, 3)
                local_rotmat[:, :, 0] = root_orient_rotmat
                pos = torch.matmul(
                    root_orient_rotmat.unsqueeze(2), pos_recon.unsqueeze(-1)
                ).squeeze(-1)
            else:
                pos = pos_recon
            ...
            gmp_data["angular"] = estimate_angular_velocity(
                local_rotmat.clone(), dt=dt
            )  # (B, T, J, 3)
    ...
    def kl_scheduler(self):
        ...
        """
        if (self.epoch_cnt % self.args.annealing_cycles == 0) and (
            self.iteration % self.batch_num == 0
        ):
            print("KL annealing restart")
            return 0.01

        return min(
            1, 0.01 + self.iteration / (self.args.annealing_warmup * self.batch_num)
        )
    ...
    def backward(self, validation=False):
        ...
        if self.args.output_trans:
            ...
            self.loss_recorder.add_scalar(
                "trans_loss", trans_loss, validation=validation
            )
            loss += self.args.lambda_trans * trans_loss

            contacts_loss = self.criterion_bce(
                self.recon_data["contacts"], self.input_data["contacts"]
            )
            self.loss_recorder.add_scalar(
                "contacts_loss", contacts_loss, validation=validation
            )
        ...
        if self.args.lambda_kl != 0:
            ...
            local_kl_loss = torch.mean(
                -0.5 * torch.sum(1 + logvar_l - mu_l.pow(2) - logvar_l.exp(), dim=-1)
            )
            self.loss_recorder.add_scalar(
                "local_kl_loss", local_kl_loss, validation=validation
            )

            mu_g = self.recon_data["mu_g"]
            logvar_g = self.recon_data["logvar_g"]
            global_kl_loss = torch.mean(
                -0.5 * torch.sum(1 + logvar_g - mu_g.pow(2) - logvar_g.exp(), dim=-1)
            )
            self.loss_recorder.add_scalar(
                "global_kl_loss", global_kl_loss, validation=validation
            )
            if not validation:
                loss += (
                    self.args.lambda_kl
                    * self.kl_scheduler()
                    * (local_kl_loss + global_kl_loss)
                )
    ...
    def save(self, optimal=False):
        ...
        local_encoder = (
            self.local_encoder.module
            if isinstance(self.local_encoder, nn.DataParallel)
            else self.local_encoder
        )
        nemf = (
        ...
        if self.args.data.root_transform:
            global_encoder = (
                self.global_encoder.module
                if isinstance(self.global_encoder, nn.DataParallel)
                else self.global_encoder
            )
            torch.save(
                global_encoder.state_dict(), os.path.join(path, "global_encoder.pth")
            )
    ...
    def load(self, epoch=None, optimal=False):
        self.args.save_dir = os.path.join(
            os.path.dirname(__file__), "../../../../model_files/nemf_body/generative"
        )
        ...
        local_encoder.load_state_dict(
            torch.load(
                os.path.join(path, "local_encoder.pth"), map_location=self.device
            )
        )
        ...
        if self.args.data.root_transform:
            ...
            global_encoder.load_state_dict(
                torch.load(
                    os.path.join(path, "global_encoder.pth"), map_location=self.device
                )
            )
        if self.is_train:
    ...
    def compute_test_result(self):
        ...
        local_rotmat_gt = local_rotmat_gt.view(
            b_size, -1, n_joints, 3, 3
        )  # (B, T, J, 3, 3)
        ...
        if "trans" in self.recon_data.keys():
            ...
            positions[..., self.v_axis] = positions[..., self.v_axis] + origin[
                ..., self.v_axis
            ].unsqueeze(1)
        ...
        for i in range(b_size):
            if self.args.bvh_viz:
                ...
                BVH.save(
                    os.path.join(
                        self.viz_dir,
                        f"test_{self.test_index + i:03d}_{int(self.fps)}fps.bvh",
                    ),
                    anim,
                ...
                BVH.save(
                    os.path.join(
                        self.viz_dir, f"test_{self.test_index + i:03d}_gt.bvh"
                    ),
                    anim,
                    names=self.args.smpl.joint_names,
                ...
                np.savez(
                    os.path.join(
                        self.viz_dir,
                        f"test_{self.test_index + i:03d}_{int(self.fps)}fps.npz",
                    ),
                    poses=poses,
                    trans=c2c(positions[i]),
                    betas=np.zeros(10),
                ...
                np.savez(
                    os.path.join(
                        self.viz_dir, f"test_{self.test_index + i:03d}_gt.npz"
                    ),
                    poses=poses_gt,
                    trans=c2c(positions_gt[i]),
                    betas=np.zeros(10),
                    gender=self.args.data.gender,
