...
def difftraj_entry(
    ...
    def pose_to_traj(npz_file, output_pth):
        ...
        if flip_x:
            cvt = a2m(torch.tensor([[rot_deg, 0, 0]])).float().to(pred_root_world.device)
            pred_root_world = m2a(cvt.mT @ a2m(pred_root_world))  

        if flip_y:
            cvt = a2m(torch.tensor([[0, rot_deg, 0]])).float().to(pred_root_world.device)
            pred_root_world = m2a(cvt.mT @ a2m(pred_root_world))  
...
"""
python3 /apdcephfs/private_wallyliang/SMPLit/difftraj_entry.py \
--inp /apdcephfs/private_wallyliang/SMPLit/ykcheng/beat2_our_rot.npz \
--flip_x --rot_deg -90

find /apdcephfs/private_wallyliang/MotionCaption/output -name *.npz -print -exec sh -c \
'python /apdcephfs/private_wallyliang/SMPLit/difftraj_entry.py --inp "$1" --flip_y' _ {} \;
"""