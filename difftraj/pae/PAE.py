...
# import model.PAE_Utility as utility
...
class PlottingWindow:
    def __init__(
        ...
        self.YMax = max
        self.YRange = [
            sys.float_info.max if min == None else min,
            sys.float_info.min if max == None else max,
        ]
    ...
    def Add(self, *args):  # arg->(value, label)
        for arg in args:
            ...
            function[1].append(
                sum(function[0][-self.CumulativeHorizon :])
                / len(function[0][-self.CumulativeHorizon :])
            )

            self.YRange[0] = (
                min(self.YRange[0], value) if self.YMin == None else self.YRange[0]
            )
            self.YRange[1] = (
                max(self.YRange[1], value) if self.YMax == None else self.YRange[1]
            )
    ...
    with open(file) as f:
        ...
        while f.readline():
            ...
            if len(pointers) % VERBOSE_STEP == 0:
                print(
                    "Collecting data pointers for " + file + " - " + str(len(pointers)),
                    end="\r",
                )
    ...
    with open(binaryFile, "rb") as f:
        for i in np.arange(sampleCount):
            if i % max(int(sampleCount / VERBOSE_STEP), 1) == 0:
                print(
                    "Reading binary " + binaryFile + "...",
                    round(100 * i / sampleCount, 2),
                    "%",
                    end="\r",
                )
...
def CreateMatrix(batch, t, r, device):
    ...
    rx = torch.stack(
        (r00, r10, r20, torch.zeros((batch), dtype=torch.float32, device=device)), 1
    )
    ry = torch.stack(
        (r01, r11, r21, torch.zeros((batch), dtype=torch.float32, device=device)), 1
    )
    rz = torch.stack(
        (r02, r12, r22, torch.zeros((batch), dtype=torch.float32, device=device)), 1
    )
...
def RainbowColor(index, count):
    ...
    return (
        Rescale(
            np.sin(frequency * index + 0.0) * (127.0) + 128.0, 0.0, 255.0, 0.0, 1.0
        ),
        Rescale(
            np.sin(frequency * index + 2.0) * (127.0) + 128.0, 0.0, 255.0, 0.0, 1.0
        ),
        Rescale(
            np.sin(frequency * index + 4.0) * (127.0) + 128.0, 0.0, 255.0, 0.0, 1.0
        ),
        1.0,
    )
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # Phase
        p = torch.empty(
            (y.shape[0], self.embedding_channels), dtype=torch.float32, device=y.device
        )
