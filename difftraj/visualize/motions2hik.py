...
def motions2hik(motions, device=0, cuda=True):
    ...
    for rep_idx in range(nreps):
        ...
        if nfeats == 3:
            print(
                f"Running SMPLify for repetition [{rep_idx + 1}] of {nreps}, it may take a few minutes."
            )
            motion_tensor, opt_dict = j2s.joint2smpl(
                rep_motions
            )  # [nframes, njoints, 3]
        ...
        thetas_6d = motion[0, :-1, :, :nframes].transpose(
            2, 0, 1
        )  # [nframes, njoints, 6]
        thetas_deg = []
        ...
        root_translation.append(
            [motion[0, -1, :3, :nframes].transpose(1, 0)]
        )  # [nframes, 3]
