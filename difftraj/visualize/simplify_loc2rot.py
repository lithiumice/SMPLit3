...
class joints2smpl:
    ...
    def npy2smpl(self, npy_path):
        ...
        for sample_i in tqdm(range(n_samples)):
            thetas, _ = self.joint2smpl(
                motions["motion"][sample_i].transpose(2, 0, 1)
            )  # [nframes, njoints, 3]
    ...
    def joint2smpl(self, input_joints, init_params=None):
        ...
        thetas = geometry.matrix_to_rotation_6d(
            geometry.axis_angle_to_matrix(thetas)
        )  # [bs, 24, 6]
        ...
        thetas = (
            torch.cat([thetas, root_loc], dim=1).unsqueeze(0).permute(0, 2, 3, 1)
        )  # [1, 25, 6, 196]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Blender file or dir with blender files",
    )
    ...
    if os.path.isfile(params.input_path) and params.input_path.endswith(".npy"):
        ...
        files = [
            os.path.join(params.input_path, f)
            for f in os.listdir(params.input_path)
            if f.endswith(".npy")
        ]
        for f in files:
            simplify.npy2smpl(f)
