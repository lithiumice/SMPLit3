...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        ######
        # beta_min = self.betas.min()
        # beta_max = self.betas.max()
        # eps_small = 1e-3
        ...
        # self.sigmas = (betas**0.5).cpu().numpy()
        # self.a_s = (torch.sqrt(1-betas)).cpu().numpy()
        # betas = get_named_beta_schedule(args.noise_schedule, steps+1, scale_beta)
        # import ipdb;ipdb.set_trace()
        self.sqrt_alphas_cumprod_ex = np.concatenate(
            [self.sqrt_alphas_cumprod, self.sqrt_alphas_cumprod[-1:]], axis=-1
        )
        self.sqrt_one_minus_alphas_cumprod_ex = np.concatenate(
            [
                self.sqrt_one_minus_alphas_cumprod,
                self.sqrt_one_minus_alphas_cumprod[-1:],
            ],
            axis=-1,
        )
        #######

    def masked_l2(self, a, b, mask, loss_weight=None):
    ...
    def q_sample_pairs(self, x_start, t):
        ...
        # x_t_plus_one = (
        #     _extract_into_tensor(self.a_s, t+1, x_start.shape) * x_start
        #     + _extract_into_tensor(self.sigmas, t+1, x_start.shape)
        #     * noise
        # )
        x_t_plus_one = (
            _extract_into_tensor(self.sqrt_alphas_cumprod_ex, t + 1, x_start.shape)
            * x_start
            + _extract_into_tensor(
                self.sqrt_one_minus_alphas_cumprod_ex, t + 1, x_start.shape
            )
            * noise
        )
    ...
    def sample_posterior(self, x_0, x_t, t):
        def q_posterior(x_0, x_t, t):
            ...
            log_var_clipped = _extract_into_tensor(
                self.posterior_log_variance_clipped, t, x_t.shape
            )
        ...
        def p_sample(x_0, x_t, t):
            ...
            return (
                mean
                + nonzero_mask[:, None, None, None] * torch.exp(0.5 * log_var) * noise
            )
    ...
    # DDGAN
    # <<====================
    def train_gan_d_losses(self, model, x_start, t, netD, model_kwargs=None):
        ...
        netD.zero_grad()
        noise = torch.randn_like(x_start)
        x_t, x_tp1 = self.q_sample_pairs(x_start, self._scale_timesteps(t))
        x_t.requires_grad = True
        # import ipdb;ipdb.set_trace()
        D_real = netD(
            x_t, self._scale_timesteps(t), x_tp1.detach(), **model_kwargs
        ).view(-1)
        ...
        if 1:
            # train with fake
            x_0_predict = model(
                x_tp1.detach(), self._scale_timesteps(t), **model_kwargs
            )  # here
            x_pos_sample = self.sample_posterior(
                x_0_predict, x_tp1, self._scale_timesteps(t)
            )
            D_fake = netD(
                x_pos_sample, self._scale_timesteps(t), x_tp1.detach(), **model_kwargs
            ).view(-1)
    ...
    def train_gan_g_losses(self, model, x_start, t, netD, model_kwargs=None):
        for p in netD.parameters():
            p.requires_grad = False
        for p in model.parameters():
            p.requires_grad = True
    ...
    #     x_t.requires_grad = True
    #     # import ipdb;ipdb.set_trace()
    #     D_real = netD(x_t, self._scale_timesteps(t), x_tp1.detach(), **model_kwargs).view(-1)
    #     errD_real = F.softplus(-D_real)
    #     errD_real = errD_real.mean()
    #     errD_real.backward(retain_graph=True)
    ...
    #     # train with fake
    #     x_0_predict = model(x_tp1.detach(), self._scale_timesteps(t), **model_kwargs)#here
    #     x_pos_sample = self.sample_posterior(x_0_predict, x_tp1, self._scale_timesteps(t))
    #     D_fake = netD(x_pos_sample, self._scale_timesteps(t), x_tp1.detach(), **model_kwargs).view(-1)
    #     errD_fake = F.softplus(D_fake)
    #     errD_fake = errD_fake.mean()
    #     errD_fake.backward()
    #     # errD = errD_real + errD_fake
    ...
    #     x_0_predict = model(x_tp1.detach(), self._scale_timesteps(t), **model_kwargs)#here
    #     x_pos_sample = self.sample_posterior(x_0_predict, x_tp1, self._scale_timesteps(t))
    #     output = netD(x_pos_sample, self._scale_timesteps(t), x_tp1.detach(), **model_kwargs).view(-1)
    #     errG = F.softplus(-output)
    #     errG = errG.mean()
    #     # errG.backward()
    ...
    # # SIDDMS
    # def train_siddms_gan_d_losses(self, model, x_start, t, netD, model_kwargs=None):
    #     for p in netD.parameters():
    #         p.requires_grad = True
    ...
    #     x_t, x_tp1 = self.q_sample_pairs(x_start, self._scale_timesteps(t))
    #     x_t.requires_grad = True
    #     D_real = netD(x_t, self._scale_timesteps(t), **model_kwargs).view(-1)
    #     errD_real = F.softplus(-D_real)
    #     errD_real = errD_real.mean()
    #     errD_real.backward(retain_graph=True)
    #     terms["errD_real"] = errD_real
    ...
    #     x_0_predict = model(x_tp1.detach(), self._scale_timesteps(t), **model_kwargs)#here
    #     x_pos_sample = self.sample_posterior(x_0_predict, x_tp1, self._scale_timesteps(t))
    #     D_fake = netD(x_pos_sample, self._scale_timesteps(t), **model_kwargs).view(-1)

    #     # import ipdb;ipdb.set_trace()
    #     # var = _extract_into_tensor(self.posterior_variance, t, x_t.shape) + 1e-5
    #     # afd_lambda = (1/var-1)
    #     # afd = (afd_lambda * (x_pos_sample - x_t)**2).mean()
    #     afd = 1*(100.0 * (x_pos_sample - x_t)**2).mean()
    #     terms["afd"] = afd
    ...
    # def train_siddms_gan_g_losses(self, model, x_start, t, netD, model_kwargs=None):
    #     for p in netD.parameters():
    #         p.requires_grad = False
    #     # model.zero_grad()
    ...
    #     x_0_predict = model(x_tp1.detach(), self._scale_timesteps(t), **model_kwargs)#here
    #     x_pos_sample = self.sample_posterior(x_0_predict, x_tp1, self._scale_timesteps(t))
    #     output = netD(x_pos_sample, self._scale_timesteps(t), **model_kwargs).view(-1)
    #     errG = F.softplus(-output)
    #     errG = errG.mean()
    #     # errG.backward()
    #     terms = {}
    ...
    def train_siddms_gan_d_losses(self, model, x_start, t, netD, model_kwargs=None):
        ...
        D_real = netD(x3, self._scale_timesteps(t), **model_kwargs).view(-1)
        errD_real = F.softplus(-D_real)
        errD_real = errD_real.mean()
        errD_real.backward(retain_graph=True)
        terms["errD_real"] = errD_real
        ...
        r1_gamma = 0.02
        grad_real = torch.autograd.grad(
            outputs=D_real.sum(), inputs=x3, create_graph=True
        )[0]
        grad_penalty = (
            grad_real.reshape(grad_real.size(0), -1).norm(2, dim=1) ** 2
        ).mean()
        ...
        x_0_predict = model(
            x_t.detach(), self._scale_timesteps(t), **model_kwargs
        )  # here
        D_fake = netD(x_0_predict, self._scale_timesteps(t), **model_kwargs).view(-1)
        errD_fake = F.softplus(D_fake)
        errD_fake = errD_fake.mean()
        terms["errD_fake"] = errD_fake
        errD_fake.backward()
    ...
    def train_siddms_gan_g_losses(self, model, x_start, t, netD, model_kwargs=None):
        ...
        output = netD(x_0_predict, self._scale_timesteps(t), **model_kwargs).view(-1)
        errG = F.softplus(-output)
        errG = errG.mean()
        terms = {}
        terms["errG"] = errG
    ...
    def training_losses(
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)  # here

            # if 0:
            #     s_idx=0
            #     local_traj = model_output[s_idx,:,0,:].permute(1,0)
            #     rec_trans, rec_orient = traj_local2global_heading(local_traj)

            #     import plotly.graph_objects as go
            #     import plotly.express as px
            #     import pandas as pd
            #     fig = go.Figure()
            #     fig.update_layout(scene=dict(xaxis_title='X', yaxis_title='Y', zaxis_title='Z', aspectmode='cube', xaxis=dict(showspikes=True), yaxis=dict(showspikes=True), zaxis=dict(showspikes=True)), title='3D Curve with Equal Axis Units and Start/End Markers')

            #     data=rec_trans.detach().cpu().numpy()
            #     x=data[:,0]
            #     y=data[:,1]
            #     z=data[:,2]
            #     c='darkblue'
            #     fig.add_trace(go.Scatter3d(x=x,y=y,z=z,marker=dict(size=2,color=z,colorscale='Viridis',),line=dict(color=c,width=2)))
            #     fig.add_trace(go.Scatter3d(x=[x[0], x[-1]], y=[y[0], y[-1]], z=[z[0], z[-1]], mode='markers', marker=dict(size=[10, 10], color=['red', 'blue'], symbol=['circle', 'cross'])))

            #     fig.write_image('model_output1.jpg')
            ...
            terms["rot_mse"] = self.masked_l2(
                target, model_output, mask, loss_weight=loss_weight
            )  # mean_flat(rot_mse)#here
            # terms["rot_mse"] = self.masked_l2(target, model_output, mask) # mean_flat(rot_mse)#here
