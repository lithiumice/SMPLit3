...
if __name__ == '__main__':
    ...
    load_con = joblib.load('/apdcephfs/share_1330077/wallyliang/jpkls/onehundred_styles_taming_glamr_dump_dataloader_pose_joints_train.jpkl')
    length_arr = load_con['length_arr']
    data_dict = load_con['data_dict']
    name_list = load_con['name_list']    
    load_con=np.load('/apdcephfs/share_1330077/wallyliang/jpkls/onehundred_styles_taming_glamr_dump_dataloader_pose_joints_train_mean_std.npz') 
    ...
    def load_idx(idx):   
        ...
        motoin_info_all = (motoin_info_all - motoin_info_all_list_mean) / motoin_info_all_list_std
        return motoin_info_all[:,-22*3:]
    
    def LoadBatches(train_indices):
        tmp = [utility.ToDevice(torch.from_numpy(load_idx(idx))) for idx in train_indices]
