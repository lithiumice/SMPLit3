...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        if not self.args.train_cmdm_base:
            ...
            self.c_past_motion_encoder = nn.Linear(203,self.latent_dim) # traj（11） + pose6d（21*3）
    ...
    def mdm_forward(self, x, timesteps, y=None, control=None):
        ...
        ctrl_traj = y['ctrl_traj'].to(x.device).permute(3,2,0,1)[:,0,:,:] # ctrl_traj: bs,T,1,10 -> T,bs,10
        ctrl_traj_emb = self.ctrl_traj_encoder(ctrl_traj) # T,bs,512
        xseq = torch.cat((past_motion_emb, ctrl_traj_emb, emb, x), axis=0) #[xx, bs, 512]
        # assert xseq.shape[0] == (20 + 1 + 15)
        # import ipdb;ipdb.set_trace()
        
        xseq = self.sequence_pos_encoder(xseq)  # [seqlen+1, bs, d]
        output = self.seqTransEncoder(xseq, control=control)[-self.inp_len:]  # , src_key_padding_mask=~maskseq)  # [seqlen, bs, d]
