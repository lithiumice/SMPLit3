    ...
    def __init__(self, init_scale=1.0):
        ...
        if 0:
            ...
            for i in range(len(layers_sizes) - 1):
                self.layers.append(dense(layers_sizes[i], layers_sizes[i + 1], init_scale))
...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
                 ...
                 arch='trans_enc', emb_trans_dec=False, clip_version=None, inp_len=200, **kargs):
        super().__init__()
        njoints = 203
        ...
        if self.use_gru_enc:
            self.pae_network = nn.GRU(input_channels, input_channels, num_layers=self.num_layers, batch_first=True)     
        ...
        if self.use_past_motion:
            self.past_motion_encoder = nn.Linear(203,self.latent_dim) # traj（11） + pose6d（21*3）
        ...
        if self.arch == 'trans_enc':
            ...
            if self.use_motion_clip:
                ...
                if 'preCompute' in self.args.in_type:
                    example_motoin_embedding = y['example_motoin'].to(x.device).squeeze(2).unsqueeze(0)
                else:
                    example_motoin = y['example_motoin'].to(x.device)[:,:,0,:,:].permute(0,2,1,3)
                    batch = {'x': example_motoin, 'y': torch.zeros(bs).long().to(x.device), 'mask': torch.ones(bs,60).bool().to(x.device), 'lengths': torch.ones(bs,60).bool().to(x.device).data.fill_(60)}
                    batch = self.motoin_clip_model(batch)
                    example_motoin_embedding = batch['z'].unsqueeze(0) # 1, bs, 512
                example_motoin_embedding = self.motion_clip_emb_fc(example_motoin_embedding)
            
            if self.use_pae_enc:
                # latent # bs, 5, 61
                example_motoin = y['example_motoin'].to(x.device)[:,:,0,:] # bs, joints*3, example_len, 
                rec_y, latent, signal, params = self.pae_network(example_motoin)
                example_motoin_embedding = self.pae2latent_fc(signal.reshape(-1,self.pae_latent_dim)).unsqueeze(0)
                            
            if self.use_gru_enc:
                example_motoin = y['example_motoin'].to(x.device).permute(3,2,0,1)[:,0,:,:] # [T,bs,x]
            ...
            output = self.seqTransEncoder(xseq)[-self.inp_len:]  # , src_key_padding_mask=~maskseq)  # [seqlen, bs, d]
