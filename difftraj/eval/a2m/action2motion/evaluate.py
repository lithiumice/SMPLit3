...
class A2MEvaluation:
    def __init__(self, device):
        ...
        self.gru_classifier_for_fid = load_classifier_for_fid(
            self.input_size_raw, self.num_classes, device
        ).eval()
        self.gru_classifier = load_classifier(
            self.input_size_raw, self.num_classes, device
        ).eval()
    ...
    def compute_features(self, model, motionloader):
        ...
        with torch.no_grad():
            for idx, batch in enumerate(motionloader):
                activations.append(
                    self.gru_classifier_for_fid(
                        batch["output_xyz"], lengths=batch["lengths"]
                    )
                )
        ...
        for key, loader in loaders.items():
            ...
            if model.cond_mode != "no_cond":
                metrics[mkey], _ = calculate_accuracy(
                    model, loader, self.num_classes, self.gru_classifier, self.device
                )
            else:
                metrics[mkey] = np.nan
            ...
            ret = calculate_diversity_multimodality(
                ...
                labels,
                self.num_classes,
                unconstrained=(model.cond_mode == "no_cond"),
            )
