...
class StyleDecoder(nn.Module):
    def __init__(self, input_size, hidden_size, style_embedding_size, time_length = 60):
        super(StyleDecoder, self).__init__()

        self.style_embedding_size = style_embedding_size

        self.projection_layer = LinearNorm(
            style_embedding_size, hidden_size * 2, w_init_gain="linear"
        )
    ...
    def forward(self, style_embedding):
        ...
        tmp = style_embedding.tile(-1,time_length)
        expanded_embedding = self.projection_layer(style_embedding).unsqueeze(1)
        decoded_output = self.convs(output)
...
# class MotionRecurrentDecoderNormal(nn.Module):
#     def __init__(self, latent_dim = 512, hidden_size = 256, output_size = 203, num_rnn_layers = 5):
#         super().__init__()
#         self.layer0 = nn.Linear(latent_dim, hidden_size)
#         self.layer1 = nn.GRU(hidden_size + latent_dim, hidden_size, num_rnn_layers, batch_first = True)
#         self.layer2 = nn.Linear(hidden_size, output_size)
        
#     def forward(self, emb, cell_state):
#         hidden = F.elu(self.layer0(emb))
#         cell_output, cell_state = self.layer1(torch.cat([hidden, emb], dim = -1).unsqueeze(1), cell_state)
#         output = self.layer2(cell_output.squeeze(1))
#         return output

# class MotionCellStateEncoder(nn.Module):
#     def __init__(self, latent_dim = 512, hidden_size = 256, num_rnn_layers = 5):
#         super().__init__()
...
#     def __init__(self, latent_dim = 512, hidden_size = 256, output_size = 203, num_rnn_layers = 5):
#         super().__init__()
#         self.recurrent_decoder = MotionRecurrentDecoderNormal(latent_dim = latent_dim, hidden_size = hidden_size, output_size = output_size, num_rnn_layers = num_rnn_layers)
#         self.cell_state_encoder = MotionCellStateEncoder(latent_dim = latent_dim, hidden_size = hidden_size, num_rnn_layers = num_rnn_layers)
        
#     def forward(self, motion_emb):
#         ret = []
#         nframe = motion_emb.shape[0]
        
#         decoder_state = self.cell_state_encoder(motion_emb[:,0])
        
#         for i in range(nframe):
#             predicted, decoder_state = self.recurrent_decoder(motion_emb[:,i], decoder_state)
