...
# 定义三元组损失
def triplet_loss(anchor_latent, positive_latent, negative_latent, margin):
    pos_distance = torch.sum((anchor_latent - positive_latent) ** 2, dim=1)
    neg_distance = torch.sum((anchor_latent - negative_latent) ** 2, dim=1)
    loss = torch.mean(torch.relu(pos_distance - neg_distance + margin))
    return loss

# @torch.jit.script
def get_triplets(motions, style_codes):
    num_samples = len(style_codes)
    anchor_indices = []
    # anchor_indices = torch.arange(num_samples)
    positive_indices = []
    negative_indices = []

    for i in range(num_samples):
        anchor_style = style_codes[i:i+1]
        positive_candidates = torch.nonzero(torch.all(style_codes == anchor_style, dim=1), as_tuple=True)[0]
        negative_candidates = torch.nonzero(torch.any(style_codes != anchor_style, dim=1), as_tuple=True)[0]
        positive_candidates = positive_candidates[positive_candidates != i]
        
        if positive_candidates.size(0) > 0 and negative_candidates.size(0) > 0:
            positive_index = random.choice(positive_candidates.tolist())
            negative_index = random.choice(negative_candidates.tolist())

            positive_indices.append(positive_index)
            negative_indices.append(negative_index)
            anchor_indices.append(i)

    anchors = motions[anchor_indices]
    positives = motions[positive_indices]
    negatives = motions[negative_indices]
...
class DecompTrainerDisc(object):
    def __init__(self, args, movement_enc, movement_dec):
        ...
        if args.is_train:
            ...
            self.contrastive_loss = ContrastiveLoss(10)
    ...
    def forward(self, batch_data):
        motions, style_codes = batch_data
        self.motions = motions.detach().to(self.device).float()
        self.style_codes = style_codes.detach().to(self.device).float()
        
        if self.opt.ae_type == 'aeTF':
            # self.motions # (bs, 60, 25, 6)
            self.motions = self.motions.permute(0,2,3,1).cuda() # bs, 25, 6, 60
            bs = self.motions.size(0)
            batch = {
                'x': self.motions, 
                'y': torch.zeros(bs).long().to(self.device), 
                'mask': torch.ones(bs,60).bool().to(self.device),
                'lengths': torch.ones(bs,60).bool().to(self.device).data.fill_(60)
            }
            # import ipdb;ipdb.set_trace() 
            batch.update(self.movement_enc(batch))
            batch.update(self.movement_dec(batch))
            self.latents = batch["z"]
            self.mu = batch['mu']
            self.logvar = batch['logvar']
            self.disc_pred = None
            self.recon_motions = batch['output']
        elif self.opt.ae_type == 'aeTFUniRep':
            # self.motions # (bs, 60, 203)
            # import ipdb;ipdb.set_trace() 
            tmp = self.motions.unsqueeze(-1).permute(0,2,3,1).cuda() # bs, 203, 1, 60
            bs = self.motions.size(0)
            batch = {
                'x': tmp, 
                'y': torch.zeros(bs).long().to(self.device), 
            ...
            self.logvar = batch['logvar']
            self.disc_pred = None
            self.recon_motions = batch['output'].squeeze().permute(0,2,1)
        elif self.opt.ae_type == 'actorVAE':
            features = self.motions
            ...
            self.dist = dist
            self.disc_pred = None
            self.recon_motions = feats_rst           
        else:
            if self.use_triplet_loss:
                anchors, positives, negatives, self.anchor_indices, self.positive_indices, self.negative_indices = get_triplets(self.motions, self.style_codes)
                self.anchor_latents, self.anchors_mu, self.anchors_logvar, self.anchors_disc_pred = self.movement_enc(anchors)
                self.positives_latents, self.positives_mu, self.positives_logvar, self.positives_disc_pred = self.movement_enc(positives)
                self.negatives_latents, self.negatives_mu, self.negatives_logvar, self.negatives_disc_pred = self.movement_enc(negatives)
                self.recon_motions = self.movement_dec(self.anchor_latents)
                self.disc_pred = self.anchors_disc_pred
                self.style_codes = self.style_codes[self.anchor_indices]
                self.motions = self.motions[self.anchor_indices]
                self.logvar = self.anchors_logvar
                self.mu = self.anchors_mu
            else:
                self.latents, self.mu, self.logvar, self.disc_pred = self.movement_enc(self.motions)
                self.recon_motions = self.movement_dec(self.latents)

    def encode(self,features,lengths):
        dist = self.movement_enc(features, lengths)
        if self.opt.use_vae:
            from networks.actor_vae import sample_from_distribution
    ...
    def decode(self, z, lengths):
        feats = self.movement_dec(z, lengths)
        return feats
    ...
    def backward(self):
        ...
        loss_w = dict(
            loss_rec = 100 if self.opt.use_vae else 1,
            loss_sparsity = 0,
            # loss_sparsity = 0 if self.opt.use_vae else self.opt.lambda_sparsity,
            loss_triplet = 0.01,
            loss_style_pred = 1,
            kld = 0.001 if self.epoch<10 else min(1.0, (self.epoch - 10) * 0.05)
        )

        losses_raw = {}
        losses_raw['loss_rec'] = self.l1_criterion(self.recon_motions, self.motions)
        
        # losses_raw['loss_sparsity'] = torch.mean(torch.abs(self.latents))
        
        if self.opt.pred_style: losses_raw['loss_style_pred'] = torch.nn.CrossEntropyLoss()(self.disc_pred, self.style_codes)
            
        if self.use_triplet_loss: losses_raw['loss_triplet'] = triplet_loss(self.anchor_latents, self.positives_latents, self.negatives_latents, margin=1.0)

        if self.opt.use_vae:
            if self.opt.ae_type == 'actorVAE':
                q_z = self.dist
                p_z = td.normal.Normal(torch.zeros_like(q_z.loc), torch.ones_like(q_z.scale))
                losses_raw['kld'] = td.kl_divergence(q_z, p_z).sum()
            else:
                losses_raw['kld'] = -0.5 * torch.sum(1 + self.logvar - self.mu.pow(2) - self.logvar.exp())

        loss = 0
        for k, v in losses_raw.items(): loss += v * loss_w[k]
        self.loss = loss
        
        self.log_loss = {k: v.item() for k,v in losses_raw.items()}
    ...
    def train(self, train_dataloader, val_dataloader, plot_eval):
        ...
        while epoch < self.opt.max_epoch:
            ...
            for i, batch_data in enumerate(train_dataloader):
                ...
                for k, v in self.log_loss.items():
                    if k not in logs:
                        logs[k] = v
                    else:
                        logs[k] += v
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(val_dataloader):
                    ...
                    self.backward()
                    for k, v in self.log_loss.items():
                        if k not in val_loss_accum: val_loss_accum[k] = 0
                        else: val_loss_accum[k] += v
            val_len = (len(val_dataloader) + 1)
            val_loss_accum = {k: v/val_len for k,v in val_loss_accum.items()}
            log_str = ','.join([f'{k}: {v:.5f}' for k,v in val_loss_accum.items()])
            print(log_str)

            if epoch % self.opt.eval_every_e == 0:
                data = self.recon_motions.detach().cpu().numpy()
...
class DecompTrainerV3(object):
    ...
    def backward(self):
        ...
        # import ipdb;ipdb.set_trace() 
        self.loss_smooth = torch.zeros_like(self.loss_sparsity)
