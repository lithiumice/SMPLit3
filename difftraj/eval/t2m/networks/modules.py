...
# <======================================
class MovementConvEncoder2(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        super(MovementConvEncoder2, self).__init__()
        self.main = nn.Sequential(
            nn.Conv1d(input_size, hidden_size, 4, 2, 1),
            nn.Dropout(0.2, inplace=True),
            nn.LeakyReLU(0.2, inplace=True),
        ...
        self.out_net = nn.Sequential(
            nn.Linear(output_size*6, output_size*2),  
            nn.BatchNorm1d(output_size*2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(output_size*2, 512),
        )
...
class MovementConvDecoder2(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        super(MovementConvDecoder2, self).__init__()
        
        self.in_net = nn.Sequential(
            nn.Linear(512,input_size*2),
            nn.BatchNorm1d(input_size*2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(input_size*2,input_size*6),
        )
        self.in_net.apply(init_weight)
    ...
    def forward(self, inputs):
        # import ipdb;ipdb.set_trace()
        inputs = self.in_net(inputs)
        inputs = inputs.view(-1,6,self.input_size)
        inputs = inputs.permute(0, 2, 1)
        outputs = self.main(inputs).permute(0, 2, 1)
        return self.out_net(outputs)
...
class StylePredictor(nn.Module):
    def __init__(self, latent_dim = 512, classes_num = 100):
        super().__init__()
    ...
    def forward(self, z):
        disc_pred = self.disc_net(z)
        # disc_pred = self.disc_net(z.detach())
        # disc_pred = torch.sigmoid(disc_pred)
        return disc_pred
    
# inception for pose
class StylePredictorFromRawPose(nn.Module):
    def __init__(self, opt, input_size, hidden_size, output_size, latent_dim = 512, classes_num = 100):
        super().__init__()
        ...
        self.layer2 = nn.Sequential(
            nn.Linear(output_size*(self.opt.window_size//4), output_size*2),  
            nn.BatchNorm1d(output_size*2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(output_size*2, latent_dim),
        )
        
        self.layer3 = nn.Sequential(
            nn.Linear(latent_dim, classes_num),
        )
        self.layer1.apply(init_weight)
        self.layer2.apply(init_weight)
        self.layer3.apply(init_weight)
        
    def forward(self, inputs):
        inputs = inputs.permute(0, 2, 1)
        outputs = self.layer1(inputs)
        outputs = outputs.flatten(1)
        latent = self.layer2(outputs)
        disc_pred = self.layer3(latent)
        return latent, disc_pred
    
    
# for windows size 24
class MovementConvEncoder3(nn.Module):
    def __init__(self, opt, input_size, hidden_size, output_size, latent_dim = 512):
        super(MovementConvEncoder3, self).__init__()
        self.opt = opt
        self.main = nn.Sequential(
            nn.Conv1d(input_size, hidden_size, 4, 2, 1),
            nn.Dropout(0.2, inplace=True),
        ...
        self.style_predictor = StylePredictor(latent_dim = latent_dim)
    ...
    def forward(self, inputs,):
        ...
        disc_pred = self.style_predictor(z)
        
        return z, mu, logvar, disc_pred

# for windows size 24
class MovementConvDecoder3(nn.Module):
    def __init__(self, opt, input_size, hidden_size, output_size, latent_dim = 512):
        super(MovementConvDecoder3, self).__init__()
        self.opt = opt
        ...
        self.in_net = nn.Sequential(
            ...
            nn.BatchNorm1d(input_size*2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(input_size*2,input_size*(self.opt.window_size//4)),
        )
        self.in_net.apply(init_weight)
    ...
    def forward(self, inputs):
        # import ipdb;ipdb.set_trace()
        inputs = self.in_net(inputs)
        inputs = inputs.view(-1,(self.opt.window_size//4),self.input_size)
        inputs = inputs.permute(0, 2, 1)
        outputs = self.main(inputs).permute(0, 2, 1)
        return self.out_net(outputs)
    
# for all win size
class MovementConvEncoderLarge(nn.Module):
    def __init__(self, opt, input_size, hidden_size, output_size, latent_dim=512):
        super(MovementConvEncoderLarge, self).__init__()
        self.opt = opt
        self.down_rate = 4
        num_layer = int(math.log2(self.down_rate))
        layers = []
        for i in range(num_layer):
            layers.extend(
                [  # Lout = (Lin + 2*1 - 1*(4-1) - 1)/2 + 1 = (Lin-2)/2+1 = Lin/2
                    nn.Conv1d(input_size if i == 0 else hidden_size, hidden_size, 4, 2, 1),
                    nn.Dropout(0.2, inplace=True),
                    nn.LeakyReLU(0.2, inplace=True),
                ]
            )
        ...
        self.layer2 = nn.Sequential(
            nn.Linear(output_size * (self.opt.window_size // self.down_rate), output_size * 2),
            nn.BatchNorm1d(output_size * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(output_size * 2, latent_dim),
        ...
        # self.layer1.apply(init_weight)
        # self.layer2.apply(init_weight)
        # self.fc_mu.apply(init_weight)
        # self.fc_logvar.apply(init_weight)
        # self.style_predictor.apply(init_weight)
...
class MovementConvDecoderLarge(nn.Module):
    def __init__(self, opt, input_size, hidden_size, output_size, latent_dim=512):
        super(MovementConvDecoderLarge, self).__init__()
        self.down_rate = 4
        self.input_size = input_size
        self.opt = opt
        ...
        self.layer1 = nn.Sequential(
            ...
            nn.BatchNorm1d(input_size * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(input_size * 2, input_size * (self.opt.window_size // self.down_rate)),
        )
        ...
        for i in range(num_layer):
            layers.extend(
                [
                    nn.ConvTranspose1d(input_size if i == 0 else hidden_size, output_size if i==num_layer-1 else hidden_size, 4, 2, 1),
                    nn.LeakyReLU(0.2, inplace=True),
                ]
            )
    ...
    def forward(self, x):
        # import ipdb;ipdb.set_trace()
        x = self.layer1(x)
        x = x.view(-1, self.opt.window_size // self.down_rate, self.input_size)
        x = x.permute(0, 2, 1)
        x = self.layer2(x)
        x = x.permute(0, 2, 1)
        x = self.layer3(x)
