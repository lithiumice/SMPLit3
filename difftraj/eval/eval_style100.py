...
from data_loaders.humanml.motion_loaders.comp_v6_model_dataset import TamingGeneratedDataset
...
def get_taming_loader(model, diffusion, batch_size, ground_truth_loader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale, args):
    dataset = TamingGeneratedDataset(model, diffusion, ground_truth_loader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale, args)
    motion_loader = DataLoader(dataset, batch_size=batch_size, collate_fn=default_collate, drop_last=True, num_workers=0)
...
def get_eval_wrapper():
    ...
    if use_standalone_style_pred:
        ...
        style_predictor.load_state_dict(torch.load('/root/apdcephfs/private_wallyliang/PLANT/text-to-motion/checkpoints/style100/style_predictor_from_motion.pt',map_location=device)['style_predictor'])
...
def evaluation(eval_wrapper, gt_loader, eval_motion_loaders, log_file, replication_times, diversity_times, mm_num_times, run_mm=False):
    ...
    global ae_mean
    global persist_gt_emb_path
    gen_mean = torch.from_numpy(gt_loader.dataset.t2m_dataset.motoin_info_all_list_mean).cpu()
    gen_std =  torch.from_numpy(gt_loader.dataset.t2m_dataset.motoin_info_all_list_std).cpu() 
    if ae_mean is None: ae_mean = gen_mean
    if ae_std is None: ae_std = gen_std
        
    with open(log_file, 'w') as f:
        metric_template = OrderedDict({'fid': OrderedDict({}),
                                'accel': OrderedDict({}),
                                'fs': OrderedDict({}),
                                't_err': OrderedDict({}),
                                'o_err': OrderedDict({}),
        ...
        for replication in range(replication_times):
            ...
            for motion_loader_name, motion_loader in motion_loaders.items():
                ...
                with torch.no_grad():
                    for idx, batch in tqdm(enumerate(motion_loader)):                    
                        ...
                        if use_standalone_style_pred:
                            motion_embeddings, pred_style_prob = style_predictor(ae_motions)
                            # import ipdb;ipdb.set_trace()
                            ...
                            acc_radio = torch.nonzero(same_indices).size(0) / torch.numel(same_indices)
                            acc_radio = torch.tensor([acc_radio])
                        else:                    
                            motion_embeddings,_,_,pred_style_prob = eval_wrapper.movement_encoder(ae_motions)
                        
                        if eval_joints_metrics:                  
                            pred_joint = torch.stack([traj2joints.traj_to_joints(denormed_traj_motion, remove_float = True) for denormed_traj_motion in denorm_target_motion])
                            pred_joint = pred_joint[:,:,:22,:].cpu()
                            
                        
                        for k in save_keys:
                            if not k in all_infos.keys(): all_infos[k] = []
            ...
            if use_standalone_style_pred:                
                ...
                for model_name, model_infos in all_infos_dict.items():
                    all_eval_dict['acc_radio'][model_name] = model_infos['acc_radio'].mean().item()
            ...
            if eval_joints_metrics:                   
                ...
                for model_name, model_infos in all_infos_dict.items():
                    ...
                    orient_err = (vald_traj[:,:,3:9] - gt_traj[:,:,3:9]).norm(dim=-1)
            ...
            print(f'==================== Replication {replication} ====================')
            print(tabulate(pd.DataFrame(all_eval_dict), headers='keys', tablefmt='grid'))
...
if __name__ == '__main__':
    ...
    if args.guidance_param != 1.: log_file += f'_gscale{args.guidance_param}'
    ...
    for k, v in opt.items(): setattr(args, k, v)
    gt_loader = get_dataset_loader(name=args.dataset, batch_size=args.batch_size, num_frames=None, split=split, hml_mode='gt',args=args)
    gen_loader = get_dataset_loader(name=args.dataset, batch_size=args.batch_size, num_frames=None, split=split, hml_mode='eval',args=args)
    print(args.in_type)
    ...
    if args.eval_cmdm_base:
        ...
        from model.mdm_taming_style import MDM
    model = MDM(**get_model_args(args, gen_loader),inp_len=gen_loader.dataset.t2m_dataset.inp_len)
    ...
    eval_motion_loaders = {
        'vald': lambda: get_taming_loader(model, diffusion, args.batch_size, gen_loader, mm_num_samples, 
                                    mm_num_repeats, gt_loader.dataset.opt.max_motion_length, num_samples_limit, args.guidance_param, args)
    }
    mean_dict, conf_dict = evaluation(eval_wrapper, gt_loader, eval_motion_loaders, log_file, replication_times, diversity_times, mm_num_times, run_mm=run_mm)
