...
import sys;sys.path.insert(0,'/apdcephfs/private_wallyliang/PLANT')
...
class TamingGeneratedDataset(Dataset):

    def __init__(self, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale=1., args = None):
        self.dataloader = dataloader
        self.dataset = dataloader.dataset
        assert mm_num_samples < len(dataloader.dataset)
        use_ddim = False  # FIXME - hardcoded
        # if args.diffusion_steps>200: use_ddim = True; print('use ddim')
        clip_denoised = False  # FIXME - hardcoded
        self.max_motion_length = max_motion_length
        ...
        gen_target_T = args.window_size
        I = torch.ones(1).cuda()
        inp_len = dataloader.dataset.t2m_dataset.inp_len
        pastMotion_len = dataloader.dataset.t2m_dataset.pastMotion_len
        # if hasattr(model, 'model'): model.model.inp_len = inp_len

        with torch.no_grad():
            for i, (motion, model_kwargs) in tqdm(enumerate(dataloader)):
                target_transl_traj = model_kwargs['y']['ctrl_traj'].clone().squeeze(2).permute(0,2,1)
                # import ipdb;ipdb.set_trace()
                target_transl_traj = torch.cat([target_transl_traj, target_transl_traj[:,-1:].repeat(1,15,1)], dim=1)
                if target_transl_traj.shape[1] < gen_target_T:
                    continue
                
                if num_samples_limit is not None and len(generated_motion) >= num_samples_limit:
                    break

                if scale != 1.: model_kwargs['y']['scale'] = torch.ones(motion.shape[0],device=dist_util.dev()) * scale
                ...
                for t in range(repeat_times):
                    # import ipdb;ipdb.set_trace()
                    ar_model_output_list = model_kwargs['y']['past_motion'].type_as(I)[:,:,:,:pastMotion_len]
                    # ar_model_output_list = torch.cat([ar_model_output_list, ar_model_output_list[:,:,:,-infer_step:]], dim=-1) #[1, 203, 1, T]
                    for time in range(0, gen_target_T, infer_step):            
                        print(f'time: {time}')
                        ctrl_traj = target_transl_traj[:,time:time+inp_len]
                        if ctrl_traj.shape[1]!=inp_len: print(f'ctrl_traj.shape: {ctrl_traj.shape}');break      
                        # if 'normCtrlTraj' in args.in_type: ctrl_traj = (ctrl_traj - torch.from_numpy(dataloader.dataset.t2m_dataset.ctrl_signal_mean)[None,None,...].type_as(ctrl_traj))/torch.from_numpy(dataloader.dataset.t2m_dataset.ctrl_signal_std)[None,None,...].type_as(ctrl_traj)
                        model_kwargs['y']['ctrl_traj'] = ctrl_traj.permute(0,2,1).unsqueeze(2)      
                        model_kwargs['y']['past_motion'] = ar_model_output_list[:,:,:,-pastMotion_len:]                
                        # import ipdb;ipdb.set_trace()
                        sample = sample_fn(
                            model,
                            (dataloader.batch_size, model.njoints, model.nfeats, inp_len),
                            clip_denoised=False,
                            model_kwargs=model_kwargs,
                            skip_timesteps=0,
                            init_image=None,
                        ...
                        # import ipdb;ipdb.set_trace()
                        if time < gen_target_T - inp_len:
                            ar_model_output_list = torch.cat([ar_model_output_list, sample[:,:,:,:infer_step]], dim=-1) #[1, 203, 1, T]
                        else:
                            ar_model_output_list = torch.cat([ar_model_output_list, sample[:,:,:,:]], dim=-1) #[1, 203, 1, T]
                            print(f'quit loop at time: {time}, gen_target_T: {gen_target_T}, inp_len: {inp_len}')
                            break
                            
                    motion = ar_model_output_list[:,:,:,-gen_target_T:].squeeze().permute(0,2,1)
                    # import ipdb;ipdb.set_trace()
                    
                    if t == 0:
                        sub_dicts = [{'motion': motion[bs_i].cpu().numpy(),'length': gen_target_T,} for bs_i in range(dataloader.batch_size)]
                        generated_motion += sub_dicts
