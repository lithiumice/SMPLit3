...
class TamingGeneratedDataset(Dataset):
    ...
    def __init__(
        ...
        with torch.no_grad():
            for i, (motion, model_kwargs) in tqdm(enumerate(dataloader)):
                target_transl_traj = (
                    model_kwargs["y"]["ctrl_traj"].clone().squeeze(2).permute(0, 2, 1)
                )
                # import ipdb;ipdb.set_trace()
                target_transl_traj = torch.cat(
                    [target_transl_traj, target_transl_traj[:, -1:].repeat(1, 15, 1)],
                    dim=1,
                )
                ...
                for t in range(repeat_times):
                    ...
                    for time in range(0, gen_target_T, infer_step):
                        ...
                        if ctrl_traj.shape[1] != inp_len:
                            print(f"ctrl_traj.shape: {ctrl_traj.shape}")
                            break
                        ...
                        model_kwargs["y"]["ctrl_traj"] = ctrl_traj.permute(
                            0, 2, 1
                        ).unsqueeze(2)
                        ...
                        if time < gen_target_T - inp_len:
                            ar_model_output_list = torch.cat(
                                [ar_model_output_list, sample[:, :, :, :infer_step]],
                                dim=-1,
                            )  # [1, 203, 1, T]
                        else:
                            ar_model_output_list = torch.cat(
                                [ar_model_output_list, sample[:, :, :, :]], dim=-1
                            )  # [1, 203, 1, T]
                            print(
                                f"quit loop at time: {time}, gen_target_T: {gen_target_T}, inp_len: {inp_len}"
                            )
                            break

                    motion = (
                        ar_model_output_list[:, :, :, -gen_target_T:]
                        .squeeze()
                        .permute(0, 2, 1)
                    )
                    # import ipdb;ipdb.set_trace()
