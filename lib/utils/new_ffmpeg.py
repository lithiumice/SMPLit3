...
def ffmpeg_merge_api(
    ...
    for row_idx in range(row_length):
        prefix = "".join(
            [f"[{info.v_idx}:v]" for info in todo_list if info.row_idx == row_idx]
        )
    ...
    if row_length > 1:
        ...
        total_cmds += [f"{vstack_list}vstack=inputs={row_length}[{output_symbol}]"]
    else:
    ...
    if 1:
        total_cmds += ["-hide_banner", f"-loglevel {loglevel}"]
...
def temporal_concat_video(
    ...
    for path in input_path_list:
        check_input_path(
            path, allowed_suffix=[".gif", ".mp4"], tag="input video", path_type="file"
        )
    ...
    for index, vid_file in enumerate(input_path_list):
        ...
        if resolution is not None:
            ...
            scale_command.append(
                ...
                % (index, input_title_list[index], font_desc, width, height, index)
            )
        else:
            scale_command.append(
    ...
    command += [
        ...
        % (scale_command, concat_command, len(input_path_list)),
        "-loglevel",
        "error",
        "-map",
        "[v]",
