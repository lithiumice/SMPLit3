...
class deeplab_seg(object):
    ...
    def __init__(self):
        ...
        self.deeplab_model = torch.hub.load(
    ...
    def predict_batch(self, img_folder, savefolder, saveVis=True, **kwargs):
        ...
        self.imagepath_list = glob_exts_in_path(
            img_folder, img_ext=["png", "jpg", "jpeg"]
        )
        ...
        for imagepath in tqdm.tqdm(self.imagepath_list):
            ...
            self.predict(
                im_path=imagepath, vis_path=vis_path if saveVis else None, **kwargs
            )
    ...
    def save_org_mask(
        self, person_seg_org, vis_path, sample_img, color=(0, 0, 255), opacity=0.5
    ):
        ...
        color_seg = np.zeros(
            (person_seg.shape[0], person_seg.shape[1], 3), dtype=np.uint8
        )
        ...
        sample_img[person_seg[:, :, 0] == 1, :] = sample_img[
            person_seg[:, :, 0] == 1, :
        ] * (1 - opacity)
    ...
    def deeplab_video(self, sample_img_list, predict_size):
        img_batch = torch.zeros(
            (len(sample_img_list), 3, predict_size[1], predict_size[0])
        )
