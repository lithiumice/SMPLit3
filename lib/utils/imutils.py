...
def cam_full2pred(full_cam, center, scale, full_img_shape, focal_length=2000.0):
    # img_h, img_w = full_img_shape[:, 0], full_img_shape[:, 1]
    img_w, img_h = full_img_shape[:, 0], full_img_shape[:, 1]
    cx, cy, b = center[:, 0], center[:, 1], scale * 200
    w_2, h_2 = img_w / 2.0, img_h / 2.0
...
def obtain_camera_intrinsics(image_shape, focal_length):
    ...
    K = (
        torch.eye(3)
        .unsqueeze(0)
        .expand(focal_length.shape[0], -1, -1)
        .to(focal_length.device)
    )
...
def compute_cam_intrinsics(res):
    ...
    focal_length = (img_w * img_w + img_h * img_h) ** 0.5
    # focal_length = 5000.0
    # print(f'hard coded focal length: {focal_length}')
...
def flip_pose(rotation, representation="rotation_6d"):
    ...
    if representation == "axis_angle":
        ...
        pose = (
            transforms.matrix_to_axis_angle(transforms.rotation_6d_to_matrix(rotation))
            .reshape(BN, -1)
            .transpose(0, 1)
        )
    else:
    ...
    if representation == "aa":
        ...
        return transforms.matrix_to_rotation_6d(transforms.axis_angle_to_matrix(pose))


def avg_preds(
    rotation, shape, flipped_rotation, flipped_shape, representation="rotation_6d"
):
    # Rotation
    flipped_rotation = flip_pose(flipped_rotation, representation=representation)

    if representation != "matrix":
        flipped_rotation = eval(f"transforms.{representation}_to_matrix")(
            flipped_rotation
        )
