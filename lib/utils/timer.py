...
def timeit(func):
    def _warp(*args, **kwargs):
        ...
        print(
            "The execution time of the function '%s'  is %.6fs\n"
...
class IterTimer:
    ...
    def print_time(self):
        if not self.enabled:
            return
        logger.info(
            "Average {} = {:.4f} seconds".format(self.name, np.average(self.times))
        )
        logger.info("\n")
...
class IterTimers(dict):
    ...
    def add_timer(self, name="time", sync=True, enabled=True):
        self.__dict__[name] = IterTimer(name, sync=sync, enabled=enabled)
