...
def video():
    for actor in tqdm(filter(os.path.isdir, glob(f"./test_results/*_rgb_*"))):
        os.system(
            f"ffmpeg -y -framerate 30 -pattern_type glob -i '{actor}/video/*.jpg' -c:v libx264 {actor}.mp4"
        )
...
@logger.catch
def frame_to_video(image_path, video_name, fps=9, cut=-1, size=(960, 540)):
    # fps=15,
    # size = (960,540)
    # size = (480,720)
...
if __name__ == "__main__":
    in_dir = sys.argv[1]
    vid_name = sys.argv[2]
    frame_to_video(in_dir, video_name="/tmp/tmp.mp4", fps=30, size=-1)
