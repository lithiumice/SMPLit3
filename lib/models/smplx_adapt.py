...
class SMPLX(_SMPLX):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.register_buffer(
            "J_regressor_wham", torch.tensor(J_regressor_wham, dtype=torch.float32)
        )
        self.register_buffer(
            "J_regressor_eval", torch.tensor(J_regressor_eval, dtype=torch.float32)
        )
        self.register_buffer(
            "J_regressor_feet",
            torch.from_numpy(np.load(_C.BMODEL.JOINTS_REGRESSOR_FEET)).float(),
        )

        smplx_to_smpl = "model_files/uhc_data/wham_data/smplx2smpl.pkl"
        with open(smplx_to_smpl, "rb") as f:
            import pickle

            smplx_to_smpl = pickle.load(f, encoding="latin1")
        smplx2smpl = (
            torch.tensor(smplx_to_smpl["matrix"])
            .float()
            .to(self.J_regressor_wham.device)
        )
        self.J_regressor_wham_smplx = torch.matmul(
            self.J_regressor_wham, smplx2smpl
        ).cuda()
        self.J_regressor_feet_smplx = torch.matmul(
            self.J_regressor_feet, smplx2smpl
        ).cuda()

    def get_local_pose_from_reduced_global_pose(self, reduced_pose):
        full_pose = torch.eye(3, device=reduced_pose.device)[(None,) * 2].repeat(
            reduced_pose.shape[0], 24, 1, 1
        )
    ...
    def forward(
        ...
        def cvt_vars(x):
            return s2a(x.reshape(*x.shape[:2], -1, 6))[0]
        ...
        if cam is not None:
            ...
            weak_joints2d = weak_perspective_projection(
                joints3d,
                rotation=torch.eye(3, device=cam.device)
                .unsqueeze(0)
                .unsqueeze(0)
                .expand(*cam.shape[:2], -1, -1),
    ...
    def forward_nd(self, pred_rot6d, root, betas, return_full_pose=False):

        rotmat = transforms.rotation_6d_to_matrix(
            pred_rot6d.reshape(*pred_rot6d.shape[:2], -1, 6)
        ).reshape(-1, 24, 3, 3)
...
def weak_perspective_projection(
    ...
    if normalize_joints2d:
        projected_points = projected_points / (img_res / 2.0)
...
def convert_full_img_cam_to_pare(
    full_cam, bbox_height, bbox_center, img_w, img_h, focal_length, crop_res=224
):
