...
class TemporalSMPLify:
    ...
    def fit(self, init_pred, keypoints, bbox, **kwargs):
        ...
        optimizer = torch.optim.LBFGS(
            optim_params, lr=lr, max_iter=self.num_iters, line_search_fn="strong_wolfe"
        )

        loss_fn = SMPLifyLoss(init_pose=pose, device=self.device, **kwargs)

        closure = loss_fn.create_closure(optimizer, self.smpl, params, bbox, keypoints)

        for j in (j_bar := tqdm(range(self.num_steps), leave=False)):
            optimizer.zero_grad()
            loss = optimizer.step(closure)
            msg = f"Trans, S{j}, Loss: {loss.item():.1f}"
            j_bar.set_postfix_str(msg)

        # Stage 2. Optimize all params
        optimizer = torch.optim.LBFGS(
            params, lr=lr * BN, max_iter=self.num_iters, line_search_fn="strong_wolfe"
        )

        for j in (j_bar := tqdm(range(self.num_steps), leave=False)):
            optimizer.zero_grad()
            loss = optimizer.step(closure)
            msg = f"ALL, S{j}, Loss: {loss.item():.1f}"
            j_bar.set_postfix_str(msg)
