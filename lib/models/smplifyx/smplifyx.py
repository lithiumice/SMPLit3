...
face_mapping = np.concatenate(
    [np.arange(127, 127 + 17, dtype=np.int32), np.arange(76, 127, dtype=np.int32)],
    axis=0,
)
...
class SMPLifyLoss(torch.nn.Module):
    def __init__(
        self, res, cam_intrinsics, init_param_th_org, init_param_th, device, **kwargs
    ):
        ...
        def to_type(x):
            return x.float().to(device).detach().clone()
    ...
    def forward(
        ...
        losses = {}
        # import ipdb;ipdb.set_trace()
        scale = bbox[..., 2:].unsqueeze(-1) * 200.0
        ...
        if args.replace_lower:
            ...
            lower_reg += (
                (norm_pose[:, np.array([0, 1, 3, 4, 6, 7, 9, 10]) + 1]) ** 2
            ).mean()
        ...
        for k, v in pred_data.items():
            losses[f"{k}_proj"] = (
                (
                    gmof(pred_data[k] - parse_dst_data[k]["data"], sigma)
                    * parse_dst_data[k]["conf"]
                    * (parse_dst_data[k]["conf"] > 0.5)
                )
                / scale
            ).mean()
        ...
        for k in keys_to_reg:
            losses[f"{k}_reg"] = torch.linalg.norm(
                params[k] - self.init_param[k], dim=-1
            ).mean()

        # import ipdb;ipdb.set_trace()
        for k in in_keys_to_reg:
            losses[f"in_{k}_reg"] = torch.linalg.norm(
                params_in[k] - self.init_param[k], dim=-1
            ).mean()

        # Loss 4. Smooth loss
        for k in keys_to_jitter:
            losses[f"{k}_jitter"] = compute_jitter(params[k]).mean()

        for k in in_keys_to_jitter:
            losses[f"in_{k}_jitter"] = l2smooth(params_in[k])
        ...
        if "z_hL" in params_in:
            losses["H_pca_jitter"] = l2smooth(params_in["z_hL"]) + l2smooth(
                params_in["z_hR"]
            )

        if args.use_opt_pose_reg:
            kpts_conf = (parse_dst_data["B_kpts"]["conf"] > 0.5).float()
            pose_conf = torch.zeros([*(params["pose"].shape[:2]), 24, 1]).type_as(
                params["pose"]
            )
            pose_conf[:, :, list(k_cmpl_v_coco.keys()), :] = kpts_conf[
                :, :, list(k_cmpl_v_coco.values()), :
            ]
            losses["pose_reg"] = torch.linalg.norm(
                (params["pose"] - self.init_param_th_org["pose"]).reshape(-1, 24, 6)
                * (pose_conf),
                dim=-1,
            ).mean()
        ...
        if args.debug:
            log_str = ", ".join(
                [f"loss/{k}: {v.item():.4f}" for k, v in losses.items()]
            )
            print(log_str)
...
class TemporalSMPLifyX:
    ...
    def __init__(self, smplx=None, img_w=None, img_h=None, device=None):
    ...
    def fit(self, init_param_th, dst_data, args, bbox, **kwargs):
        # import ipdb;ipdb.set_trace()
        ...
        # <===========================
        def to_params(x):
            return x.to(self.device).requires_grad_(True)
        ...
        for stage_name, stage_info in stage_dict.items():
            ...
            loss_fn = SMPLifyLoss(
                init_param_th_org=init_param_th_org,
        ...
        for k in init_param_th:
            init_param_th_detach[k] = init_param_th[k].detach()
