...
class BodyProxy():
    ...
    def prepare_nemf_inp_data(self, body_pose):
        ...
        poses = torch.cat([torch.zeros(body_pose.shape[0],3).type_as(I), body_pose, 
                           torch.zeros(body_pose.shape[0],6).type_as(I)],dim=1)
        ...
        pos = pos.contiguous().view(-1, self.clip_length, 24, 3)  # local joint positions (N, T, J, 3)
        ...
        global_xform = global_xform.view(-1, self.clip_length, 24, 4, 4)  # global transformation matrix for each joint (N, T, J, 4, 4)
    ...
    def dec(self, proxy_var):
        ...
        nemf_output = self.nemf_model.decode(z_l, None, length=self.clip_length, step=1.0)
        rotmat = nemf_output['rotmat']  # (B, T, J, 3, 3)
        b_size, _, n_joints = rotmat.shape[:3]
        local_rotmat = self.nemf_fk.global_to_local(rotmat.view(-1, n_joints, 3, 3))  # (B x T, J, 3, 3)
        local_rotmat = local_rotmat.view(b_size, -1, n_joints, 3, 3)
        # import ipdb;ipdb.set_trace()
        local_rotmat = merge_tensor_w_avg((m2s(local_rotmat)), self.overlap_len, org_T)# (T, J, 3)        
        rec_pose = torch.cat([orient, local_rotmat[...,1:,:]], dim=1).unsqueeze(0).reshape(1,-1,24*6)
        # rec_pose: 1,T,24,6
