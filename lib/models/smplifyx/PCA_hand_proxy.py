...
class PCAHandsProxy:
    ...
    def dec(self, proxy_var):
        ...
        Lh_rec_pose, Rh_rec_pose = self.smplx.hand_pca_to_axis(
            self.smplx, Lh_pca, Rh_pca
        )
        rec_pose = torch.stack(
            [Lh_rec_pose.reshape(-1, 15, 3), Rh_rec_pose.reshape(-1, 15, 3)], dim=1
        ).unsqueeze(0)
        # rec_pose: [1, T, 2, 15, 3]
