...
class HandsProxy:
    def __init__(self):
        ...
        self.L_hands_mean = (
            toth(self.L_nemf_fk.smpl_data["hands_mean"].reshape(15, 3)).cuda().float()
        )
        self.R_hands_mean = (
            toth(self.L_nemf_fk.smpl_data["hands_mean"].reshape(15, 3)).cuda().float()
        )
    ...
    def prepare_nemf_inp_data(self, hand_pose, fk):
        ...
        # import ipdb;ipdb.set_trace()
        poses = torch.cat(
            [torch.zeros(hand_pose.shape[0], 1, 3).type_as(I), hand_pose], dim=1
        ).reshape(-1, 16 * 3)
    ...
    def dec(self, proxy_var):
        ...
        def dec_var(z_var, model, fk, mean):
            ...
            local_rotmat = merge_tensor_w_avg(
                (m2s(local_rotmat)), overlap_len, org_T
            )  # (T, J, 3)
        ...
        Lh_rec_pose = dec_var(
            proxy_var["cvt_vars"]["z_hL"],
            self.L_nemf_model,
            self.L_nemf_fk,
            self.L_hands_mean,
        )
        Rh_rec_pose = dec_var(
            proxy_var["cvt_vars"]["z_hR"],
            self.R_nemf_model,
            self.R_nemf_fk,
            self.R_hands_mean,
        )
