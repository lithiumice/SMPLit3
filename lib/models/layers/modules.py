...
class Regressor(nn.Module):
    def __init__(
        self, in_dim, hid_dim, out_dims, init_dim, layer="LSTM", n_layers=2, n_iters=1
    ):
        super().__init__()
...
class NeuralInitialization(nn.Module):
    ...
    def forward(self, x):
        ...
        out = (
            out.view(b, self.num_inits, self.n_layers, -1)
            .permute(1, 2, 0, 3)
            .contiguous()
        )
...
class MotionEncoder(nn.Module):
    def __init__(self, in_dim, d_embed, pose_dr, rnn_type, n_layers, n_joints):
        super().__init__()
    ...
    def forward(self, x, init):
        ...
        motion_context = torch.cat(
            (motion_context, pred_kp3d.reshape(self.b, self.f, -1)), dim=-1
        )
        return pred_kp3d, motion_context


class TrajectoryDecoder(nn.Module):
    def __init__(self, d_embed, rnn_type, n_layers):
        super().__init__()
...
class MLP(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.fc2.weight.data = torch.eye(output_size)
...
class MotionDecoder(nn.Module):
    def __init__(self, d_embed, rnn_type, n_layers):
        ...
        self.neural_init = NeuralInitialization(
            len(_C.BMODEL.MAIN_JOINTS) * 6, d_embed, rnn_type, n_layers
        )
        ...
        # TODO: add correction layer in here, fix contact predict
        if 0:
            self.corr_module = nn.Linear(4, 4)
            self.corr_module.weight.data = torch.eye(4)
            self.corr_module.bias.data.fill_(0)
    ...
    def forward(self, x, init):
        ...
        for i in range(f):
            ...
            # TODO: add correction layer in here, fix contact predict
            if 0:
                pred_contact = self.corr_module(pred_contact)
...
class TrajectoryRefiner(nn.Module):
    def __init__(self, d_embed, d_hidden, rnn_type, n_layers):
        super().__init__()

        d_input = d_embed + 12
        self.refiner = Regressor(d_input, d_hidden, [6, 3], 9, rnn_type, n_layers)
    ...
    def forward(self, context, pred_vel, output, cam_angvel, return_y_up):
        ...
        feet_vel = (
            torch.cat(
                (torch.zeros_like(feet[:, :1]), feet[:, 1:] - feet[:, :-1]), dim=1
            )
            * 30
        )  # Normalize to 30 times
        ...
        (delta_root, delta_vel), _, _ = self.refiner(
            inpt_feat, [pred_root[:, 1:], pred_vel], h0=None
        )
        ...
        if 1:
            ...
            print("[WARNING] here block refine vel")

        root_world, trans_world = rollout_global_motion(pred_root, pred_vel)

        if return_y_up:
            yup2ydown = (
                axis_angle_to_matrix(torch.tensor([[np.pi, 0, 0]]))
                .float()
                .to(root_world.device)
            )
