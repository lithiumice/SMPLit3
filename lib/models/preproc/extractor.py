    ...
    def run(self, video, tracking_results, patch_h=256, patch_w=256, stride = 1):
        ...
        while True:
            ...
            for _id, val in tracking_results.items():
                ...
                if self.flip_eval:
                    if not 'flipped_bbox' in tracking_results[_id]: tracking_results[_id]['flipped_bbox'] = []
                    if not 'flipped_keypoints' in tracking_results[_id]: tracking_results[_id]['flipped_keypoints'] = []
                    if not 'flipped_features' in tracking_results[_id]: tracking_results[_id]['flipped_features'] = []
