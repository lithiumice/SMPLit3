...
class FeatureExtractor(object):
    def __init__(self, device, flip_eval=False, max_batch_size=64):
        ...
        ckpt = "model_files/wham_ckpt/hmr2a.ckpt"
        self.model = hmr2(ckpt).to(device).eval()

    def run(self, video, tracking_results, patch_h=256, patch_w=256, stride=1):
        ...
        frame_id = 0
        bar = Bar("Feature extraction ...", fill="#", max=length // stride)
        while True:
            if is_video:
            ...
            for _id, val in tracking_results.items():
                ...
                norm_img, crop_img = process_image(
                    img[..., ::-1], [cx, cy], scale, patch_h, patch_w
                )
                ...
                if frame_id2 == 0:  # First frame of this subject
                    tracking_results = self.predict_init(
                        norm_img, tracking_results, _id, flip_eval=False
                    )

                if self.flip_eval:
                    if not "flipped_bbox" in tracking_results[_id]:
                        tracking_results[_id]["flipped_bbox"] = []
                    if not "flipped_keypoints" in tracking_results[_id]:
                        tracking_results[_id]["flipped_keypoints"] = []
                    if not "flipped_features" in tracking_results[_id]:
                    ...
                    flipped_features = self.model(
                        torch.flip(norm_img, (3,)), encode=True
                    )
                    tracking_results[_id]["flipped_features"].append(
                        flipped_features.cpu()
                    )

                    if frame_id2 == 0:
                        tracking_results = self.predict_init(
                            torch.flip(norm_img, (3,)),
                            tracking_results,
                            _id,
                            flip_eval=True,
                        )
    ...
    def predict_init(self, norm_img, tracking_results, _id, flip_eval=False):
        ...
        pred_global_orient, pred_body_pose, pred_betas, _ = self.model(
            norm_img, encode=False
        )
