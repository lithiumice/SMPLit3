...
class Face_var_init_deca():
    def __init__(self):
        ...
        import sys;sys.path.append(DECA_PATH)
    ...
    def get_result(
        ...
        if len(image.shape) == 2:image = image[:,:,None].repeat(1,1,3)
        if len(image.shape) == 3 and image.shape[2] > 3:image = image[:,:,:3]
        # h, w, _ = image.shape
...
class Face_var_init_smirk():
    def __init__(self):
        # SMIRK
        # import ipdb;ipdb.set_trace()
        import sys;sys.path.append(SMIRK_PATH)
...
class Body_var_init_hmr2():
    def __init__(self):
        # INIT 4D human
        import sys; sys.path.append(HMR2_PATH)    
    ...
    def get_result(
        ...
        for batch in dataloader:
            ...
            with torch.no_grad():
                ...
                scaled_focal_length = self.hmr2_model_cfg.EXTRA.FOCAL_LENGTH / self.hmr2_model_cfg.MODEL.IMAGE_SIZE * img_size.max()
                pred_cam_t_full = cam_crop_to_full(pred_cam, box_center, box_size, img_size, scaled_focal_length).detach().cpu().numpy()
                
                pose_result['hmr_orient'] = tonp(m2a(out['pred_smpl_params']['global_orient'][0]))#1,3
                pose_result['hmr_body_pose'] = tonp(m2a(out['pred_smpl_params']['body_pose'][0]))#23,3
                pose_result['hmr_betas'] = tonp(out['pred_smpl_params']['betas'])#1,10
...
class Body_var_init_smplerx():
    def __init__(self):
        import sys;sys.path.append(SMPLERX_PATH)
...
class Hand_var_init_hamer():
    def __init__(self):
        ...
        import sys; sys.path.insert(0, HAMER_PATH)
    ...
    def get_result(
        ...
        # 得到mano参数
        pose_result['hand_pose'] = out['pred_mano_params']['hand_pose'].cpu().numpy()#[2, 15, 3, 3]
        return pose_result
...
class Hand_var_init_acr():
    def __init__(self):
        # init ACR
        import os;os.environ['PYOPENGL_PLATFORM'] = 'osmesa'
        import sys; sys.path.append(ACR_PATH)    
    ...
    def get_result(
        ...
        pose_result['hand_pose'] = tonp(a2m(outputs['params_dict']['poses'][:,3:].reshape(2,15,3)))
        return pose_result
...
class DetectionModel(object):
    def __init__(self, device, args):
        ...
        if self.args.use_smplerx:
            self.var_initers.append(
                Body_var_init_smplerx()
            )
        ...
        if self.args.vitpose_model=='body_only':
            ...
            self.pose_model = init_pose_model(
                osp.join(VIT_DIR, 'configs/body/2d_kpt_sview_rgb_img/topdown_heatmap/coco/ViTPose_huge_coco_256x192.py'), 
            ...
            self.cpm = ViTPoseModel(
                'cuda',
                model_name=self.args.vitpose_model
            )              
    ...
    def track(
        ...
        # img: np array
        # bbox detection
        # TODO: add anno bbox here
        if pre_def_bboxes is not None:
            bboxes = pre_def_bboxes
        else:
            bboxes = self.bbox_model.predict(img, device=self.device, classes=0, conf=BBOX_CONF, save=False, verbose=False)[0].boxes.xyxy.detach().cpu().numpy()
        ...
        if self.args.vitpose_model=='body_only':
            ...
            if len(bboxes)>0:
                ...
                vitpose_results = self.cpm.predict_pose(img,[np.concatenate([pred_bboxes, np.ones_like(pred_bboxes[:,:1])], axis=1)],)
        ...
        for pose_result in pose_results:
            ...
            for k_to_add in self.all_vars_to_save:
                if k_to_add in pose_result.keys():
                    if k_to_add not in self.tracking_results.keys(): self.tracking_results[k_to_add] = []
