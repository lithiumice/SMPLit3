    ...
    def render_mesh(self, vertices, background, colors=[0.8, 0.8, 0.8]):
        ...
        if False:
            if not hasattr(self,'verts_features'):
                ...
                with open(segm,'rb') as file: segm_data = pickle.load(file, encoding='latin1')
                # import ipdb;ipdb.set_trace() 
                
                verts_features = torch.zeros((1, vertices.shape[1], 3)).type_as(vertices)
                ...
                for idx in range(3):
                    verts_features[0,self.faces[0,:,idx],:]=part_color_map[tmp.long()]
        ...
        if False:
            ...
            depth_img = (depth_img-depth_img[vis_mask].min())/(depth_img[vis_mask].max()-depth_img[vis_mask].min())
            normal_img = (normal_img-normal_img[vis_mask].min())/(normal_img[vis_mask].max()-normal_img[vis_mask].min())
        ...
        rgb_img = self.soft_phong_shader(fragment, mesh, materials=materials, cameras=self.cameras, lights=self.lights)
        ...
        if False:
            # import ipdb;ipdb.set_trace() 
            depth_img = overlay_image_onto_background(depth_img.unsqueeze(-1).repeat(1,1,3), mask, self.bboxes, np.zeros_like(background))
            normal_img = overlay_image_onto_background(normal_img, mask, self.bboxes, np.zeros_like(background))
