...
def get_ffmpeg_args(use_nvdec=True):
    if use_nvdec:
        ffmpeg_bin_path = (
            "/is/cluster/fast/hyi/workspace/VidGen/ffmpeg/local/bin/ffmpeg"
        )
...
def save_crop_vid(
    ...
    ffmpeg_bin_path, mp4_codec_name, ffmpeg_cmd_prefix = get_ffmpeg_args(
        use_nvdec=use_nvdec
    )
    ...
    if move_area_bbox is not None:
        ...
        ffmpeg_cmd += [
            (
                ...
                if use_nvdec
                else f'-vf "crop={ww}:{hh}:{x0}:{y0}"'
            ),  # "-filter:v", f"crop={ww}:{hh}:{x0}:{y0}",
        ]
...
def get_wav_full(audiostart, audioend, total_wav_path, part_wav_path):
    command = (
        "ffmpeg -y -i %s -ac 1 -vn -acodec pcm_s16le -ar 16000 -ss %.3f -to %.3f %s -loglevel error"
        % (str(total_wav_path), audiostart, audioend, part_wav_path)
    )
...
def run_vis_on_demo(
    ...
    for sid in results.keys():
        ...
        if (
            not Path(
                origin_fps_raw_split_vid_save_path := osp.join(
                    output_pth, f"{fname}_raw_origin_fps.mp4"
                )
            ).exists()
            or must_redump
        ):
        ...
        try:
            ...
            if not Path(save_path).exists():
                ...
                DATA_PROCESS_CODE = (
                    "/is/cluster/fast/hyi/workspace/VidGen/talking_avatar_data"
                )
        ...
        if vis_global:
            ...
            sx, sz = (verts_glob.mean(1).max(0)[0] - verts_glob.mean(1).min(0)[0])[
                [0, 2]
            ]
        ...
        for frame_i in results[sid]["frame_ids"]:
            ...
            if vis_global:
                ...
                if frame_i in results[sid]["frame_ids"]:
                    ...
                    cameras = renderer.create_camera(
                        global_R[frame_i3], global_T[frame_i3]
                    )
