...
def main(cfg, args):
    ...
    J_regressor_eval = (
        torch.from_numpy(np.load(_C.BMODEL.JOINTS_REGRESSOR_H36M))[
            _C.KEYPOINTS.H36M_TO_J14, :
        ]
        .unsqueeze(0)
        .float()
        .to(cfg.DEVICE)
    ...
    with torch.no_grad():
        for i, batch in enumerate(eval_loader):
            ...
            renderer = Renderer(
                width, height, focal_length, cfg.DEVICE, smpl["neutral"].faces
            )
            ...
            writer = imageio.get_writer(
                osp.join(viz_pth, batch["vid"][0] + ".mp4"),
                mode="I",
                format="FFMPEG",
                fps=30,
                macro_block_size=1,
