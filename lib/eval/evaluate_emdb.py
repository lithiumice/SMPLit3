...
def main(cfg, args):
    if args.post_fitting:
        import sys; sys.path.append('/root/apdcephfs/private_wallyliang/PLANT/Thirdparty/WHAM')
    ...
    with torch.no_grad():
        for i, batch in enumerate(eval_loader):
            ...
            if args.post_fitting:
                ...
                # import ipdb;ipdb.set_trace()
                pred_root_world = transforms.matrix_to_axis_angle(pred['poses_root_world']).cpu().numpy().reshape(-1, 3)
                pred_body_pose = transforms.matrix_to_axis_angle(pred['poses_body']).cpu().numpy().reshape(-1, 69)
