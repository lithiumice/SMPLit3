...
m2mm = 1e3
smplx2smpl = (
    torch.from_numpy(joblib.load(_C.BMODEL.SMPLX2SMPL)["matrix"])
    .unsqueeze(0)
    .float()
    .cuda()
)
...
def main(cfg, args):
    ...
    with torch.no_grad():
        for i, batch in enumerate(eval_loader):
            ...
            smplx = SMPLX(
                _C.BMODEL.FLDR.replace("smpl", "smplx"),
                gender=batch["gender"][0],
                batch_size=len(pred_verts),
            ).to(cfg.DEVICE)
            gt_pose = transforms.matrix_to_axis_angle(
                transforms.rotation_6d_to_matrix(gt["pose"][0])
            )
            target_output = smplx(
            ...
            target_j3d = torch.matmul(
                J_regressor_eval, target_verts.to(cfg.DEVICE)
            ).cpu()
            # =======>
            ...
            pa_mpjpe = (
                torch.sqrt(((S1_hat - target_j3d) ** 2).sum(dim=-1))
                .mean(dim=-1)
                .numpy()
                * m2mm
            )
            mpjpe = (
                torch.sqrt(((pred_j3d - target_j3d) ** 2).sum(dim=-1))
                .mean(dim=-1)
                .numpy()
                * m2mm
            )
            pve = (
                torch.sqrt(((pred_verts - target_verts) ** 2).sum(dim=-1))
                .mean(dim=-1)
                .numpy()
                * m2mm
            )
            accel = compute_error_accel(joints_pred=pred_j3d, joints_gt=target_j3d)[
                1:-1
            ]
