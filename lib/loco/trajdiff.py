...
SHOW_to_AMASS_cvt = torch.tensor([[ 1.,  0.,  0.],[ 0.,  0., -1.],[ 0.,  1.,  0.]])
...
def tt(x): return x.float().cuda()
def t2t(x): return torch.from_numpy(x).float().cuda()
...
def get_overlap_pair(splits, overlap_len, B):
    ...
    for idx in range(1,split_num):
        pairs.append([
            splits[idx-1][-overlap_len:],
            splits[idx][:overlap_len]
        ])
    return pairs        
...
def split_tensor(tensor, clip_len, overlap_len):
    ...
    tmp = tensor
    if m>0: tmp = torch.cat((tmp, tensor[-1:].repeat(m,1)), dim=0)
...
def merge_tensor(splits, overlap_len, B):
    ...
    clip_len = splits.shape[1]
    step = clip_len - overlap_len
    merged_tensor = torch.cat([splits[idx][:step] if idx!=split_num-1 else splits[idx] for idx in range(split_num)],dim=0)[:B]
...
def merge_tensor_w_avg(splits, overlap_len, B):
    ...
    for ii in range(len(splits.shape)-2): linear_weight = linear_weight.unsqueeze(-1)
    
    for pairs_idx in range(len(pairs)):
        avg_part = linear_weight*pairs[pairs_idx][1] + (1-linear_weight)*pairs[pairs_idx][0]
...
def save_all_npz(
    ...
    for _id in results.keys():
        ...
        poses = torch.cat([global_orient, pose_var, torch.zeros((pose_var.shape[0], 6), device = pose_var.device)], dim = 1).clone().reshape(-1,24,3).cpu()
        orient = poses[:,0,:]
        transl = apply_cvt_T(SHOW_to_AMASS_cvt, trans_world).detach().cpu().numpy()
        poses[:,0,:] = apply_cvt_R(SHOW_to_AMASS_cvt, orient, in_type='aa', out_type='aa')    
        ...
        to_save = {
            ...
            'trans': transl, # B,3
            'exp': exp, #T,50
            'poses': np.concatenate([poses[:,:22,:], jaw, leye, reye, hand_pose], axis=1).reshape(-1, 55, 3)[::1],
            'gender': 'male',
            'mocap_framerate': fps,
        }
        
        # if tracking_results is not None:
        #     dst_data = tracking_results[_id]
...
class DiffTraj():
    def __init__(self):
        ...
        model_path = 'model_files/DiffTraj_models/traj_feet_trajRot_12_8/model000100000.pt'
        use_old_model = True
        seq_len = 200
        ...
        self.male_bm = BodyModel(bm_fname=male_bm_path, num_betas=10, num_dmpls=8, dmpl_fname=male_dmpl_path)
        ...
        if seq_len is not None: self.difftraj_args.seq_len = seq_len

        if 'use_ar' not in self.difftraj_args: self.difftraj_args.use_ar = False

        if self.difftraj_args.diffusion_steps>200: use_ddim = 1 
        else: use_ddim = 0        
        if use_ddim:self.difftraj_args.diffusion_steps=100
        ...
        load_model_wo_clip(self.difftraj_model, torch.load(model_path, map_location='cpu'))

        # import ipdb;ipdb.set_trace()
        if self.difftraj_args.guidance_param != 1:
            self.difftraj_model = ClassifierFreeSampleModel(self.difftraj_model)   # wrapping self.difftraj_model with the classifier-free sampler
        ...
        if use_ddim: self.sample_fn = diffusion.ddim_sample_loop
        else: self.sample_fn = diffusion.p_sample_loop
        # <----
    ...
    def pose_to_traj(self, pred_root_world, pred_body_pose, pred_trans_world):
        ...
        if pred_body_pose.shape[-2] == 21 or pred_body_pose.shape[-1] == 63:
            pose_body_all = torch.cat([pred_body_pose.detach().reshape(-1,21,3), torch.zeros(pred_body_pose.shape[0],2,3).to(device)],dim=1)
        else:
        ...
        orient = apply_cvt_R(SHOW_to_AMASS_cvt, pred_root_world, in_type='aa', out_type='aa')
        trans = apply_cvt_T(SHOW_to_AMASS_cvt, pred_trans_world)
        
        # import ipdb;ipdb.set_trace()
        pose_seq = self.male_bm(pose_body=pose_body_all[:1,:21,:].reshape(-1,63),root_orient=orient[:1],trans=trans[:1]).Jtr[:,:22,:]
        min_z_first = pose_seq[0,:,2].min()
        trans[:,2] -= min_z_first
                
        local_rep_slam_cam_to_world = traj_global2local_heading(trans, a2q(orient.contiguous()))
        ...
        if self.difftraj_args.use_ar:
            ...
            precompute_motions = torch.cat([local_rep_slam_cam_to_world, pose_body_6d, local_pose],dim=-1)   
            precompute_motions = (precompute_motions-mean)/std
            
            # cur_point = 0
            p_motion = precompute_motions[self.difftraj_args.p_len*0:self.difftraj_args.p_len*1]
            f_motion = precompute_motions[self.difftraj_args.p_len*1:self.difftraj_args.p_len*1+self.difftraj_args.f_len]
            ...
            model_kwargs_custom['y']['scale'] = torch.ones(1, device=device) * self.difftraj_args.guidance_param
            ...
            for t in range(self.difftraj_args.p_len, origin_length, infer_step):            
                ...
                model_kwargs_custom['y']['f_pose'] = f_pose[None].permute(0,2,1).unsqueeze(2)      
                model_kwargs_custom['y']['p_motion'] = ar_model_output_list[:,:,:,t-self.difftraj_args.p_len:t]   
                                
                sample_custom = self.sample_fn(
                    self.difftraj_model,
                    (len(batch), self.difftraj_model.njoints, self.difftraj_model.nfeats, self.difftraj_args.f_len), 
                    clip_denoised=False,
                ...
                ar_model_output_list = torch.cat([ar_model_output_list, sample_custom[:,:,:,:infer_step]], dim=-1) #[1, 203, 1, T]
                          
                # model_kwargs['y']['p_motion'] = ar_model_output_list[:,:,:,-past_motion_offset:]                             
            sample_custom = ar_model_output_list[:,:,:,:origin_length]
            sample_rep_pred_list = [sample_custom[idx, :, 0,:].permute(1,0) for idx in range(sample_custom.shape[0])] 
            
        else:
            pose_chunk = split_tensor(pose_body_all.reshape(-1,69), self.seq_len, difftraj_overlap_len)
            vec_chunk = split_tensor(traj_vec, self.seq_len, difftraj_overlap_len)
                                  
            for chunk_idx in range(len(pose_chunk)):
                pose_body = pose_chunk[chunk_idx].reshape(-1,23,3)[:,:21,:].reshape(-1,63)
                body = self.male_bm(pose_body=pose_body)
                pose_seq = body.Jtr[:,:22,:]
            ...
            sample_custom = self.sample_fn(
                self.difftraj_model,
                (len(batch), self.difftraj_model.njoints, self.difftraj_model.nfeats, local_pose.shape[0]), 
                clip_denoised=False,
                model_kwargs=model_kwargs_custom,
                skip_timesteps=0, 
                init_image=None,
        ...
        # sample_rep_pred = merge_tensor(sample_rep_pred_list, difftraj_overlap_len, origin_length)
        sample_rep_pred = merge_tensor_w_avg(sample_rep_pred_list, difftraj_overlap_len, origin_length)
        ...
        transl_amass_pred, orient_q_amass_pred = traj_local2global_heading(traj_local_pred)
        root = apply_cvt_R(AMASS_to_SHOW_cvt, orient_q_amass_pred, in_type='quat', out_type='aa')
...
if __name__ == "__main__":
    ...
    def pose_to_traj(npz_file, output_pth):
        ...
        difftraj_root, difftraj_trans = traj_predictor.pose_to_traj(pred_root_world, pred_body_pose, pred_trans_world)    
        results = {
            '0': {
        ...
        save_all_npz(results, 'difTraj_raw', output_pth = output_pth)
